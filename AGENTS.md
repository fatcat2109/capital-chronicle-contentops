...
- Annotated tag `v1.0` exists at immutable release commit `6983bfb3ef300414b744f3f8f97ca81ff699348b`; the release is operator accepted. The reusable backend foundation now derives semantic authority, permission, roles, candidate authorization, and exact-set feature aggregation from registered extraction contracts and consumed evidence; the current next action is the independent audit named in `next_task_pointer.md`.
