...
def run():
    target_dir = r"scratch\v4_recheck_0174T"
    ...
    with sync_playwright() as p:
        ...
        nav_item = page.get_by_role("button", name="Content Calendar / Workflow", exact=True)
        if nav_item.count() > 0: nav_item.nth(0).click()
        page.wait_for_timeout(500)
        page.screenshot(path=os.path.join(target_dir, "1440x900_content_calendar.png"), full_page=True)
        
        # Visual Export / Screenshot-Safe
        nav_item = page.get_by_role("button", name="Visual Export / Screenshot-Safe", exact=True)
        if nav_item.count() > 0: nav_item.nth(0).click()
        page.wait_for_timeout(500)
    ...
    print("Recheck 0174T screenshots captured.")
