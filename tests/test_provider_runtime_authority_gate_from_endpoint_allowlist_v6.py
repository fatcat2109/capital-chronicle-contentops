...
from live_contentops.provider_runtime_authority_gate_from_endpoint_allowlist_v6 import *
...
SAMPLE=Path('docs/automation/V6_ENDPOINT_ALLOWLIST_GATE_FROM_OFFICIAL_PROVIDER_DOCS_SCOPE_HEAVY_BATCH_DOCS_ONLY_NO_PROVIDER_NO_DISPATCH_NO_LIVE/sample_endpoint_allowlist_gate_bundle.json')
def _default(): return json.loads(SAMPLE.read_text(encoding='utf-8'))
def _valid(): return asdict(make_endpoint_allowlist_gate_bundle(asdict(make_official_provider_docs_scope_gate_bundle(docs_valid(), official_docs_fixture()))))
def _gate(u=None): return make_provider_runtime_authority_gate_bundle(_valid() if u is None else u)
def test_default_sample_blocked():
 b=_gate(_default()); assert b.provider_runtime_authority_gate_status=='blocked_invalid_endpoint_allowlist_or_provider_runtime_authority'; assert b.provider_runtime_authority_records==[]; assert b.eligible_for_future_credential_hydration_gate_task is False
def test_valid_synthetic_runtime_records_only_and_mapping():
 b=_gate(); s=json.dumps(asdict(b)); assert b.provider_runtime_authority_gate_status==GATE_STATUS; assert b.eligible_for_future_credential_hydration_gate_task is True; assert b.eligible_for_future_exact_payload_rehydration_gate_task is False; assert b.eligible_for_future_destination_resolution_gate_task is False; assert b.eligible_for_future_request_shape_gate_task is False; assert b.eligible_for_live_send_now is False; assert len(b.provider_runtime_authority_records)==3
 assert set(b.sanitized_endpoint_operation_ids)==ALLOWED_OPS
 for r in b.provider_runtime_authority_records:
  assert r['runtime_authority_symbolic_only'] is True and r['runtime_authority_prerequisites_present'] is True and r['runtime_authority_does_not_authorize_execution'] is True and r['non_executable_runtime_authority_record'] is True
...
def test_raw_and_unsafe_flags_fail_closed():
 for k in ['provider_call_made','network_call_made','browser_session_used','dispatch_allowed','live_send_allowed','executable_request_artifact_created','endpoint_url_present','webhook_url_present','channel_id_present','account_id_present','token_present','payload_body_present','public_url_created','metrics_created']:
  u=_valid(); u['endpoint_allowlist_records'][0][k]=True; b=_gate(u); assert b.eligible_for_future_credential_hydration_gate_task is False; assert any(k in x for x in b.blockers)
...
def test_runtime_output_marker_tamper_fails_closed():
 u=_valid(); b=_gate(u); d=asdict(b); d['provider_runtime_authority_records'][0]['runtime_authority_does_not_authorize_execution']=False; assert any('runtime_authority_does_not_authorize_execution_not_true' in x for x in validate_runtime_records(d['provider_runtime_authority_records']))
 d['provider_runtime_authority_records'][0]['non_executable_runtime_authority_record']=False; assert any('non_executable_runtime_authority_record_not_true' in x for x in validate_runtime_records(d['provider_runtime_authority_records']))
def test_docs_sample_hygiene():
 base=Path('docs/automation/V6_PROVIDER_RUNTIME_AUTHORITY_GATE_FROM_ENDPOINT_ALLOWLIST_HEAVY_BATCH_NO_PROVIDER_NO_DISPATCH_NO_LIVE')
 paths=[base/'implementation_report.md',base/'provider_runtime_authority_gate_contract.md',base/'sample_provider_runtime_authority_gate_bundle.json',Path('docs/runbooks/V6_PROVIDER_RUNTIME_AUTHORITY_GATE_OPERATOR_RUNBOOK_NO_PROVIDER_NO_DISPATCH_NO_LIVE.md')]
 for p in paths:
 ...
 d=json.loads(paths[2].read_text(encoding='utf-8')); assert d['provider_runtime_authority_gate_status']=='blocked_invalid_endpoint_allowlist_or_provider_runtime_authority'; assert d['eligible_for_future_credential_hydration_gate_task'] is False
