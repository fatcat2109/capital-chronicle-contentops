...
def test_status_last_updated_and_accepted_task_are_consistent_after_metadata_repair():
    ...
    assert status["last_updated_by_task"] == "TASK_CONTENTOPS_V6_FAST_SHIP_LIVE_DISPATCH_META_FAMILY_V0"
    assert "## last_updated_by_task\nTASK_CONTENTOPS_V6_FAST_SHIP_LIVE_DISPATCH_META_FAMILY_V0" in md_text
    assert status["latest_accepted_task"] == "TASK_CONTENTOPS_V6_FAST_SHIP_LIVE_DISPATCH_META_FAMILY_V0"
    assert status["current_product_phase"] == "TASK 0088 Facebook Page, Threads, and Instagram Graph/official API adapters integrated and verified under Fast Ship Mode"
