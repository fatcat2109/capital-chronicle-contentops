...
def test_status_last_updated_and_accepted_task_are_consistent_after_metadata_repair():
    ...
    assert status["current_product_phase"] == "Periodic final release go/no-go rehearsal refreshed release evidence, red-team posture, browser-QA boundary, and canonical V5 validation"
...
def test_ui_status_hardening_after_task_0059_is_non_semantic():
    ...
    assert status["last_updated_by_task"] == "TASK_CONTENTOPS_V6_FINAL_RELEASE_GO_NO_GO_REHEARSAL_V0"
    assert status["last_updated_by_task"] == status["latest_accepted_task"]
    assert guardrail in status_text or "fast ship" in status_text or "go/no-go rehearsal" in status_text
    assert guardrail in md_text or "task_0069 status acknowledges accepted v5 final readiness ui hardening" in md_text or "go/no-go rehearsal" in md_text
