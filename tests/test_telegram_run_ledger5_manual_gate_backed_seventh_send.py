...
RUNNER_PATH = REPO_ROOT / "tools/telegram_run_ledger5_manual_gate_backed_seventh_send.py"
GATE_PACKET_PATH = REPO_ROOT / "docs/automation/0174WD_WE_WF/telegram_ledger5_next_manual_gate_packet.json"
REMOTE_PACKET_PATH = REPO_ROOT / "docs/automation/0174WA_WB_WC/telegram_ledger5_remote_operator_loop_state_packet.json"


def load_module():
    spec = importlib.util.spec_from_file_location("seventh_runner", RUNNER_PATH)
...
def test_import_has_no_side_effects():
    result = subprocess.run(
        [sys.executable, "-c", "import importlib.util; s=importlib.util.spec_from_file_location('r', r'tools/telegram_run_ledger5_manual_gate_backed_seventh_send.py'); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); print('imported')"],
...
def test_default_runner_no_env_network(runner, manual_gate, remote_packet, existing):
    ...
    res = runner.run_ledger5_manual_gate_backed_send(
...
def test_mocked_success_advances_ledger_5_to_6(runner, manual_gate, remote_packet, existing):
    transport = CountingTransport()
    res = runner.run_ledger5_manual_gate_backed_send(
        manual_gate_packet=manual_gate, remote_loop_packet=remote_packet,
        operator_live_send_enabled=True, token="token", destination="__approved_destination__",
    ...
    assert res["append"]["ledger_entry_count"] == 6
...
def test_proof_doc_deterministic_and_scanner_clean(runner, manual_gate, remote_packet, existing):
    ...
    res1 = runner.run_ledger5_manual_gate_backed_send(
        manual_gate_packet=manual_gate, remote_loop_packet=remote_packet,
        operator_live_send_enabled=True, token="token", destination="__approved_destination__",
    ...
    res2 = runner.run_ledger5_manual_gate_backed_send(
        manual_gate_packet=manual_gate, remote_loop_packet=remote_packet,
        operator_live_send_enabled=True, token="token", destination="__approved_destination__",
