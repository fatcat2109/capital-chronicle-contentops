"""UI guardrail tests for V6 Explicit Live Scope Gate."""
...
ADAPTER = V5 / "data" / "explicitLiveScopeGateSourceCandidateAdapter.ts"
...
def test_surfaces_import_adapter() -> None:
    for path in SURFACES:
        text = _read(path)
        assert "explicitlivescopegatesourcecandidateadapter" in text
        assert "explicitlivescopegatepacket" in text
