...
def test_registry_declares_edge_only_canonical_authority():
    ...
    assert packet["allowed_cdp_ports"] == [9222, 9223]


def test_doctor_rejects_chrome_and_recommends_free_fallback_port(monkeypatch):
    chrome = _process(
        "chrome.exe",
        101,
    ...
    assert by_port[9222]["status"] == "blocked_by_noncanonical_owner"
