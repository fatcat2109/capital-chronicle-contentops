import json
from dataclasses import asdict
from pathlib import Path

from live_contentops.local_payload_review_hash_approval_ledger_v6 import (
    make_payload_review_approval_ledger_packet,
    write_payload_review_approval_ledger_packet,
    load_json_packet,
    main,
)


def _staging():
    return {
        "schema_version": "6.0.0",
        "task_label": "TASK_CONTENTOPS_V6_LOCAL_PLATFORM_VARIANT_PREVIEW_STAGING_FROM_METADATA_VALUES_V0",
        "variant_preview_staging_id": "local_platform_variant_preview_staging_abc123",
        "metadata_values_review_id": "operator_metadata_values_review_abc123",
        "metadata_values_review_sha256": "h" * 64,
        "metadata_values_id": "operator_metadata_values_abc123",
        "metadata_proposal_id": "seo_editorial_metadata_proposal_abc123",
        "source_pack_intake_id": "operator_source_pack_intake_abc123",
        "source_pack_id": "operator_source_pack_abc123",
        "editorial_workflow_id": "accepted_review_editorial_workflow_abc123",
        "canonical_title": "Sample Title Grounding Analysis",
        "canonical_slug": "sample-title-grounding-analysis",
        "preview_platforms": ["substack", "discord"],
        "preview_files": [
            "A:/staging/sample-title-grounding-analysis_substack_preview.md",
            "A:/staging/sample-title-grounding-analysis_discord_preview.md"
        ],
        "variant_preview_staging_available": True,
        "variant_previews_generated": True,
        "preview_only": True,
        "generated_citations_allowed": False,
        "citations_verified": False,
        "approved_canonical_article_available": False,
        "publication_ready": False,
        "dispatch_allowed": False,
        "platform_variant_generation_allowed": False,
        "outbox_creation_allowed": False,
        "public_url": None,
        "public_metrics": None,
        "review_only": True,
        "human_review_required": True,
        "kill_switch_active": True,
        "runtime_truth": False,
        "blockers": [],
        "warnings": [],
    }


def _intent():
    return {
        "schema_version": "6.0.0",
        "approval_intent_id": "approval_intent_abc123",
        "operator_id": "jim",
        "created_at_manual": "2026-06-29T23:00:00+07:00",
        "variant_preview_staging_id": "local_platform_variant_preview_staging_abc123",
        "reviewed_preview_files": [
            "A:/staging/sample-title-grounding-analysis_substack_preview.md",
            "A:/staging/sample-title-grounding-analysis_discord_preview.md"
        ],
        "approval_phrase": "REVIEWED_LOCAL_PREVIEWS_ONLY_NOT_APPROVED_FOR_DISPATCH",
        "approval_scope": "payload_review_hash_only",
        "notes": "Safe notes.",
    }


def _preview_texts():
    return {
        "A:/staging/sample-title-grounding-analysis_substack_preview.md": "# Sample Title\nLOCAL PREVIEW ONLY - NOT APPROVED FOR PUBLICATION\nSafe body content.",
        "A:/staging/sample-title-grounding-analysis_discord_preview.md": "LOCAL PREVIEW ONLY - NOT APPROVED FOR DISCORD DISPATCH\nSafe discord content.",
    }


def _paths():
    return [
        Path("A:/staging/sample-title-grounding-analysis_substack_preview.md"),
        Path("A:/staging/sample-title-grounding-analysis_discord_preview.md"),
    ]


def _assert_no_public_state(packet):
    assert packet.approval_for_dispatch is False
    assert packet.approval_for_outbox_creation is False
    assert packet.approval_for_publication is False
    assert packet.approved_canonical_article_available is False
    assert packet.publication_ready is False
    assert packet.dispatch_allowed is False
    assert packet.platform_variant_generation_allowed is False
    assert packet.outbox_creation_allowed is False
    assert packet.generated_citations_allowed is False
    assert packet.citations_verified is False
    assert packet.public_url is None
    assert packet.public_metrics is None
    assert packet.review_only is True
    assert packet.human_review_required is True
    assert packet.kill_switch_active is True
    assert packet.runtime_truth is False


def test_valid_inputs_emits_ledger_packet(tmp_path):
    st = _staging()
    paths = _paths()
    texts = _preview_texts()
    intent = _intent()
    
    packet = make_payload_review_approval_ledger_packet(st, paths, texts, intent)
    assert packet.payload_review_hash_available is True
    assert packet.approval_intent_recorded is True
    assert not packet.blockers
    assert packet.combined_payload_hash != ""
    assert "A:/staging/sample-title-grounding-analysis_substack_preview.md".lower() in packet.preview_file_hashes
    
    _assert_no_public_state(packet)
    
    # Assert output does not leak raw markdown body or notes
    dumped = json.dumps(asdict(packet))
    assert "Safe body content." not in dumped
    assert "Safe notes." not in dumped


def test_wrong_staging_task_label_fails_closed():
    st = _staging()
    st["task_label"] = "wrong"
    packet = make_payload_review_approval_ledger_packet(st, _paths(), _preview_texts(), _intent())
    assert packet.payload_review_hash_available is False
    assert "staging_task_label_invalid" in packet.blockers


def test_staging_packet_eligibility_failures():
    for fld, val in [
        ("variant_preview_staging_available", False),
        ("variant_previews_generated", False),
        ("preview_only", False),
        ("blockers", ["some_blocker"]),
        ("publication_ready", True),
        ("public_url", "https://example.invalid"),
    ]:
        st = _staging()
        st[fld] = val
        packet = make_payload_review_approval_ledger_packet(st, _paths(), _preview_texts(), _intent())
        assert packet.payload_review_hash_available is False
        assert packet.blockers


def test_preview_file_path_mismatch_fails_closed():
    paths = [
        Path("A:/staging/sample-title-grounding-analysis_substack_preview.md"),
        Path("A:/staging/wrong_discord_preview.md"),
    ]
    packet = make_payload_review_approval_ledger_packet(_staging(), paths, _preview_texts(), _intent())
    assert packet.payload_review_hash_available is False
    assert "preview_file_paths_mismatch" in packet.blockers


def test_missing_or_extra_preview_file_fails_closed():
    paths = [Path("A:/staging/sample-title-grounding-analysis_substack_preview.md")]
    packet = make_payload_review_approval_ledger_packet(_staging(), paths, _preview_texts(), _intent())
    assert packet.payload_review_hash_available is False
    assert "preview_file_paths_count_invalid" in packet.blockers


def test_preview_file_missing_local_only_warning_fails_closed():
    texts = _preview_texts()
    texts["A:/staging/sample-title-grounding-analysis_substack_preview.md"] = "# Title\nSafe content but no warning."
    packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), texts, _intent())
    assert packet.payload_review_hash_available is False
    assert "preview_substack_warning_missing" in packet.blockers


def test_secret_marker_in_inputs_fails_closed_and_hashes_cleared():
    # Secret in staging
    st = _staging()
    st["canonical_title"] = "my api_key is secret-val"
    packet = make_payload_review_approval_ledger_packet(st, _paths(), _preview_texts(), _intent())
    assert packet.payload_review_hash_available is False
    assert "staging_secret_marker_detected" in packet.blockers
    assert packet.variant_preview_staging_sha256 == ""
    assert packet.combined_payload_hash == ""
    assert packet.canonical_title == "[REDACTED_SECRET_MARKER_DETECTED]"

    # Secret in preview file
    texts = _preview_texts()
    texts["A:/staging/sample-title-grounding-analysis_substack_preview.md"] = "# Title\nLOCAL PREVIEW ONLY - NOT APPROVED FOR PUBLICATION\nbearer secret-token-val"
    packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), texts, _intent())
    assert packet.payload_review_hash_available is False
    assert "preview_substack_secret_marker_detected" in packet.blockers
    assert packet.combined_payload_hash == ""

    # Secret in intent
    intent = _intent()
    intent["notes"] = "password is xyz"
    packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), _preview_texts(), intent)
    assert packet.payload_review_hash_available is False
    assert "intent_secret_marker_detected" in packet.blockers
    assert packet.combined_payload_hash == ""


def test_fake_claims_in_preview_fail_closed():
    for claim in ["fake_url", "fake_metrics", "fake_comments", "fake_readiness", "fake_citation"]:
        texts = _preview_texts()
        texts["A:/staging/sample-title-grounding-analysis_substack_preview.md"] = f"# Title\nLOCAL PREVIEW ONLY - NOT APPROVED FOR PUBLICATION\n{claim} is supported."
        packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), texts, _intent())
        assert packet.payload_review_hash_available is False
        assert any(claim in blocker for blocker in packet.blockers)


def test_financial_advice_in_preview_fails_closed():
    for pattern in ["buy target", "exit strategy", "trading advice", "signal service", "guaranteed prediction"]:
        texts = _preview_texts()
        texts["A:/staging/sample-title-grounding-analysis_substack_preview.md"] = f"# Title\nLOCAL PREVIEW ONLY - NOT APPROVED FOR PUBLICATION\nsome {pattern} advice."
        packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), texts, _intent())
        assert packet.payload_review_hash_available is False
        assert "preview_substack_financial_advice_or_signal_framing_detected" in packet.blockers


def test_invalid_approval_phrase_or_scope_fails_closed():
    intent = _intent()
    intent["approval_phrase"] = "WRONG_PHRASE"
    packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), _preview_texts(), intent)
    assert packet.payload_review_hash_available is False
    assert "intent_approval_phrase_invalid" in packet.blockers

    intent = _intent()
    intent["approval_scope"] = "WRONG_SCOPE"
    packet = make_payload_review_approval_ledger_packet(_staging(), _paths(), _preview_texts(), intent)
    assert packet.payload_review_hash_available is False
    assert "intent_approval_scope_invalid" in packet.blockers


def test_malformed_json_fails_closed(tmp_path):
    bad = tmp_path / "bad.json"
    bad.write_text("{bad", encoding="utf-8")
    try:
        load_json_packet(bad, "malformed_variant_preview_staging_json")
    except ValueError as exc:
        assert str(exc) == "malformed_variant_preview_staging_json"
    else:
        raise AssertionError("expected malformed_variant_preview_staging_json")


def test_non_object_json_fails_closed_without_exception_and_cli_returns_1(tmp_path):
    packet = make_payload_review_approval_ledger_packet([], [], {}, {})
    assert "malformed_variant_preview_staging_json" in packet.blockers
    _assert_no_public_state(packet)
    
    st_path = tmp_path / "staging.json"
    st_path.write_text("[]", encoding="utf-8")
    intent_path = tmp_path / "intent.json"
    intent_path.write_text("{}", encoding="utf-8")
    
    output_dir = tmp_path / "out"
    exit_code = main([str(st_path), str(intent_path), "--preview-files", "A:/a.md", "A:/b.md", "--output-dir", str(output_dir)])
    assert exit_code == 1
    
    packets = list(output_dir.glob("payload_review_ledger_*.json"))
    assert len(packets) == 1
    written = json.loads(packets[0].read_text(encoding="utf-8"))
    assert "malformed_variant_preview_staging_json" in written["blockers"]
    assert written["payload_review_hash_available"] is False


def test_module_contains_no_getenv_environ_network_provider_browser_imports():
    source = Path("live_contentops/local_payload_review_hash_approval_ledger_v6.py").read_text(encoding="utf-8")
    forbidden = ["getenv", "environ", "requests", "urllib", "httpx", "provider_gateway", "browser", "webbrowser"]
    assert not any(marker in source for marker in forbidden)


def test_cli_writes_deterministic_packet(tmp_path):
    st_path = tmp_path / "staging.json"
    st_path.write_text(json.dumps(_staging(), sort_keys=True), encoding="utf-8")
    intent_path = tmp_path / "intent.json"
    intent_path.write_text(json.dumps(_intent(), sort_keys=True), encoding="utf-8")
    
    substack_path = tmp_path / "sample-title-grounding-analysis_substack_preview.md"
    substack_path.write_text(_preview_texts()["A:/staging/sample-title-grounding-analysis_substack_preview.md"], encoding="utf-8")
    
    discord_path = tmp_path / "sample-title-grounding-analysis_discord_preview.md"
    discord_path.write_text(_preview_texts()["A:/staging/sample-title-grounding-analysis_discord_preview.md"], encoding="utf-8")
    
    # Overwrite preview paths in staging and intent for local test execution
    st_data = _staging()
    st_data["preview_files"] = [str(substack_path), str(discord_path)]
    st_path.write_text(json.dumps(st_data, sort_keys=True), encoding="utf-8")
    
    intent_data = _intent()
    intent_data["reviewed_preview_files"] = [str(substack_path), str(discord_path)]
    intent_path.write_text(json.dumps(intent_data, sort_keys=True), encoding="utf-8")
    
    output_dir = tmp_path / "out"
    assert main([
        str(st_path),
        str(intent_path),
        "--preview-files",
        str(substack_path),
        str(discord_path),
        "--output-dir",
        str(output_dir)
    ]) == 0
    
    first = list(output_dir.glob("payload_review_ledger_*.json"))[0]
    first_packet = json.loads(first.read_text(encoding="utf-8"))
    
    assert main([
        str(st_path),
        str(intent_path),
        "--preview-files",
        str(substack_path),
        str(discord_path),
        "--output-dir",
        str(output_dir)
    ]) == 0
    
    second = list(output_dir.glob("payload_review_ledger_*.json"))[0]
    second_packet = json.loads(second.read_text(encoding="utf-8"))
    
    assert first_packet["payload_review_ledger_id"] == second_packet["payload_review_ledger_id"]
    assert first_packet["payload_review_hash_available"] is True


def test_new_files_and_sample_json_are_utf8_without_bom():
    docs_dir = Path("docs/automation/V6_LOCAL_PAYLOAD_REVIEW_HASH_AND_APPROVAL_LEDGER_STRENGTHENING")
    paths = [
        Path("live_contentops/local_payload_review_hash_approval_ledger_v6.py"),
        Path("tests/test_local_payload_review_hash_approval_ledger_v6.py"),
    ]
    paths.extend(path for path in docs_dir.glob("*") if path.is_file())
    for path in paths:
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf")
    loaded = json.loads((docs_dir / "sample_payload_review_approval_ledger_packet.json").read_text(encoding="utf-8"))
    assert loaded["sample_packet_non_runtime"] is True
    assert loaded["runtime_truth"] is False
