...
MODULE_PATH = REPO_ROOT / "live_contentops" / "telegram_ledger10_remote_operator_loop_state.py"
eleventh_PROOF_PATH = REPO_ROOT / "docs/automation/0174WP_WQ_WR/telegram_ledger9_eleventh_send_proof_packet.json"
MANUAL_GATE_PATH = REPO_ROOT / "docs/automation/0174WP_WQ_WR/telegram_ledger9_next_manual_gate_packet.json"
PREVIOUS_LOOP_PATH = REPO_ROOT / "docs/automation/0174WM_WN_WO/telegram_ledger9_remote_operator_loop_state_packet.json"
...
@pytest.fixture(scope="module")
def proof():
    return json.loads(eleventh_PROOF_PATH.read_text(encoding="utf-8"))
...
def state(ledger9, proof, manual_gate, previous_loop):
    return ledger9.build_ledger10_remote_operator_loop_state(
        proof, manual_gate, previous_loop_state_packet=previous_loop)


def test_import_has_no_side_effects():
    result = subprocess.run(
        [sys.executable, "-c", "import importlib.util; s=importlib.util.spec_from_file_location('m', r'live_contentops/telegram_ledger10_remote_operator_loop_state.py'); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); print('imported')"],
        cwd=str(REPO_ROOT), capture_output=True, text=True)
    assert result.returncode == 0
    assert result.stdout.strip() == "imported"


def test_accept_committed_eleventh_proof(ledger9, proof, previous_loop):
    rec = ledger9.reconcile_eleventh_send_proof(proof, previous_loop)
    assert rec["reconciliation_outcome_class"] == ledger9.RECONCILE_OK


def test_block_missing_proof(ledger9):
    rec = ledger9.reconcile_eleventh_send_proof(None)
...
def test_block_send_not_succeeded(ledger9, proof):
    ...
    rec = ledger9.reconcile_eleventh_send_proof(p)
...
def test_state_has_ledger_count_8_and_sequence_9(state):
    assert state["current_ledger_count"] == 10
    assert state["last_successful_send_sequence"] == 11
...
def test_build_artifact_from_repo(ledger9):
    ...
    assert packet["current_ledger_count"] == 10
    assert packet["last_successful_send_sequence"] == 11
