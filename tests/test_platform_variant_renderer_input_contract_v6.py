"""Test V6 Platform Variant Renderer Input Contract."""
from __future__ import annotations

from live_contentops import platform_variant_renderer_input_contract_v6 as contract_builder


def test_make_platform_variant_renderer_input_contract():
    contract = contract_builder.make_platform_variant_renderer_input_contract()

    assert contract["contract_status"] == "FUTURE_PLATFORM_VARIANT_RENDERER_INPUT_CONTRACT_ONLY"
    assert contract["runtime_truth"] is False
    assert len(contract["required_inputs"]) == 9
