"""Test V6 Canonical Article Studio Review Queue Coordinator."""
...
def test_coordinator_execution(tmp_path):
    out_dir = tmp_path / "V6_CANONICAL_ARTICLE_STUDIO_REVIEW_QUEUE"
    coordinator.main(["--output-dir", str(out_dir)])

    expected = [
        "canonical_article_studio_queue_packet.json",
        "canonical_article_studio_review_item.json",
        "canonical_article_studio_editor_checklist.json",
        "canonical_article_studio_review_queue_validation_report.json",
        "canonical_article_studio_local_mock.html",
        "canonical_article_studio_screenshot_manifest.json",
        "canonical_article_studio_blocker_report.md",
        "canonical_article_studio_runbook.md",
    ...
    html_mock = (out_dir / "canonical_article_studio_local_mock.html").read_text(encoding="utf-8")
    assert "<script" not in html_mock.lower()
    assert "visual-pass" not in html_mock.lower()
    
    # Check for screenshot manifest settings
    manifest_data = json.loads((out_dir / "canonical_article_studio_screenshot_manifest.json").read_text(encoding="utf-8"))
