"""V6 Outbox Entry Validator.

Ensures empty outbox entry contract states are secure, redacted, and blocked.
"""
...
def validate_outbox_entry_contract(
    outbox_entry_packet: dict[str, Any],
    outbox_entry_input_contract: dict[str, Any],
    outbox_entry_blocked_template: dict[str, Any],
    outbox_entry_blocked_output: dict[str, Any],
    outbox_entry_gate_matrix: list[dict[str, Any]],
    outbox_entry_checklist: dict[str, Any]
) -> tuple[dict[str, Any], list[str]]:
    """Validates the offline dry-run outbox entry contract state."""
    blockers = []
    failed = False
    ...
    for flag in packet_flags_to_fail:
        if outbox_entry_packet.get(flag) is True:
            blockers.append("outbox_entry_creation_blocked")
            failed = True
    ...
    for field in template_fields:
        val = outbox_entry_blocked_template.get(field)
        if val is not None and val != "":
            blockers.append("non_null_outbox_template_fields_detected")
            failed = True

    template_flags = [
        "approval_valid_for_dispatch", "outbox_entry_created", "outbox_entry_creation_allowed",
        "dispatch_attempt_created", "dispatch_allowed_now", "public_postable", "live_write_allowed_now"
    ]
    for flag in template_flags:
        if outbox_entry_blocked_template.get(flag) is True:
            blockers.append("forbidden_active_dispatch_flags")
            failed = True

    # 4. Check output fields and lists
    output_flags_to_fail = [
        "outbox_entry_created", "outbox_entry_id_created", "outbox_payload_hash_created",
        "dispatch_attempt_created", "dispatch_allowed_now", "live_write_allowed_now",
        "approval_valid_for_dispatch", "destination_binding_completed", "account_binding_completed"
    ]
    for flag in output_flags_to_fail:
        if outbox_entry_blocked_output.get(flag) is True:
            blockers.append("outbox_entry_creation_blocked")
            failed = True
    ...
    for field in output_lists:
        if len(outbox_entry_blocked_output.get(field, [])) > 0:
    ...
    for key in count_keys:
        if outbox_entry_blocked_output.get(key, 0) > 0:
    ...
    for row in outbox_entry_gate_matrix:
        ...
        if row.get("outbox_gate_status") != "blocked_missing_approved_payload_and_destination":
            blockers.append("readiness_matrix_active_lane_detected")
            failed = True
    ...
    if outbox_entry_input_contract.get("contract_status") != "FUTURE_OUTBOX_ENTRY_INPUT_CONTRACT_ONLY":
        blockers.append("platform_input_contract_incomplete")
        failed = True

    required_inputs = outbox_entry_input_contract.get("required_inputs")
    expected_names = {
        "approved_exact_payload_review_ref",
        "approval_queue_review_output_ref",
        "rendered_platform_payloads_ref",
    ...
    if not isinstance(required_inputs, list) or len(required_inputs) == 0:
        ...
        for inp in required_inputs:
            ...
            if inp.get("blocks_outbox_entry_creation") is not True:
    ...
    check_value(outbox_entry_packet)
    check_value(outbox_entry_input_contract)
    check_value(outbox_entry_blocked_template)
    check_value(outbox_entry_blocked_output)
    check_value(outbox_entry_gate_matrix)
    check_value(outbox_entry_checklist)
