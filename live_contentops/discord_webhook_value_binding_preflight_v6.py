"""V6 Discord Webhook Value Binding Preflight Gate."""
...
ENDPOINT_MAPPING_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_ENDPOINT_MAPPING_PREFLIGHT_FROM_CAPABILITY_LANE_SPLIT_V0"
...
class DiscordWebhookValueBindingPreflightPacket:
    schema_version: str
    task_label: str
    discord_webhook_value_binding_preflight_id: str
    discord_webhook_value_binding_declaration_id: str
    operator_id: str
    ...
    webhook_value_source: str
    webhook_value_read_allowed: bool
    webhook_value_persist_allowed: bool
    webhook_value_hash_allowed: bool
    webhook_value_length_allowed: bool
    webhook_value_prefix_suffix_allowed: bool
    webhook_url_shape_validation_allowed: bool
    discord_api_validation_allowed: bool
    webhook_send_test_allowed: bool
    webhook_value_observed: bool
    webhook_value_persisted: bool
    webhook_value_hash_observed: bool
    webhook_value_length_observed: bool
    webhook_value_prefix_suffix_observed: bool
    webhook_url_shape_validated: bool
    discord_api_validated: bool
    webhook_send_test_performed: bool
    ...
    discord_webhook_value_binding_preflight_available: bool
    discord_webhook_value_binding_preflight_declared_ready: bool
    eligible_for_discord_permission_probe_gate: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != ENDPOINT_MAPPING_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("discord_endpoint_mapping_preflight_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("discord_endpoint_mapping_preflight_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_discord_webhook_value_binding_gate") is not True:
        blockers.append("preflight_eligible_for_discord_webhook_value_binding_gate_invalid")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "credential_key_name",
        "credential_kind",
        "webhook_value_source",
        "webhook_value_read_allowed",
        "webhook_value_persist_allowed",
        "webhook_value_hash_allowed",
        "webhook_value_length_allowed",
        "webhook_value_prefix_suffix_allowed",
        "webhook_url_shape_validation_allowed",
        "discord_api_validation_allowed",
    ...
    if decl.get("binding_mode") != "operator_declared_discord_webhook_value_binding_presence_only_not_value":
        blockers.append("declaration_binding_mode_invalid")

    if decl.get("platform") != "discord":
        blockers.append("declaration_platform_invalid")

    if decl.get("credential_key_name") != "DISCORD_LIVE_ANNOUNCEMENTS_WEBHOOK":
        blockers.append("declaration_credential_key_name_invalid")

    if decl.get("credential_kind") != "discord_webhook_url_secret_value_later":
        blockers.append("declaration_credential_kind_invalid")

    if decl.get("webhook_value_source") != "process_env_exact_key_membership_only":
        blockers.append("declaration_webhook_value_source_invalid")

    # Confirms later required
    for b_field in [
        "channel_identity_later_required",
        "permission_verification_later_required",
        "payload_hash_revalidation_later_required",
    ...
    if decision not in ["mark_discord_webhook_value_binding_preflight_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_webhook_value_binding_preflight_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_WEBHOOK_VALUE_BINDING_PREFLIGHT_READY_ONLY_NOT_SEND":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_webhook_value_binding_preflight_only":
...
def make_discord_webhook_value_binding_preflight_packet(
    preflight_packet: Any,
    operator_declaration: Any,
    check_env: bool = False,
    env_mapping: Mapping[str, str] | None = None
) -> DiscordWebhookValueBindingPreflightPacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_endpoint_mapping_preflight_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_discord_webhook_value_binding_declaration_json")
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_webhook_value_binding_declaration_id = str(operator_declaration.get("discord_webhook_value_binding_declaration_id") or "")
        ...
        credential_key_name = str(operator_declaration.get("credential_key_name") or "")
        credential_kind = str(operator_declaration.get("credential_kind") or "")

        webhook_value_source = str(operator_declaration.get("webhook_value_source") or "")
        webhook_value_read_allowed = bool(operator_declaration.get("webhook_value_read_allowed"))
        webhook_value_persist_allowed = bool(operator_declaration.get("webhook_value_persist_allowed"))
        webhook_value_hash_allowed = bool(operator_declaration.get("webhook_value_hash_allowed"))
        webhook_value_length_allowed = bool(operator_declaration.get("webhook_value_length_allowed"))
        webhook_value_prefix_suffix_allowed = bool(operator_declaration.get("webhook_value_prefix_suffix_allowed"))
        webhook_url_shape_validation_allowed = bool(operator_declaration.get("webhook_url_shape_validation_allowed"))
        discord_api_validation_allowed = bool(operator_declaration.get("discord_api_validation_allowed"))
        webhook_send_test_allowed = bool(operator_declaration.get("webhook_send_test_allowed"))
        ...
        # Exact-key membership check
        if check_env:
            credential_presence_checked = True
            credential_presence_mode = "process_env_exact_key_membership_only"
            if env_mapping is not None:
                credential_present = credential_key_name in env_mapping
            else:
                credential_present = credential_key_name in getattr(os, "environ")

            if not credential_present:
                blockers.append("discord_credential_presence_missing")
        else:
            credential_presence_checked = False
            credential_presence_mode = "not_checked"
            credential_present = False
    ...
    severe_blockers = [b for b in blockers if b != "discord_credential_presence_missing"]
    prepared = not severe_blockers

    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_webhook_value_binding_preflight_ready") and not blockers
    )

    eligible_permission_probe = False
    if prepared and declared_ready and not blockers:
        if check_env:
            if credential_present:
                eligible_permission_probe = True
        else:
            # Explicitly deferred eligibility is true
            eligible_permission_probe = True
    ...
    if has_secrets:
        discord_webhook_value_binding_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_webhook_value_binding_declaration_id": discord_webhook_value_binding_declaration_id,
        "blockers": blockers,
    }
    discord_webhook_value_binding_preflight_id = f"discord_webhook_value_binding_preflight_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_webhook_value_binding_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")
        if not check_env:
            warnings.append("discord_credential_presence_deferred")

    return DiscordWebhookValueBindingPreflightPacket(
        ...
        credential_presence_checked=credential_presence_checked,
        credential_present=credential_present,
        credential_presence_mode=credential_presence_mode,
        webhook_value_source=webhook_value_source,
        webhook_value_read_allowed=webhook_value_read_allowed,
        webhook_value_persist_allowed=webhook_value_persist_allowed,
        webhook_value_hash_allowed=webhook_value_hash_allowed,
        webhook_value_length_allowed=webhook_value_length_allowed,
        webhook_value_prefix_suffix_allowed=webhook_value_prefix_suffix_allowed,
        webhook_url_shape_validation_allowed=webhook_url_shape_validation_allowed,
        discord_api_validation_allowed=discord_api_validation_allowed,
        webhook_send_test_allowed=webhook_send_test_allowed,
        webhook_value_observed=webhook_value_observed,
        webhook_value_persisted=webhook_value_persisted,
        webhook_value_hash_observed=webhook_value_hash_observed,
        webhook_value_length_observed=webhook_value_length_observed,
        webhook_value_prefix_suffix_observed=webhook_value_prefix_suffix_observed,
        webhook_url_shape_validated=webhook_url_shape_validated,
        discord_api_validated=discord_api_validated,
        webhook_send_test_performed=webhook_send_test_performed,
        ...
        all_platforms_endpoint_mapping_ready=False,
        discord_webhook_value_binding_preflight_available=prepared,
        discord_webhook_value_binding_preflight_declared_ready=declared_ready,
        eligible_for_discord_permission_probe_gate=eligible_permission_probe,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
        blockers=blockers,
        warnings=warnings,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Webhook Value Binding Preflight CLI")
    parser.add_argument("preflight_packet", help="Path to Discord endpoint mapping preflight packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator Discord webhook value binding declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write value binding preflight packet JSON")
    parser.add_argument("--check-process-env-membership", action="store_true", help="Perform process environment exact key membership check")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_endpoint_mapping_preflight_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_discord_webhook_value_binding_declaration_json")

        packet = make_discord_webhook_value_binding_preflight_packet(
            preflight,
            decl,
            check_env=args.check_process_env_membership
        )
        ...
        if not packet.discord_webhook_value_binding_preflight_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordWebhookValueBindingPreflightPacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_webhook_value_binding_preflight_id=f"discord_webhook_value_binding_preflight_blocked_{str(val_err)}",
            discord_webhook_value_binding_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_endpoint_mapping_preflight_id="",
            ...
            substack_manual_fallback_required=True,
            all_platforms_endpoint_mapping_ready=False,
            discord_webhook_value_binding_preflight_available=False,
            discord_webhook_value_binding_preflight_declared_ready=False,
            eligible_for_discord_permission_probe_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            blockers=[str(val_err)],
            warnings=["discord_webhook_value_binding_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
