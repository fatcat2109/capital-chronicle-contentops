...
def run_cdp_preflight(*, draft_url: str | None, use_current_draft: bool, cdp_port: int) -> dict[str, Any]:
    try:
        ...
        return {"result_status": "BLOCKED", "blocker": "missing_cdp", "diagnostic": "playwright_not_installed", "attempted": 0}
    ...
    try:
        with sync_playwright() as p:
            try:
                ...
                return {"result_status": "BLOCKED", "blocker": "missing_cdp", "diagnostic": f"cdp_connection_failed: {str(exc)}", "attempted": 0}

            if not browser.contexts:
                return {"result_status": "BLOCKED", "blocker": "missing_cdp", "diagnostic": "no_active_contexts", "attempted": 1}

            context = browser.contexts[0]
            page = None
            if use_current_draft and getattr(context, "pages", None):
            ...
            if page is None:
                ...
                if not draft_url:
                    return {"result_status": "BLOCKED", "blocker": "missing_draft_url", "diagnostic": "draft_url_required_without_current_page", "attempted": 0}
                try:
                    page.goto(draft_url, timeout=15000)
                    page.wait_for_load_state("load", timeout=15000)
                except Exception as exc:
                    page.close()
                    return {"result_status": "BLOCKED", "blocker": "ui_uncertainty", "diagnostic": f"navigation_failed: {str(exc)}", "attempted": 1}

            current_url = page.url
            if "sign-in" in current_url.lower() or "login" in current_url.lower():
                if not use_current_draft:
                    page.close()
                return {"result_status": "BLOCKED", "blocker": "login_or_account_mismatch", "diagnostic": f"redirected_to_login: {current_url}", "attempted": 1}
            ...
            if publish_controls <= 0:
                return {"result_status": "BLOCKED", "blocker": "ui_uncertainty", "diagnostic": "publish_controls_not_detected", "attempted": 1}
            return {
                "result_status": "PASS",
                "blocker": None,
        ...
        return {"result_status": "FAIL", "blocker": "execution_failed", "diagnostic": f"unhandled_error: {str(exc)}", "attempted": 1}
...
def build_evidence(*, draft_url: str | None, use_current_draft: bool, execute: bool, task_id: str, secrets: list[str] | None = None) -> dict[str, Any]:
    if not execute:
        res = {"result_status": "DRY_RUN", "blocker": None, "diagnostic": "dry_run_no_browser_action", "attempted": 0}
    else:
