"""V6 Canonical Draft Eligibility Validator.

Ensures no raw secrets, real approvals, or active publishing flows are claimed.
"""
...
def validate_canonical_draft_eligibility(
    eligibility_packet: dict[str, Any],
    summary: dict[str, Any],
    matrix: list[dict[str, Any]],
    preview_md: str
) -> tuple[dict[str, Any], list[str]]:
    """Validates the canonical draft eligibility status offline."""
    blockers = []
    failed = False

    # 1. Required default blockers
    blockers.extend([
        "runtime_operator_approval_missing",
        "real_source_pack_not_approved",
        "runtime_draft_generation_blocked",
    ...
    # 2. Block active runtime approval claims
    if eligibility_packet.get("real_source_pack_approved") is True:
        blockers.append("real_source_pack_approved_claimed")
        failed = True
    if summary.get("real_operator_approval_created") is True:
        blockers.append("real_operator_approval_created")
        failed = True
    if summary.get("real_jim_approval_created") is True:
        blockers.append("real_jim_approval_created")
        failed = True

    # 3. Block active gate and generation flags
    if eligibility_packet.get("approval_gate_passed_for_runtime") is True:
        blockers.append("approval_gate_passed_for_runtime")
        failed = True
    if eligibility_packet.get("canonical_draft_generation_allowed_for_runtime") is True:
        blockers.append("canonical_draft_generation_allowed_for_runtime")
        failed = True
    if eligibility_packet.get("article_copy_generated") is True:
        blockers.append("article_copy_generated")
        failed = True
    if eligibility_packet.get("draft_markdown_created") is True:
        blockers.append("draft_markdown_created")
        failed = True

    # 4. Block active dispatch flags
    forbidden_flags = [
        "public_postable",
    ...
    for flag in forbidden_flags:
        if eligibility_packet.get(flag) is True or summary.get(flag) is True:
            blockers.append("forbidden_active_dispatch_flags")
            failed = True

    # Check matrix rows
    for row in matrix:
        if row.get("runtime_approval_present") is True:
            blockers.append("real_source_pack_approved_claimed")
            failed = True
        if row.get("eligible_for_runtime_draft") is True:
            blockers.append("canonical_draft_generation_allowed_for_runtime")
            failed = True
        if row.get("allowed_for_publication") is True:
            blockers.append("forbidden_active_dispatch_flags")
            failed = True

    # 5. Scan texts recursively for leaks
    texts_to_scan: list[str] = [preview_md]
