"""Cockpit read model policy (LOCAL, READ-MODEL ONLY, NO DISPATCH)."""
...
POLICY_PACKET = "cockpit_read_model_policy_packet.json"
POLICY_DOC = "cockpit_read_model_policy.md"
...
FORBIDDEN_MATERIAL_PATTERNS = [
    ...
    re.compile(r"provider_response", re.I),
]
...
def build_policy_packet():
    ...
    packet["cockpit_read_model_policy_checksum"] = adapter.compute_checksum(packet)
    return packet


def render_doc(title, packet):
    lines = [f"# {title}", "", "> [!IMPORTANT]", "> Local cockpit read model only. Live dispatch remains blocked; no platform/provider/env/credential behavior occurs.", ""]
...
def write_artifacts(repo_root=".", output_dir=None):
    ...
    (out / POLICY_DOC).write_text(render_doc("Cockpit Read Model Policy", packet), encoding="utf-8", newline="\n")
    return copy.deepcopy(packet)


if __name__ == "__main__":
    result = write_artifacts(".")
    print("COCKPIT_READ_MODEL_POLICY_CHECKSUM", result["cockpit_read_model_policy_checksum"])
