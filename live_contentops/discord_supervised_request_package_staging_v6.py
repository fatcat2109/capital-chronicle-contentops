"""V6 Discord Supervised Request Package Staging Gate."""
...
REQUEST_POLICY_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_REQUEST_POLICY_GATE_FROM_OPERATOR_PAYLOAD_REVIEW_V0"
...
class DiscordSupervisedRequestPackageStagingPacket:
    schema_version: str
    task_label: str
    discord_supervised_request_package_staging_id: str
    discord_supervised_request_package_staging_declaration_id: str
    operator_id: str
    ...
    request_package_kind: str
    request_package_non_executable: bool
    ...
    idempotency_later_confirmed: bool
    payload_hash_revalidation_later_confirmed: bool
    permission_verification_later_confirmed: bool
    operator_staging_decision: str
    ...
    discord_supervised_request_package_staging_available: bool
    discord_supervised_request_package_staging_declared_ready: bool
    eligible_for_discord_final_manual_execution_review_gate: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != REQUEST_POLICY_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("discord_request_policy_gate_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("discord_request_policy_gate_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_discord_supervised_request_package_staging_gate") is not True:
        blockers.append("preflight_eligible_for_discord_supervised_request_package_staging_gate_invalid")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "audit_redaction_confirmed",
        "manual_fallback_confirmed",
        "idempotency_later_confirmed",
        "payload_hash_revalidation_later_confirmed",
        "permission_verification_later_confirmed",
        "live_dispatch_approval_granted",
        "publication_approval_granted",
        "discord_api_call_allowed",
        "webhook_send_test_allowed",
        "operator_staging_decision",
    ...
    if decl.get("staging_mode") != "operator_declared_discord_supervised_request_package_staging_only_not_request":
        blockers.append("declaration_staging_mode_invalid")
    ...
    if decl.get("request_package_kind") != "non_executable_discord_supervised_request_package_shell":
        blockers.append("declaration_request_package_kind_invalid")

    if decl.get("request_package_non_executable") is not True:
        blockers.append("declaration_request_package_non_executable_not_true")
    ...
    if decl.get("max_request_count") != preflight.get("max_request_count"):
        blockers.append("declaration_max_request_count_mismatch")

    if decl.get("max_request_count") != 1:
        blockers.append("declaration_max_request_count_invalid")

    if decl.get("timeout_seconds") != preflight.get("timeout_seconds"):
        blockers.append("declaration_timeout_seconds_mismatch")
    ...
    staging_decision = decl.get("operator_staging_decision")
    if staging_decision not in ["approve_non_executable_request_package_shell_for_next_local_gate_only", "reject", "defer"]:
        blockers.append("declaration_operator_staging_decision_invalid")
    elif staging_decision in ["reject", "defer"]:
        blockers.append(f"declaration_staging_rejected_or_deferred_{staging_decision}")

    decision = decl.get("declaration_decision")
    if decision not in ["mark_discord_supervised_request_package_staging_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_supervised_request_package_staging_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_SUPERVISED_REQUEST_PACKAGE_STAGING_READY_ONLY_NOT_REQUEST":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_supervised_request_package_staging_only":
...
def make_discord_supervised_request_package_staging_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordSupervisedRequestPackageStagingPacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_request_policy_gate_packet_json")
    ...
    reviewed_payload_hash = ""
    request_package_kind = ""
    request_package_non_executable = False
    ...
    idempotency_later_confirmed = False
    payload_hash_revalidation_later_confirmed = False
    permission_verification_later_confirmed = False
    operator_staging_decision = ""
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_supervised_request_package_staging_declaration_id = str(operator_declaration.get("discord_supervised_request_package_staging_declaration_id") or "")
        ...
        request_package_kind = str(operator_declaration.get("request_package_kind") or "")
        request_package_non_executable = bool(operator_declaration.get("request_package_non_executable"))
        executable_request_artifact_created = bool(operator_declaration.get("executable_request_artifact_created"))
        webhook_url_included = bool(operator_declaration.get("webhook_url_included"))
        webhook_token_included = bool(operator_declaration.get("webhook_token_included"))
        endpoint_url_included = bool(operator_declaration.get("endpoint_url_included"))
        channel_identity_included = bool(operator_declaration.get("channel_identity_included"))
        http_headers_included = bool(operator_declaration.get("http_headers_included"))
        http_body_included = bool(operator_declaration.get("http_body_included"))
        curl_command_included = bool(operator_declaration.get("curl_command_included"))
        browser_instruction_included = bool(operator_declaration.get("browser_instruction_included"))
        public_url_included = bool(operator_declaration.get("public_url_included"))
        metrics_included = bool(operator_declaration.get("metrics_included"))
        ...
        kill_switch_confirmed = bool(operator_declaration.get("kill_switch_confirmed"))
        audit_redaction_confirmed = bool(operator_declaration.get("audit_redaction_confirmed"))
        manual_fallback_confirmed = bool(operator_declaration.get("manual_fallback_confirmed"))
        idempotency_later_confirmed = bool(operator_declaration.get("idempotency_later_confirmed"))
        payload_hash_revalidation_later_confirmed = bool(operator_declaration.get("payload_hash_revalidation_later_confirmed"))
        permission_verification_later_confirmed = bool(operator_declaration.get("permission_verification_later_confirmed"))
        operator_staging_decision = str(operator_declaration.get("operator_staging_decision") or "")
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_supervised_request_package_staging_ready")
    )
    ...
    if (
        prepared
        and declared_ready
        and request_package_non_executable
        and not executable_request_artifact_created
        and not webhook_url_included
        and not webhook_token_included
        and not endpoint_url_included
        and not channel_identity_included
        and not http_headers_included
        and not http_body_included
        and not curl_command_included
        and not browser_instruction_included
        and not public_url_included
        and not metrics_included
        and max_request_count == 1
        and max_retries == 0
        and not hidden_retry_allowed
        and kill_switch_confirmed
        and audit_redaction_confirmed
        and manual_fallback_confirmed
    ):
    ...
    if has_secrets:
        discord_supervised_request_package_staging_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_supervised_request_package_staging_declaration_id": discord_supervised_request_package_staging_declaration_id,
        "blockers": blockers,
    }
    discord_supervised_request_package_staging_id = f"discord_supervised_request_package_staging_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_supervised_request_package_staging_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordSupervisedRequestPackageStagingPacket(
        ...
        request_package_kind=request_package_kind,
        request_package_non_executable=request_package_non_executable,
        executable_request_artifact_created=executable_request_artifact_created,
        webhook_url_included=webhook_url_included,
        webhook_token_included=webhook_token_included,
        endpoint_url_included=endpoint_url_included,
        channel_identity_included=channel_identity_included,
        ...
        idempotency_later_confirmed=idempotency_later_confirmed,
        payload_hash_revalidation_later_confirmed=payload_hash_revalidation_later_confirmed,
        permission_verification_later_confirmed=permission_verification_later_confirmed,
        operator_staging_decision=operator_staging_decision,
        ...
        discord_supervised_request_package_staging_available=prepared,
        discord_supervised_request_package_staging_declared_ready=declared_ready,
        eligible_for_discord_final_manual_execution_review_gate=eligible_staging,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Supervised Request Staging CLI")
    parser.add_argument("preflight_packet", help="Path to Discord request policy gate packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator staging declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write supervised request staging packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_request_policy_gate_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_request_policy_declaration_json")

        packet = make_discord_supervised_request_package_staging_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.discord_supervised_request_package_staging_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordSupervisedRequestPackageStagingPacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_supervised_request_package_staging_id=f"discord_supervised_request_package_staging_blocked_{str(val_err)}",
            discord_supervised_request_package_staging_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_request_policy_gate_id="",
            ...
            discord_api_call_allowed=False,
            webhook_send_test_allowed=False,
            discord_supervised_request_package_staging_available=False,
            discord_supervised_request_package_staging_declared_ready=False,
            eligible_for_discord_final_manual_execution_review_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            next_local_gate_later_required=False,
            substack_manual_fallback_required=True,
            all_platforms_endpoint_mapping_ready=False,
            blockers=[str(val_err)],
            warnings=["discord_supervised_request_package_staging_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
