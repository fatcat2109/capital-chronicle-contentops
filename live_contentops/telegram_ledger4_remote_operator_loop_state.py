"""Ledger-4 Telegram remote operator loop state after fifth send.

Task 0174VU/VV/VW. Promotes accepted fifth-send proof into local-only
...
TASK_LABEL = (
    "TASK_CONTENTOPS_0174VU_VV_VW_TELEGRAM_LEDGER4_REMOTE_OPERATOR_LOOP_"
    "RECONCILIATION_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER4_REMOTE_OPERATOR_LOOP_STATE_0174VU_VV_VW"
MODEL_VERSION = "0174VU_VV_VW_TELEGRAM_LEDGER4_REMOTE_OPERATOR_LOOP_STATE_V1"
SOURCE_BASELINE_COMMIT = "8126fd5b43bb741375caa0310521a4348579e623"
DOC_REL_DIR = os.path.join("docs", "automation", "0174VU_VV_VW")
PACKET_FILENAME = "telegram_ledger4_remote_operator_loop_state_packet.json"
DOC_FILENAME = "telegram_ledger4_remote_operator_loop_state.md"
FIFTH_PROOF_REL = os.path.join(
    "docs", "automation", "0174VR_VS_VT", "telegram_ledger3_fifth_send_proof_packet.json")
MANUAL_GATE_REL = os.path.join(
    "docs", "automation", "0174VO_VP_VQ", "telegram_ledger3_next_manual_gate_packet.json")
PREVIOUS_LOOP_REL = os.path.join(
    "docs", "automation", "0174VL_VM_VN", "telegram_remote_operator_loop_state_packet.json")

RECONCILE_OK = "ledger4_reconciliation_ok_ledger_advanced_to_4"
RECONCILE_MISSING = "ledger4_reconciliation_blocked_missing_fifth_send_proof"
RECONCILE_SEND_NOT_SUCCEEDED = "ledger4_reconciliation_blocked_send_not_succeeded"
RECONCILE_GATE_NOT_REVALIDATED = "ledger4_reconciliation_blocked_manual_gate_not_revalidated"
RECONCILE_LEDGER_NOT_ADVANCED = "ledger4_reconciliation_blocked_ledger_not_advanced"
RECONCILE_MANIFEST_NOT_ADVANCED = "ledger4_reconciliation_blocked_manifest_not_advanced"
RECONCILE_FAIL_CLOSED = "ledger4_reconciliation_fail_closed_forbidden_value"
TRUTH_CLASS = "remote_loop_truth_ledger_count_4_test5_accepted"
...
FIFTH_GATE_ID = "ledger3_next_manual_gate_transient_local_gate_v1"
...
def _entry_from_proof(proof):
    ...
    entry = ledger.build_ledger_entry(ev, operator_gate_id=FIFTH_GATE_ID)
    p = proof or {}
...
def reconcile_fifth_send_proof(fifth_send_proof, previous_loop_state_packet=None):
    p = fifth_send_proof or {}
    blockers = []
    ...
    elif not (p.get("ledger_appended") is True and p.get("ledger_entry_count_before") == 3
              and p.get("ledger_entry_count") == 4 and p.get("request_budget_used") == 1):
    ...
    result = {
        ...
        "fifth_send_proof_checksum": p.get("evidence_checksum") or compute_checksum(p) if p else None,
...
def build_ledger4_remote_operator_loop_state(fifth_send_proof, manual_gate_packet,
                                             previous_loop_state_packet=None):
    proof = fifth_send_proof or {}
    ...
    reconciliation = reconcile_fifth_send_proof(proof, prev_packet)
    reconciled = reconciliation.get("reconciled") is True
    entry = _entry_from_proof(proof) if proof else {}
    audit_refs = {
        "fifth_send_proof_checksum": proof.get("evidence_checksum") or compute_checksum(proof) if proof else None,
...
def _candidate_from_proof(proof, *, sequence=None, response_suffix=None):
    ...
    if response_suffix:
        ev["response_checksum"] = compute_checksum({
            "kind": "ledger4_new_candidate_response_placeholder",
...
def _new_payload_candidate_from_proof(proof):
    ...
    ev["send_text_checksum"] = compute_checksum({
        "kind": "ledger4_new_payload_example",
...
def build_artifact_packet(fifth_send_proof, manual_gate_packet, previous_loop_state_packet=None):
    proof = fifth_send_proof or {}
    state = build_ledger4_remote_operator_loop_state(
    ...
    examples = {
        ...
        "exact_replay": build_next_gate_precheck_state(
            state, exact_replay_candidate, fresh_operator_gate_id=FIFTH_GATE_ID),
        ...
        "same_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, same_payload_candidate, fresh_operator_gate_id="fresh_ledger4_next_gate"),
        "new_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, new_payload_candidate, fresh_operator_gate_id="fresh_ledger4_next_gate"),
    }
    packet = {
        "task_label": TASK_LABEL,
        "model": MODEL,
...
def build_artifact_from_repo(repo_root):
    ...
    fifth = load_packet(os.path.join(root, FIFTH_PROOF_REL))
    manual = load_packet(os.path.join(root, MANUAL_GATE_REL))
    previous = load_packet(os.path.join(root, PREVIOUS_LOOP_REL))
    return build_artifact_packet(fifth, manual, previous_loop_state_packet=previous)
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger4 remote loop artifacts: scan found %d violation(s)" % len(violations))
