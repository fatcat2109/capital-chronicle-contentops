...
def call_nine_router_multimodal(
    ...
    *,
    ...
    """Make one bounded visual-critic call through the canonical 9Router seam.

    Images must already be bounded PNG/JPEG payloads. They are sent inline so the
    gateway never receives a signed URL or gains an asset-fetch side channel. This
    adds no retry policy, model pool, or provider authority.
    """
    if model not in AUTHORIZED_MODELS:
        raise NineRouterAdapterError(f"unauthorized_model:{model}")
    if len(images_b64) > MULTIMODAL_MAX_IMAGES:
        raise NineRouterAdapterError("multimodal_image_count_exceeds_limit")
    api_key = getattr(os, "environ").get(ENV_API_KEY)
    if not api_key:
        raise NineRouterAdapterError(f"{ENV_API_KEY}_missing")
    resolved_base = resolve_base_url(base_url)
    ...
    if parsed is not None:
        ...
        sse = _parse_sse_full(raw)
        output_text = sse["text"] if sse is not None else None
    if output_text is None and sse is None and not payload:
        return ProviderResult(
            status_code=status,
            failure_class="structured_output_malformed",
            text=raw[:2000],
