...
class LinkedInManualExportError(ValueError): pass
def build_linkedin_manual_export_packet(source_packet: Mapping[str,Any]) -> dict[str,Any]:
    _assert_safe(source_packet, LinkedInManualExportError)
    article=_require_mapping(source_packet,'canonical_article_draft',LinkedInManualExportError)
    source_id=_require_str(source_packet,'packet_id',LinkedInManualExportError)
    source_hash=_require_str(article,'canonical_payload_hash',LinkedInManualExportError)
    title=_require_str(article,'title',LinkedInManualExportError)
    post_body=(f"Capital Chronicle educational briefing: {title}\n\n"
        "Process note: This manual LinkedIn post is fixture-only evidence for operator review. "
    ...
    manual_copy_payload={'platform':'linkedin','target':'linkedin_manual_copy','copy_mode':'manual copy only','post_body':post_body,'operator_instructions':'Review in canonical V5, then manually copy into LinkedIn only if separately approved outside ContentOps.','safety_labels':[SAMPLE_SCOPE,'manual copy only','LinkedIn API not used','live publish disabled','no runtime proof']}
    exact=_stable_hash(manual_copy_payload)
    packet={'schema_version':SCHEMA_VERSION,'task_label':TASK_LABEL,'export_packet_id':f'linkedin_manual_export_{exact[:16]}','platform':'linkedin','source_article_packet_id':source_id,'source_canonical_hash':source_hash,'post_body_fixture':post_body,'manual_copy_payload':manual_copy_payload,'exact_payload_hash':exact,'hash_algorithm':HASH_ALGORITHM,'export_status':'ready_for_manual_review','approval_status':'pending','sample_scope':SAMPLE_SCOPE,'manual_copy_only':True,'linkedin_api_used':False,'provider_call_made':False,'network_call_made':False,'credential_read_made':False,'env_value_read_made':False,'browser_session_used':False,'live_publish_allowed':False,'live_publish_performed':False,'blocked_controls':['approve','send','publish','dispatch','schedule'],'enabled_publish_send_dispatch_approve_controls':False,'warnings':['sample_fixture_only','manual_copy_only_no_linkedin_api','no_runtime_proof'],'recommended_next_task':'TASK_CONTENTOPS_V6_LINKEDIN_MANUAL_APPROVAL_EXPORT_EVIDENCE_V0'}
    _assert_safe(packet, LinkedInManualExportError); return packet
def main():
    p=argparse.ArgumentParser(); p.add_argument('--input',required=True,type=Path); p.add_argument('--output',required=True,type=Path); a=p.parse_args(); _write(a.output, build_linkedin_manual_export_packet(load_json(a.input)))
