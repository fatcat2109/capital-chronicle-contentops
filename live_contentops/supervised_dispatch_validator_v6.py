"""V6 Supervised Dispatch Validator.

Ensures empty supervised dispatch contract states are secure, redacted, and blocked.
"""
...
def is_placeholder(val: Any) -> bool:
    ...
    if isinstance(val, str):
        ...
        exact_placeholders = {
            ...
            "supervised_dispatch_blocked_waiting_for_valid_outbox_and_authorization",
            "future_supervised_dispatch_input_contract_only",
            "blocked_template_only_not_dispatch",
            "blocked_no_dispatch_attempt_created",
            "blocked_missing_outbox_authorization_and_kill_switch",
            "supervised_dispatch_blocked_pending_outbox_authorization_and_kill_switch",
            "supervised_dispatch_input_contract.json",
...
def validate_supervised_dispatch_contract(
    supervised_dispatch_packet: dict[str, Any],
    supervised_dispatch_input_contract: dict[str, Any],
    supervised_dispatch_blocked_template: dict[str, Any],
    supervised_dispatch_blocked_output: dict[str, Any],
    supervised_dispatch_gate_matrix: list[dict[str, Any]],
    supervised_dispatch_checklist: dict[str, Any]
) -> tuple[dict[str, Any], list[str]]:
    """Validates the offline dry-run supervised dispatch contract state."""
    blockers = []
    failed = False
    ...
    for flag in packet_flags_to_fail:
        if supervised_dispatch_packet.get(flag) is True:
    ...
    for field in template_fields:
        val = supervised_dispatch_blocked_template.get(field)
        if val is not None and val != "":
            blockers.append("non_null_dispatch_template_fields_detected")
            failed = True

    template_flags = [
        "dispatch_preflight_performed", "dispatch_attempt_created",
        "dispatch_request_sent", "live_write_attempted", "retry_attempted",
        "dispatch_allowed_now", "public_postable", "live_write_allowed_now"
    ]
    for flag in template_flags:
        if supervised_dispatch_blocked_template.get(flag) is True:
            blockers.append("forbidden_active_dispatch_flags")
            failed = True

    # 4. Check output fields and lists
    output_flags_to_fail = [
        "dispatch_preflight_performed", "dispatch_attempt_created", "dispatch_request_prepared",
        "dispatch_request_sent", "live_write_attempted", "retry_attempted",
        "dispatch_allowed_now", "live_write_allowed_now", "public_url_created", "public_postable"
    ]
    for flag in output_flags_to_fail:
        if supervised_dispatch_blocked_output.get(flag) is True:
            blockers.append("dispatch_blocked")
            failed = True
    ...
    for field in output_lists:
        if len(supervised_dispatch_blocked_output.get(field, [])) > 0:
    ...
    for key in count_keys:
        if supervised_dispatch_blocked_output.get(key, 0) > 0:
    ...
    for row in supervised_dispatch_gate_matrix:
        ...
        if row.get("dispatch_gate_status") != "blocked_missing_outbox_authorization_and_kill_switch":
            blockers.append("readiness_matrix_active_lane_detected")
            failed = True
    ...
    if supervised_dispatch_input_contract.get("contract_status") != "FUTURE_SUPERVISED_DISPATCH_INPUT_CONTRACT_ONLY":
        blockers.append("platform_input_contract_incomplete")
        failed = True

    required_inputs = supervised_dispatch_input_contract.get("required_inputs")
    expected_names = {
        "valid_outbox_entry_ref",
        "approved_exact_payload_review_ref",
        "rendered_platform_payload_ref",
    ...
    if not isinstance(required_inputs, list) or len(required_inputs) == 0:
        ...
        for inp in required_inputs:
            ...
            if inp.get("blocks_supervised_dispatch") is not True:
    ...
    def check_text(t: str, key_name: str = ""):
        ...
        # E. Operator signature / Approval ID / Approval Hash / Dispatch statement / IDs checks
        ...
        # F. Timestamp check
        is_date_key = key_name.lower() in ["approved_at", "retrieved_at", "created_at", "reviewed_at", "dispatched_at_redacted"] or (key_name.lower().endswith("_at") and not key_name.lower().endswith("_redacted"))
        ...
        # G. Public ready/dispatch ready checks
    ...
    check_value(supervised_dispatch_packet)
    check_value(supervised_dispatch_input_contract)
    check_value(supervised_dispatch_blocked_template)
    check_value(supervised_dispatch_blocked_output)
    check_value(supervised_dispatch_gate_matrix)
    check_value(supervised_dispatch_checklist)
    ...
    report = {
        ...
        "runtime_truth": False,
        "supervised_dispatch_validated": True,
        "blockers": blockers,
