"""Manual Publish Record Stub to Metrics Precheck contract, 0175AW.

Deterministic local-only contract defining metrics precheck records.
"""
...
TASK_LABEL = "TASK_CONTENTOPS_0175AW_MANUAL_PUBLISH_RECORD_STUB_TO_METRICS_PRECHECK_V0"
MATRIX_VERSION = "0175AW_MANUAL_PUBLISH_RECORD_STUB_TO_METRICS_PRECHECK_V1"
SOURCE_BASELINE_COMMIT = "1c8d66919f6e577b247f32b096b12f7eccd09bd6"
LEDGER_FAMILY = "manual_publish_record_stub_to_metrics_precheck_future"
HASH_ALGORITHM = "sha256"
DOC_REL_DIR = Path("docs") / "automation" / "0175AW"
PACKET_FILENAME = "manual_publish_record_stub_to_metrics_precheck_contract_packet.json"
RUNBOOK_FILENAME = "manual_publish_record_stub_to_metrics_precheck_contract.md"
...
@dataclass(frozen=True)
class MetricsPrecheckMetricField:
    field_name: str
...
@dataclass(frozen=True)
class MetricsPrecheckInvariant:
    invariant_id: str
    expected_state: str
    actual_state: str
    passed: bool
...
@dataclass(frozen=True)
class MetricsPrecheckRecord:
    metrics_precheck_id: str
    ...
    platform_target_id: str
    platform_family: str
    metrics_precheck_status: str
    metrics_target_type: str
    metric_fields: list[MetricsPrecheckMetricField]
    invariants: list[MetricsPrecheckInvariant]
...
@dataclass(frozen=True)
class MetricsPrecheckPacket:
    task_label: str
    matrix_version: str
    source_baseline_commit: str
    generated_at_epoch: int
    precheck_records: list[MetricsPrecheckRecord]
    precheck_targets: list[MetricsPrecheckTarget]
...
def build_precheck_targets() -> list[MetricsPrecheckTarget]:
    targets = {
    ...
    return [
        MetricsPrecheckTarget(platform_target_id=tid, metrics_target_type=val[0], description=val[1])
        for tid, val in targets.items()
    ]


def build_metric_fields_by_target(platform_target_id: str) -> list[MetricsPrecheckMetricField]:
    fields_map = {
    ...
    return [
        MetricsPrecheckMetricField(
            field_name=fname,
            placeholder_value=f"[METRICS_PRECHECK_STUB_ONLY: {platform_target_id}.{fname}]"
        )
        for fname in field_names
    ]


def build_invariants() -> list[MetricsPrecheckInvariant]:
    invariants = [
...
def build_contract_packet() -> dict[str, Any]:
    ...
    target_type_map = {t.platform_target_id: t.metrics_target_type for t in precheck_targets}
    ...
    for rec in stub_records:
        ...
        metrics_type = target_type_map.get(tid, "generic_metrics_precheck")
        metric_fields = build_metric_fields_by_target(tid)

        raw_record = {
            "metrics_precheck_id": f"metrics_precheck_{tid}",
            "source_manual_publish_record_stub_id": rec["manual_publish_record_stub_id"],
            ...
            "metrics_precheck_status": "metrics_precheck_blocked",
            "metrics_target_type": metrics_type,
            "metric_fields": [_asdict(f) for f in metric_fields],
            "invariants": [_asdict(inv) for inv in invariants],
            "blocked_reasons": blocked_reasons,
            "missing_future_gates": ["lane_c_platform_metrics_precheck_to_metrics_record_stub", "production_key_vault_decrypter", "live_operator_signature_vault"],
            ...
            "metrics_gate_status": "metrics_gate_required_but_locked",
    ...
    fields_count = sum(len(rec.metric_fields) for rec in precheck_records)
    ...
    summary_counts = {
        ...
        "precheck_metric_fields_count": fields_count,
    ...
    packet = MetricsPrecheckPacket(
        ...
        next_required_gate="lane_c_platform_metrics_precheck_to_metrics_record_stub"
    )
...
def render_runbook(packet: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        "",
        f"- **Registered Metrics Precheck Records**: `{counts['registered_precheck_records_count']}`",
        f"- **Registered Metrics Precheck Targets**: `{counts['precheck_targets_count']}`",
        f"- **Invariants Checked**: `{counts['precheck_invariants_count']}`",
        f"- **Placeholder Metric Fields Defined**: `{counts['precheck_metric_fields_count']}`",
        "",
    ...
    for t in targets:
        lines.append(f"| `{t['platform_target_id']}` | `{t['metrics_target_type']}` | {t['description']} |")
    ...
    for r in records:
        lines.extend([
            f"### Metrics Precheck Record: `{r['metrics_precheck_id']}`",
            "",
            ...
            f"- **Metrics Precheck Status**: `{r['metrics_precheck_status']}`",
...
def write_artifacts(repo_root: str | Path = ".", output_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    if out != allowed:
        raise ValueError("artifact_writer_refuses_paths_outside_docs_automation_0175AW")
