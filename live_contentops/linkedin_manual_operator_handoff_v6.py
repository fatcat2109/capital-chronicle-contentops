...
class LinkedInManualOperatorHandoffError(ValueError): pass
def build_linkedin_manual_operator_handoff_packet(export_packet,evidence_packet):
    _assert_safe(export_packet,LinkedInManualOperatorHandoffError); _assert_safe(evidence_packet,LinkedInManualOperatorHandoffError)
    eid=_require_str(export_packet,'export_packet_id',LinkedInManualOperatorHandoffError); exact=_require_str(export_packet,'exact_payload_hash',LinkedInManualOperatorHandoffError); evid=_require_str(evidence_packet,'approval_export_evidence_packet_id',LinkedInManualOperatorHandoffError); evh=_require_str(evidence_packet,'approval_export_evidence_hash',LinkedInManualOperatorHandoffError)
    sid=_require_str(export_packet,'source_article_packet_id',LinkedInManualOperatorHandoffError); sh=_require_str(export_packet,'source_canonical_hash',LinkedInManualOperatorHandoffError)
    _eq(evidence_packet.get('source_export_packet_id'),eid,'export_packet_id',LinkedInManualOperatorHandoffError); _eq(evidence_packet.get('exact_payload_hash'),exact,'exact_payload_hash',LinkedInManualOperatorHandoffError)
    for pkt in (export_packet,evidence_packet):
        for k in ('linkedin_api_used','provider_call_made','network_call_made','credential_read_made','env_value_read_made','browser_session_used','enabled_publish_send_dispatch_approve_controls'):_require_false(pkt,k,LinkedInManualOperatorHandoffError)
    checklist=[{'check_id':'confirm_article_source','label':'Confirm canonical article source packet and hash','status':'pending_review','required':True},{'check_id':'confirm_export_payload','label':'Confirm LinkedIn manual export payload hash before copy','status':'pending_review','required':True},{'check_id':'confirm_approval_evidence','label':'Confirm approval/export evidence packet remains pending','status':'pending_review','required':True},{'check_id':'confirm_manual_copy_only','label':'Confirm manual copy only; no LinkedIn API, publish, send, dispatch, scheduler, DM, comment, like, or reaction','status':'pending_review','required':True}]
    instructions=['Open canonical V5 Manual Export, Approval Queue, and Evidence Vault views only.','Compare article, export, approval/export evidence, and handoff hashes before manual copy.','If separate human approval is granted outside this packet, manually copy the payload into LinkedIn outside ContentOps.','Do not use LinkedIn API, browser automation, live publish, dispatch, scheduler, provider calls, env values, credentials, browser sessions, cookies, localStorage, tokens, DMs, comments, likes, or reactions.']
    ...
    core={'schema_version':SCHEMA_VERSION,'task_label':TASK_LABEL,'source_article_packet_id':sid,'source_article_hash':sh,'source_export_packet_id':eid,'source_export_payload_hash':exact,'approval_export_evidence_packet_id':evid,'approval_export_evidence_hash':evh,'manual_copy_payload':_require_mapping(export_packet,'manual_copy_payload',LinkedInManualOperatorHandoffError),'exact_payload_hash':_stable_hash({'manual_copy_payload':export_packet['manual_copy_payload'],'operator_instructions':instructions}),'hash_algorithm':HASH_ALGORITHM,'approval_status':'pending','operator_handoff_status':'ready_for_manual_review','manual_copy_only':True,'linkedin_api_used':False,'provider_call_made':False,'network_call_made':False,'credential_read_made':False,'env_value_read_made':False,'browser_session_used':False,'live_publish_allowed':False,'live_publish_performed':False,'sample_scope':SAMPLE_SCOPE,'manual_copy_checklist':checklist,'operator_instructions':instructions,'evidence_cards':cards,'blockers':['operator_approval_pending','live_publish_disabled','manual_copy_only','linkedin_api_disabled'],'blocked_controls':['approve','send','publish','dispatch','schedule'],'enabled_publish_send_dispatch_approve_controls':False,'warnings':['sample_fixture_only','manual_copy_only_no_linkedin_api','live_publish_disabled','operator_handoff_pending_review'],'recommended_next_task':'TASK_CONTENTOPS_V6_LINKEDIN_MANUAL_PUBLICATION_URL_AUDIT_IMPORT_V0'}
    ph=_stable_hash(core); core['evidence_cards']=[*cards,{'card_id':'operator_handoff_packet','card_type':'operator_handoff_packet','display_status':'ready_for_manual_review','source_id':'operator_handoff','hash':ph}]; ph=_stable_hash(core); packet={'operator_handoff_packet_id':f'linkedin_manual_operator_handoff_{ph[:16]}','operator_handoff_hash':ph,**core}; _assert_safe(packet,LinkedInManualOperatorHandoffError); return packet
def main():
    p=argparse.ArgumentParser(); p.add_argument('--export-input',required=True,type=Path); p.add_argument('--evidence-input',required=True,type=Path); p.add_argument('--output',required=True,type=Path); a=p.parse_args(); _write(a.output, build_linkedin_manual_operator_handoff_packet(load_json(a.export_input),load_json(a.evidence_input)))
