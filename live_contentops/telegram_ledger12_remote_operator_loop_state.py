"""Ledger-12 Telegram remote operator loop state after thirteenth send.

Task 0174WV/WW/WX. Promotes accepted thirteenth-send proof into local-only
...
TASK_LABEL = (
    "TASK_CONTENTOPS_0174WV_WW_WX_TELEGRAM_LEDGER12_REMOTE_OPERATOR_LOOP_"
    "RECONCILIATION_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER12_REMOTE_OPERATOR_LOOP_STATE_0174WV_WW_WX"
MODEL_VERSION = "0174WV_WW_WX_TELEGRAM_LEDGER12_REMOTE_OPERATOR_LOOP_STATE_V1"
SOURCE_BASELINE_COMMIT = "19c7c24253f8ab948e5034da8ce1739f15c365b8"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WV_WW_WX")
PACKET_FILENAME = "telegram_ledger12_remote_operator_loop_state_packet.json"
DOC_FILENAME = "telegram_ledger12_remote_operator_loop_state.md"
thirteenth_PROOF_REL = os.path.join(
    "docs", "automation", "0174WV_WW_WX", "telegram_ledger11_thirteenth_send_proof_packet.json")
MANUAL_GATE_REL = os.path.join(
    "docs", "automation", "0174WV_WW_WX", "telegram_ledger11_next_manual_gate_packet.json")
PREVIOUS_LOOP_REL = os.path.join("docs", "automation", "0174WS_WT_WU", "telegram_ledger11_remote_operator_loop_state_packet.json")

RECONCILE_OK = "ledger12_reconciliation_ok_ledger_advanced_to_12"
RECONCILE_MISSING = "ledger12_reconciliation_blocked_missing_thirteenth_send_proof"
RECONCILE_SEND_NOT_SUCCEEDED = "ledger12_reconciliation_blocked_send_not_succeeded"
RECONCILE_GATE_NOT_REVALIDATED = "ledger12_reconciliation_blocked_manual_gate_not_revalidated"
RECONCILE_LEDGER_NOT_ADVANCED = "ledger12_reconciliation_blocked_ledger_not_advanced"
RECONCILE_MANIFEST_NOT_ADVANCED = "ledger12_reconciliation_blocked_manifest_not_advanced"
RECONCILE_FAIL_CLOSED = "ledger12_reconciliation_fail_closed_forbidden_value"
TRUTH_CLASS = "remote_loop_truth_ledger_count_12_test13_accepted"
NEXT_ALLOWED_ACTION = "prepare_next_manual_gate_candidate"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WY_WZ_XA_LEDGER12_TO_LEDGER13_FOURTEENTH_"
    "SEND_BATCH_V0"
)
thirteenth_GATE_ID = "ledger11_next_manual_gate_transient_local_gate_v1"
...
def _entry_from_proof(proof):
    ...
    entry = ledger.build_ledger_entry(ev, operator_gate_id=thirteenth_GATE_ID)
    p = proof or {}
...
def reconcile_thirteenth_send_proof(thirteenth_send_proof, previous_loop_state_packet=None):
    p = thirteenth_send_proof or {}
    blockers = []
    ...
    elif not (p.get("ledger_appended") is True and p.get("ledger_entry_count_before") == 11 and p.get("ledger_entry_count") == 12 and p.get("request_budget_used") == 1):
    ...
    result = {
        ...
        "thirteenth_send_proof_checksum": p.get("evidence_checksum") or compute_checksum(p) if p else None,
...
def build_ledger12_remote_operator_loop_state(thirteenth_send_proof, manual_gate_packet,
                                             previous_loop_state_packet=None):
    proof = thirteenth_send_proof or {}
    ...
    reconciliation = reconcile_thirteenth_send_proof(proof, prev_packet)
    reconciled = reconciliation.get("reconciled") is True
    entry = _entry_from_proof(proof) if proof else {}
    audit_refs = {
        "thirteenth_send_proof_checksum": proof.get("evidence_checksum") or compute_checksum(proof) if proof else None,
...
def _candidate_from_proof(proof, *, sequence=None, response_suffix=None):
    ...
    if response_suffix:
        ev["response_checksum"] = compute_checksum({
            "kind": "ledger12_new_candidate_response_placeholder",
...
def _new_payload_candidate_from_proof(proof):
    ...
    ev["send_text_checksum"] = compute_checksum({
        "kind": "ledger12_new_payload_example",
...
def build_artifact_packet(thirteenth_send_proof, manual_gate_packet, previous_loop_state_packet=None):
    proof = thirteenth_send_proof or {}
    state = build_ledger12_remote_operator_loop_state(
    ...
    examples = {
        ...
        "exact_replay": build_next_gate_precheck_state(
            state, exact_replay_candidate, fresh_operator_gate_id=thirteenth_GATE_ID),
        ...
        "same_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, same_payload_candidate, fresh_operator_gate_id="fresh_ledger12_next_gate"),
        "new_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, new_payload_candidate, fresh_operator_gate_id="fresh_ledger12_next_gate"),
    }
    packet = {
        "task_label": TASK_LABEL,
        "model": MODEL,
...
def build_artifact_from_repo(repo_root):
    ...
    thirteenth = load_packet(os.path.join(root, thirteenth_PROOF_REL))
    manual = load_packet(os.path.join(root, MANUAL_GATE_REL))
    previous = load_packet(os.path.join(root, PREVIOUS_LOOP_REL))
    return build_artifact_packet(thirteenth, manual, previous_loop_state_packet=previous)
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger12 remote loop artifacts: scan found %d violation(s)" % len(violations))
