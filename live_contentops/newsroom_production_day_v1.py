...
def bounded_deficit_work_needed(
    *,
    ...
    routine windows. Capacity never grants evidence, truth, or publication authority and never
    authorizes filler. Qualified article count remains separate economics/dedupe telemetry.
    """
...
def count_reconciled_published_articles(
    ...
    for value in published_corpus:
        ...
        if isinstance(value, Mapping):
            published_at = value.get("published_at_utc")
            identity = value.get("article_identity")
        else:
            published_at = getattr(value, "published_at_utc", None)
            identity = getattr(value, "article_identity", None)
        try:
