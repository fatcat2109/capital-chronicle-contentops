"""V6 Discord Permission Probe Preflight Gate."""
...
VALUE_BINDING_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_WEBHOOK_VALUE_BINDING_PREFLIGHT_FROM_ENDPOINT_MAPPING_V0"
...
class DiscordPermissionProbePreflightPacket:
    schema_version: str
    task_label: str
    discord_permission_probe_preflight_id: str
    discord_permission_probe_declaration_id: str
    operator_id: str
    ...
    probe_kind: str
    permission_probe_preflight_available: bool
    permission_probe_preflight_declared_ready: bool
    eligible_for_discord_dry_run_payload_gate: bool
    ...
    channel_identity_verification_later_required: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != VALUE_BINDING_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("discord_webhook_value_binding_preflight_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("discord_webhook_value_binding_preflight_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_discord_permission_probe_gate") is not True:
        blockers.append("preflight_eligible_for_discord_permission_probe_gate_invalid")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "channel_identity_verification_later_required",
        "permission_verification_later_required",
        "payload_hash_revalidation_later_required",
    ...
    if decl.get("permission_probe_mode") != "operator_declared_discord_permission_probe_preflight_only_not_call":
        blockers.append("declaration_permission_probe_mode_invalid")
    ...
    if decl.get("probe_kind") != "discord_webhook_permission_probe_later_not_now":
        blockers.append("declaration_probe_kind_invalid")
    ...
    if decision not in ["mark_discord_permission_probe_preflight_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_permission_probe_preflight_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_PERMISSION_PROBE_PREFLIGHT_READY_ONLY_NOT_CALL":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_permission_probe_preflight_only":
...
def make_discord_permission_probe_preflight_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordPermissionProbePreflightPacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_webhook_value_binding_preflight_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_discord_permission_probe_declaration_json")
    ...
    channel_identity_verification_later_required = False
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_permission_probe_declaration_id = str(operator_declaration.get("discord_permission_probe_declaration_id") or "")
        ...
        probe_kind = str(operator_declaration.get("probe_kind") or "")
        ...
        discord_api_call_allowed = bool(operator_declaration.get("discord_api_call_allowed"))
        webhook_send_test_allowed = bool(operator_declaration.get("webhook_send_test_allowed"))
        channel_identity_value_allowed = bool(operator_declaration.get("channel_identity_value_allowed"))

        channel_identity_verification_later_required = bool(operator_declaration.get("channel_identity_verification_later_required"))
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_permission_probe_preflight_ready")
    )

    eligible_dry_run = False
    if prepared and declared_ready:
        eligible_dry_run = True

    # Safety redaction on secret/marker detection
    has_secrets = any("secret_marker_detected" in b for b in blockers)

    if has_secrets:
        discord_permission_probe_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_permission_probe_declaration_id": discord_permission_probe_declaration_id,
        "blockers": blockers,
    }
    discord_permission_probe_preflight_id = f"discord_permission_probe_preflight_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_permission_probe_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordPermissionProbePreflightPacket(
        ...
        permission_probe_preflight_available=prepared,
        permission_probe_preflight_declared_ready=declared_ready,
        eligible_for_discord_dry_run_payload_gate=eligible_dry_run,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
        ...
        channel_identity_verification_later_required=channel_identity_verification_later_required,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Permission Probe Preflight CLI")
    parser.add_argument("preflight_packet", help="Path to Discord webhook value binding preflight packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator Discord permission probe declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write permission probe preflight packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_webhook_value_binding_preflight_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_discord_permission_probe_declaration_json")

        packet = make_discord_permission_probe_preflight_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.permission_probe_preflight_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordPermissionProbePreflightPacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_permission_probe_preflight_id=f"discord_permission_probe_preflight_blocked_{str(val_err)}",
            discord_permission_probe_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_webhook_value_binding_preflight_id="",
            ...
            credential_key_name="",
            probe_kind="",
            permission_probe_preflight_available=False,
            permission_probe_preflight_declared_ready=False,
            eligible_for_discord_dry_run_payload_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            webhook_value_read_allowed=False,
            webhook_url_shape_validation_allowed=False,
            discord_api_call_allowed=False,
            ...
            warnings=["discord_permission_probe_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
