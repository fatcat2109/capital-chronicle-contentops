"""V6 Discord Exact Operator Live Dispatch Approval Gate."""
...
MATERIALIZATION_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_SUPERVISED_LIVE_PILOT_MATERIALIZATION_GATE_FROM_EXPLICIT_LIVE_SCOPE_CONTRACT_V0"
...
ALLOWED_SCHEMA_PHRASES = (
    ...
    "discord_exact_operator_live_dispatch_approval_record_gate_only",
...
class DiscordExactOperatorLiveDispatchApprovalGatePacket:
    schema_version: str
    task_label: str
    discord_exact_operator_live_dispatch_approval_gate_id: str
    discord_exact_operator_live_dispatch_approval_declaration_id: str
    operator_id: str
    ...
    platform: str
    approved_action_class_label: str
    approved_platform_family_label: str
    ...
    approved_idempotency_required: bool
    approved_kill_switch_required: bool
    approved_audit_redaction_required: bool
    approved_manual_fallback_required: bool
    approved_permission_probe_required_later: bool
    approved_payload_hash_revalidation_required: bool
    approved_destination_binding_required_later: bool
    approved_request_artifact_creation_required_later: bool
    approved_package_non_executable: bool
    approved_contains_webhook_url: bool
    approved_contains_webhook_token: bool
    approved_contains_endpoint_url: bool
    approved_contains_channel_identity: bool
    approved_contains_http_headers: bool
    approved_contains_http_body: bool
    approved_contains_curl_command: bool
    approved_contains_browser_instruction: bool
    operator_exact_approval_decision: str
    live_dispatch_approval_granted: bool
    approval_for_live_dispatch: bool
    dispatch_allowed: bool
    publication_approval_granted: bool
    approval_for_publication: bool
    ...
    discord_exact_operator_live_dispatch_approval_gate_available: bool
    discord_exact_operator_live_dispatch_approval_gate_declared_ready: bool
    eligible_for_supervised_request_artifact_draft_gate: bool
    eligible_for_full_live_dispatch_endpoint_mapping_gate: bool
    next_supervised_request_artifact_draft_gate_required: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != MATERIALIZATION_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("discord_supervised_live_pilot_materialization_gate_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("discord_supervised_live_pilot_materialization_gate_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_exact_operator_live_dispatch_approval_gate") is not True:
        blockers.append("preflight_eligible_for_exact_operator_live_dispatch_approval_gate_invalid")
    ...
    t_sec = packet.get("materialized_timeout_seconds")
    if not isinstance(t_sec, int) or isinstance(t_sec, bool) or t_sec < 5 or t_sec > 30:
        blockers.append("preflight_materialized_timeout_seconds_invalid")
    ...
    if not packet.get("materialized_payload_hash"):
        blockers.append("preflight_materialized_payload_hash_missing")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "public_url_included",
        "metrics_included",
        "operator_exact_approval_decision",
        "next_supervised_request_artifact_draft_gate_required",
    ...
    if decl.get("exact_approval_mode") != "operator_declared_discord_exact_live_dispatch_approval_record_only_not_live":
        blockers.append("declaration_exact_approval_mode_invalid")

    if decl.get("platform") != "discord":
        blockers.append("declaration_platform_invalid")

    if decl.get("approved_action_class_label") != "supervised_discord_webhook_send_pilot_exact_approval_record_future_only":
        blockers.append("declaration_approved_action_class_label_invalid")

    if decl.get("approved_platform_family_label") != "discord_webhook":
        blockers.append("declaration_approved_platform_family_label_invalid")

    if decl.get("approved_endpoint_family_label") != "discord_execute_webhook_label_only":
        blockers.append("declaration_approved_endpoint_family_label_invalid")

    if decl.get("approved_credential_key_name") != "DISCORD_LIVE_ANNOUNCEMENTS_WEBHOOK":
        blockers.append("declaration_approved_credential_key_name_invalid")

    if decl.get("approved_payload_hash") != preflight.get("materialized_payload_hash"):
        blockers.append("declaration_approved_payload_hash_mismatch")

    if decl.get("approved_request_budget_label") != "single_supervised_request_future_scope":
        blockers.append("declaration_approved_request_budget_label_invalid")

    if decl.get("approved_max_request_count") != preflight.get("materialized_max_request_count"):
        blockers.append("declaration_approved_max_request_count_mismatch")

    if decl.get("approved_max_request_count") != 1:
        blockers.append("declaration_approved_max_request_count_invalid")

    if decl.get("approved_timeout_seconds") != preflight.get("materialized_timeout_seconds"):
        blockers.append("declaration_approved_timeout_seconds_mismatch")

    t_sec = decl.get("approved_timeout_seconds")
    if not isinstance(t_sec, int) or isinstance(t_sec, bool) or t_sec < 5 or t_sec > 30:
        blockers.append("declaration_approved_timeout_seconds_invalid")

    if decl.get("approved_max_retries") != 0:
        blockers.append("declaration_approved_max_retries_invalid")

    if decl.get("approved_hidden_retry_allowed") is not False:
        blockers.append("declaration_approved_hidden_retry_allowed_not_false")

    # Approved required confirmations must be True
    for f in [
        "approved_idempotency_required",
        "approved_kill_switch_required",
        "approved_audit_redaction_required",
    ...
    if decl.get("approved_package_non_executable") is not True:
        blockers.append("declaration_approved_package_non_executable_not_true")
    ...
    if decl.get("discord_supervised_live_pilot_materialization_gate_id") != preflight.get("discord_supervised_live_pilot_materialization_gate_id"):
        blockers.append("declaration_discord_supervised_live_pilot_materialization_gate_id_mismatch")
    ...
    app_decision = decl.get("operator_exact_approval_decision")
    if app_decision not in ["record_exact_operator_approval_for_future_request_artifact_draft_only", "reject", "defer"]:
        blockers.append("declaration_operator_exact_approval_decision_invalid")
    elif app_decision in ["reject", "defer"]:
        blockers.append(f"declaration_operator_exact_approval_rejected_or_deferred_{app_decision}")

    if decl.get("next_supervised_request_artifact_draft_gate_required") is not True:
        blockers.append("declaration_next_supervised_request_artifact_draft_gate_required_not_true")

    decision = decl.get("declaration_decision")
    if decision not in ["mark_discord_exact_operator_live_dispatch_approval_gate_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_exact_operator_live_dispatch_approval_gate_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_EXACT_OPERATOR_LIVE_DISPATCH_APPROVAL_RECORDED_ONLY_NOT_LIVE":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_exact_operator_live_dispatch_approval_record_gate_only":
...
def make_discord_exact_operator_live_dispatch_approval_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordExactOperatorLiveDispatchApprovalGatePacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_supervised_live_pilot_materialization_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_exact_live_dispatch_approval_declaration_json")
    ...
    approved_idempotency_required = False
    approved_kill_switch_required = False
    approved_audit_redaction_required = False
    approved_manual_fallback_required = False
    approved_permission_probe_required_later = False
    approved_payload_hash_revalidation_required = False
    approved_destination_binding_required_later = False
    approved_request_artifact_creation_required_later = False
    approved_package_non_executable = False
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_exact_operator_live_dispatch_approval_declaration_id = str(operator_declaration.get("discord_exact_operator_live_dispatch_approval_declaration_id") or "")
        ...
        approved_action_class_label = str(operator_declaration.get("approved_action_class_label") or "")
        approved_platform_family_label = str(operator_declaration.get("approved_platform_family_label") or "")
        approved_endpoint_family_label = str(operator_declaration.get("approved_endpoint_family_label") or "")
        approved_credential_key_name = str(operator_declaration.get("approved_credential_key_name") or "")
        approved_payload_hash = str(operator_declaration.get("approved_payload_hash") or "")
        approved_request_budget_label = str(operator_declaration.get("approved_request_budget_label") or "")
        approved_max_request_count = int(operator_declaration.get("approved_max_request_count") or 0)
        approved_timeout_seconds = int(operator_declaration.get("approved_timeout_seconds") or 0)
        approved_max_retries = int(operator_declaration.get("approved_max_retries") or 0)
        approved_hidden_retry_allowed = bool(operator_declaration.get("approved_hidden_retry_allowed"))
        approved_idempotency_required = bool(operator_declaration.get("approved_idempotency_required"))
        approved_kill_switch_required = bool(operator_declaration.get("approved_kill_switch_required"))
        approved_audit_redaction_required = bool(operator_declaration.get("approved_audit_redaction_required"))
        approved_manual_fallback_required = bool(operator_declaration.get("approved_manual_fallback_required"))
        approved_permission_probe_required_later = bool(operator_declaration.get("approved_permission_probe_required_later"))
        approved_payload_hash_revalidation_required = bool(operator_declaration.get("approved_payload_hash_revalidation_required"))
        approved_destination_binding_required_later = bool(operator_declaration.get("approved_destination_binding_required_later"))
        approved_request_artifact_creation_required_later = bool(operator_declaration.get("approved_request_artifact_creation_required_later"))
        approved_package_non_executable = bool(operator_declaration.get("approved_package_non_executable"))
        approved_contains_webhook_url = bool(operator_declaration.get("approved_contains_webhook_url"))
        approved_contains_webhook_token = bool(operator_declaration.get("approved_contains_webhook_token"))
        approved_contains_endpoint_url = bool(operator_declaration.get("approved_contains_endpoint_url"))
        approved_contains_channel_identity = bool(operator_declaration.get("approved_contains_channel_identity"))
        approved_contains_http_headers = bool(operator_declaration.get("approved_contains_http_headers"))
        approved_contains_http_body = bool(operator_declaration.get("approved_contains_http_body"))
        approved_contains_curl_command = bool(operator_declaration.get("approved_contains_curl_command"))
        approved_contains_browser_instruction = bool(operator_declaration.get("approved_contains_browser_instruction"))
        operator_exact_approval_decision = str(operator_declaration.get("operator_exact_approval_decision") or "")
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_exact_operator_live_dispatch_approval_gate_ready")
    )

    eligible_draft = False
    if (
        prepared
        and declared_ready
        and approved_payload_hash == preflight_packet.get("materialized_payload_hash")
        and approved_max_request_count == 1
        and approved_max_retries == 0
        and not approved_hidden_retry_allowed
        and approved_idempotency_required
        and approved_kill_switch_required
        and approved_audit_redaction_required
        and approved_manual_fallback_required
        and approved_permission_probe_required_later
        and approved_payload_hash_revalidation_required
        and approved_destination_binding_required_later
        and approved_request_artifact_creation_required_later
        and approved_package_non_executable
        and not approved_contains_webhook_url
        and not approved_contains_webhook_token
        and not approved_contains_endpoint_url
        and not approved_contains_channel_identity
        and not approved_contains_http_headers
        and not approved_contains_http_body
        and not approved_contains_curl_command
        and not approved_contains_browser_instruction
        and operator_exact_approval_decision == "record_exact_operator_approval_for_future_request_artifact_draft_only"
    ):
        eligible_draft = True
    ...
    if has_secrets:
        discord_exact_operator_live_dispatch_approval_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_exact_operator_live_dispatch_approval_declaration_id": discord_exact_operator_live_dispatch_approval_declaration_id,
        "blockers": blockers,
    }
    discord_exact_operator_live_dispatch_approval_gate_id = f"discord_exact_operator_live_dispatch_approval_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_exact_operator_live_dispatch_approval_gate_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordExactOperatorLiveDispatchApprovalGatePacket(
        ...
        platform=platform,
        approved_action_class_label=approved_action_class_label,
        approved_platform_family_label=approved_platform_family_label,
        ...
        approved_idempotency_required=approved_idempotency_required,
        approved_kill_switch_required=approved_kill_switch_required,
        approved_audit_redaction_required=approved_audit_redaction_required,
        approved_manual_fallback_required=approved_manual_fallback_required,
        approved_permission_probe_required_later=approved_permission_probe_required_later,
        approved_payload_hash_revalidation_required=approved_payload_hash_revalidation_required,
        approved_destination_binding_required_later=approved_destination_binding_required_later,
        approved_request_artifact_creation_required_later=approved_request_artifact_creation_required_later,
        approved_package_non_executable=approved_package_non_executable,
        approved_contains_webhook_url=approved_contains_webhook_url,
        approved_contains_webhook_token=approved_contains_webhook_token,
        approved_contains_endpoint_url=approved_contains_endpoint_url,
        approved_contains_channel_identity=approved_contains_channel_identity,
        approved_contains_http_headers=approved_contains_http_headers,
        approved_contains_http_body=approved_contains_http_body,
        approved_contains_curl_command=approved_contains_curl_command,
        approved_contains_browser_instruction=approved_contains_browser_instruction,
        operator_exact_approval_decision=operator_exact_approval_decision,
        live_dispatch_approval_granted=live_dispatch_approval_granted,
        approval_for_live_dispatch=approval_for_live_dispatch,
        dispatch_allowed=dispatch_allowed,
        publication_approval_granted=publication_approval_granted,
        approval_for_publication=approval_for_publication,
        ...
        discord_exact_operator_live_dispatch_approval_gate_available=prepared,
        discord_exact_operator_live_dispatch_approval_gate_declared_ready=declared_ready,
        eligible_for_supervised_request_artifact_draft_gate=eligible_draft,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Exact Operator Live Dispatch Approval CLI")
    parser.add_argument("preflight_packet", help="Path to Discord supervised live pilot materialization packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator exact live dispatch approval declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write exact operator live dispatch approval packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_supervised_live_pilot_materialization_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_exact_live_dispatch_approval_declaration_json")

        packet = make_discord_exact_operator_live_dispatch_approval_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.discord_exact_operator_live_dispatch_approval_gate_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordExactOperatorLiveDispatchApprovalGatePacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_exact_operator_live_dispatch_approval_gate_id=f"discord_exact_operator_live_dispatch_approval_blocked_{str(val_err)}",
            discord_exact_operator_live_dispatch_approval_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_supervised_live_pilot_materialization_gate_id="",
            ...
            public_url_included=False,
            metrics_included=False,
            discord_exact_operator_live_dispatch_approval_gate_available=False,
            discord_exact_operator_live_dispatch_approval_gate_declared_ready=False,
            eligible_for_supervised_request_artifact_draft_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            next_supervised_request_artifact_draft_gate_required=True,
            substack_manual_fallback_required=True,
            all_platforms_endpoint_mapping_ready=False,
            blockers=[str(val_err)],
            warnings=["discord_exact_operator_live_dispatch_approval_gate_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
