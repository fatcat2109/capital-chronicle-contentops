"""Ledger-7 Telegram remote operator loop state after eighth send.

Task 0174WG/WH/WI. Promotes accepted eighth-send proof into local-only
...
TASK_LABEL = (
    "TASK_CONTENTOPS_0174WG_WH_WI_TELEGRAM_LEDGER7_REMOTE_OPERATOR_LOOP_"
    "RECONCILIATION_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER7_REMOTE_OPERATOR_LOOP_STATE_0174WG_WH_WI"
MODEL_VERSION = "0174WG_WH_WI_TELEGRAM_LEDGER7_REMOTE_OPERATOR_LOOP_STATE_V1"
SOURCE_BASELINE_COMMIT = "053ac56e35e363c0b1ca3f397e384e5a2ded6bcf"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WG_WH_WI")
PACKET_FILENAME = "telegram_ledger7_remote_operator_loop_state_packet.json"
DOC_FILENAME = "telegram_ledger7_remote_operator_loop_state.md"
EIGHTH_PROOF_REL = os.path.join(
    "docs", "automation", "0174WG_WH_WI", "telegram_ledger6_eighth_send_proof_packet.json")
MANUAL_GATE_REL = os.path.join(
    "docs", "automation", "0174WG_WH_WI", "telegram_ledger6_next_manual_gate_packet.json")
PREVIOUS_LOOP_REL = os.path.join(
    "docs", "automation", "0174WD_WE_WF", "telegram_ledger6_remote_operator_loop_state_packet.json")

RECONCILE_OK = "ledger7_reconciliation_ok_ledger_advanced_to_7"
RECONCILE_MISSING = "ledger7_reconciliation_blocked_missing_eighth_send_proof"
RECONCILE_SEND_NOT_SUCCEEDED = "ledger7_reconciliation_blocked_send_not_succeeded"
RECONCILE_GATE_NOT_REVALIDATED = "ledger7_reconciliation_blocked_manual_gate_not_revalidated"
RECONCILE_LEDGER_NOT_ADVANCED = "ledger7_reconciliation_blocked_ledger_not_advanced"
RECONCILE_MANIFEST_NOT_ADVANCED = "ledger7_reconciliation_blocked_manifest_not_advanced"
RECONCILE_FAIL_CLOSED = "ledger7_reconciliation_fail_closed_forbidden_value"
TRUTH_CLASS = "remote_loop_truth_ledger_count_7_test8_accepted"
NEXT_ALLOWED_ACTION = "prepare_next_manual_gate_candidate"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WG_WH_WI_TELEGRAM_LEDGER7_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
EIGHTH_GATE_ID = "ledger6_next_manual_gate_transient_local_gate_v1"
...
def _entry_from_proof(proof):
    ...
    entry = ledger.build_ledger_entry(ev, operator_gate_id=EIGHTH_GATE_ID)
    p = proof or {}
...
def reconcile_eighth_send_proof(eighth_send_proof, previous_loop_state_packet=None):
    p = eighth_send_proof or {}
    blockers = []
    ...
    elif not (p.get("ledger_appended") is True and p.get("ledger_entry_count_before") == 6
              and p.get("ledger_entry_count") == 7 and p.get("request_budget_used") == 1):
    ...
    result = {
        ...
        "eighth_send_proof_checksum": p.get("evidence_checksum") or compute_checksum(p) if p else None,
...
def build_ledger7_remote_operator_loop_state(eighth_send_proof, manual_gate_packet,
                                             previous_loop_state_packet=None):
    proof = eighth_send_proof or {}
    ...
    reconciliation = reconcile_eighth_send_proof(proof, prev_packet)
    reconciled = reconciliation.get("reconciled") is True
    entry = _entry_from_proof(proof) if proof else {}
    audit_refs = {
        "eighth_send_proof_checksum": proof.get("evidence_checksum") or compute_checksum(proof) if proof else None,
...
def _candidate_from_proof(proof, *, sequence=None, response_suffix=None):
    ...
    if response_suffix:
        ev["response_checksum"] = compute_checksum({
            "kind": "ledger7_new_candidate_response_placeholder",
...
def _new_payload_candidate_from_proof(proof):
    ev = _candidate_from_proof(proof, sequence=8, response_suffix="new_payload")
    ev["send_text_checksum"] = compute_checksum({
        "kind": "ledger7_new_payload_example",
...
def build_artifact_packet(eighth_send_proof, manual_gate_packet, previous_loop_state_packet=None):
    proof = eighth_send_proof or {}
    state = build_ledger7_remote_operator_loop_state(
    ...
    same_payload_candidate = _candidate_from_proof(proof, sequence=8, response_suffix="same_payload")
    ...
    examples = {
        ...
        "exact_replay": build_next_gate_precheck_state(
            state, exact_replay_candidate, fresh_operator_gate_id=EIGHTH_GATE_ID),
        ...
        "same_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, same_payload_candidate, fresh_operator_gate_id="fresh_ledger7_next_gate"),
        "new_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, new_payload_candidate, fresh_operator_gate_id="fresh_ledger7_next_gate"),
    }
    packet = {
        "task_label": TASK_LABEL,
        "model": MODEL,
...
def build_artifact_from_repo(repo_root):
    ...
    eighth = load_packet(os.path.join(root, EIGHTH_PROOF_REL))
    manual = load_packet(os.path.join(root, MANUAL_GATE_REL))
    previous = load_packet(os.path.join(root, PREVIOUS_LOOP_REL))
    return build_artifact_packet(eighth, manual, previous_loop_state_packet=previous)
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger7 remote loop artifacts: scan found %d violation(s)" % len(violations))
