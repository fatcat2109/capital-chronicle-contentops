...
ENTRYPOINTS: Final[tuple[LiveEntrypoint, ...]] = (
    LiveEntrypoint("contentops.production_orchestrator.v1", "live_contentops.production_orchestrator_v1", "ContentOpsProductionOrchestrator.execute / run", "Python API", True, "CANONICAL", "Canonical Substack-first publication and configured derivative writes", "Private provider/browser implementation reachable only after exact canonical operation validation", "Exact operation allowlist plus accepted approval/freeze/publication guards", "Unknown operations fail before private implementation import; accepted operations dispatch exactly once", (_TEST + "::test_exactly_one_registry_row_is_canonical", _TEST + "::test_orchestrator_is_lazy_and_bound", _TEST + "::test_orchestrator_rejects_unknown_operation_before_import")),
    LiveEntrypoint("contentops.canonical_module_cli.v1", "live_contentops.eight_platform_substack_first_pipeline_v1", "public compatibility APIs / main / script CLI", "Module CLI, script CLI, and historical Python API", False, "DELEGATE", "Canonical compatibility access for all accepted operation families", "Import-safe facade has no provider/browser/adapter reachability before delegation", "Every live-capable route must call ContentOpsProductionOrchestrator.execute exactly once", "Compatibility surface delegates to orchestrator, then one private dispatcher and one private implementation", (_TEST + "::test_public_compatibility_import_is_safe_and_all_live_apis_delegate_once", _TEST + "::test_every_live_capable_canonical_cli_family_delegates_once", _TEST + "::test_private_dispatcher_exact_map_has_no_public_wrapper_targets")),
