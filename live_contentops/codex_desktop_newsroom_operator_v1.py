...
DESKTOP_TASK_PROMPT = (
    ...
    "ingestion, cutoff, durable evaluated/update-chain/duplicate memory, candidate ranking, bounded "
    "learning, and exact nine-surface readiness. Use the proven current provider-resilient batch/tail "
    "discovery and governed evidence path when needed; do not restore the historical 35-call / "
    ...
    "partial evidence projection lacks it. Only when one real candidate has enough governed evidence "
    "and article production is warranted in any article mode, including BREAKING_BRIEF, create exactly "
    "one fresh isolated gpt-5.6-sol / XHIGH editorial worker using only the bounded governed packet and "
