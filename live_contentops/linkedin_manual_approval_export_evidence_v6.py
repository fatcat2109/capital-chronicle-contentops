...
class LinkedInManualApprovalExportEvidenceError(ValueError): pass
def build_linkedin_manual_approval_export_evidence_packet(export_packet):
    _assert_safe(export_packet, LinkedInManualApprovalExportEvidenceError)
    for k in ('linkedin_api_used','provider_call_made','network_call_made','credential_read_made','env_value_read_made','browser_session_used','live_publish_allowed','live_publish_performed','enabled_publish_send_dispatch_approve_controls'):_require_false(export_packet,k,LinkedInManualApprovalExportEvidenceError)
    _require_true(export_packet,'manual_copy_only',LinkedInManualApprovalExportEvidenceError)
    eid=_require_str(export_packet,'export_packet_id',LinkedInManualApprovalExportEvidenceError); exact=_require_str(export_packet,'exact_payload_hash',LinkedInManualApprovalExportEvidenceError); sid=_require_str(export_packet,'source_article_packet_id',LinkedInManualApprovalExportEvidenceError); sh=_require_str(export_packet,'source_canonical_hash',LinkedInManualApprovalExportEvidenceError)
    checklist=[{'check_id':'manual_copy_payload_present','label':'LinkedIn manual copy payload reviewed in V5','status':'pending_review','required':True},{'check_id':'operator_confirms_no_live_publish','label':'Operator confirms no publish/send/dispatch/schedule action is enabled','status':'pending_review','required':True},{'check_id':'operator_confirms_linkedin_api_absent','label':'Operator confirms LinkedIn API was not used','status':'pending_review','required':True},{'check_id':'operator_confirms_hash_match','label':'Operator confirms exact payload hash before manual copy','status':'pending_review','required':True}]
    cards=[{'card_id':'article_source_packet','card_type':'article_source_packet','display_status':'bound','source_id':sid,'hash':sh},{'card_id':'linkedin_export_packet','card_type':'linkedin_export_packet','display_status':'bound','source_id':eid,'hash':exact},{'card_id':'approval_checkpoint','card_type':'approval_checkpoint','display_status':'pending_review','source_id':'operator_review_status','hash':exact},{'card_id':'blocked_live_publish_state','card_type':'blocked_live_publish_state','display_status':'blocked','source_id':'live_publish_allowed=false','hash':_stable_hash({'live_publish_allowed':False,'linkedin_api_used':False})}]
    core={'schema_version':SCHEMA_VERSION,'task_label':TASK_LABEL,'source_export_packet_id':eid,'source_article_packet_id':sid,'source_canonical_hash':sh,'exact_payload_hash':exact,'operator_review_status':'pending_review','approval_status':'pending','manual_export_status':'ready_for_manual_copy','linkedin_api_used':False,'provider_call_made':False,'network_call_made':False,'credential_read_made':False,'env_value_read_made':False,'browser_session_used':False,'live_publish_allowed':False,'live_publish_performed':False,'sample_scope':SAMPLE_SCOPE,'hash_algorithm':HASH_ALGORITHM,'manual_copy_checklist':checklist,'evidence_cards':cards,'blocked_controls':['approve','send','publish','dispatch','schedule'],'enabled_publish_send_dispatch_approve_controls':False,'operator_review_proof':'pending operator review; deterministic fixture only; no runtime proof','warnings':['sample_fixture_only','manual_copy_only_no_linkedin_api','live_publish_disabled','operator_review_pending']}
    eh=_stable_hash(core); packet={'approval_export_evidence_packet_id':f'linkedin_manual_approval_export_evidence_{eh[:16]}','approval_export_evidence_hash':eh,**core}; _assert_safe(packet,LinkedInManualApprovalExportEvidenceError); return packet
def main():
    p=argparse.ArgumentParser(); p.add_argument('--input',required=True,type=Path); p.add_argument('--output',required=True,type=Path); a=p.parse_args(); _write(a.output, build_linkedin_manual_approval_export_evidence_packet(load_json(a.input)))
