...
RENDERER_SOURCE_FILES = (
    ...
    "src/types.ts",
    "src/beat.tsx",
    "src/generated/authored_shots.tsx",
)
...
MIN_NARRATION_LED_BEAT_SECONDS = 2.5
MASTERING_TRUE_PEAK_DBTP = -2.5
...
def generate_narration(
    ...
    *,
    ...
    for row in segments:
        ...
        # The Director target is a pacing allocation, not permission to inflate the whole
        # variant when local TTS runs long. Preserve every spoken byte at natural authored
        # speed, remove surplus silence from shorter neighboring beats, and let narration lead
        # only where it exceeds the 2.5-second professional shot floor. This keeps the authored
        # variant duration bounded without clipping or post-hoc speed manipulation.
        duration_seconds = max(MIN_NARRATION_LED_BEAT_SECONDS, audio_seconds + pause)
        duration_frames = int(math.ceil(duration_seconds * int(row["fps"])))
        row.update({
            "audio_duration_seconds": round(audio_seconds, 6),
            "authored_target_duration_seconds": float(row["target_duration_seconds"]),
            "narration_led_pacing": True,
            "minimum_beat_duration_seconds": MIN_NARRATION_LED_BEAT_SECONDS,
...
def _score_and_mix_variant(
    ...
    *,
    ...
    first_pass = _run([
        ...
        "-af", f"loudnorm=I=-16:TP={MASTERING_TRUE_PEAK_DBTP}:LRA=11:print_format=json",
        "-f", "null", "NUL" if os.name == "nt" else "/dev/null",
    ], capture=True, timeout=900)
    measured = _parse_loudnorm(first_pass.stderr)
    master = audio_dir / "master.wav"
    filter_value = (
        f"loudnorm=I=-16:TP={MASTERING_TRUE_PEAK_DBTP}:LRA=11:"
    ...
    return {
        ...
        "processing_true_peak_dbtp": MASTERING_TRUE_PEAK_DBTP,
