"""V6 Feedback Summary Backlog Validator.

Ensures empty feedback summary backlog contract states are secure, redacted, and blocked.
"""
...
def is_placeholder(val: Any) -> bool:
    ...
    if isinstance(val, str):
        ...
        exact_placeholders = {
            ...
            "feedback_summary_backlog_blocked_waiting_for_feedback_capture",
            "future_feedback_summary_backlog_input_contract_only",
            "blocked_template_only_not_feedback_summary",
            "blocked_no_feedback_summary_created",
            "blocked_missing_feedback_capture_and_policies",
            "feedback_summary_backlog_blocked_pending_feedback_capture",
            "feedback_summary_backlog_input_contract.json",
...
def validate_feedback_summary_backlog_contract(
    feedback_summary_backlog_packet: dict[str, Any],
    feedback_summary_backlog_input_contract: dict[str, Any],
    feedback_summary_backlog_blocked_template: dict[str, Any],
    feedback_summary_backlog_blocked_output: dict[str, Any],
    feedback_summary_backlog_gate_matrix: list[dict[str, Any]],
    feedback_summary_backlog_checklist: dict[str, Any]
) -> tuple[dict[str, Any], list[str]]:
    """Validates the offline dry-run feedback summary backlog contract state."""
    blockers = []
    failed = False
    ...
    for flag in packet_flags_to_fail:
        if feedback_summary_backlog_packet.get(flag) is True:
            blockers.append("feedback_summary_generation_blocked")
            failed = True
    ...
    for field in template_fields:
        val = feedback_summary_backlog_blocked_template.get(field)
        if val is not None and val != "":
            blockers.append("non_null_feedback_template_fields_detected")
            failed = True

    template_flags = [
        "summary_generation_performed", "backlog_item_created",
        "next_article_signal_created", "audit_record_mutated"
    ]
    for flag in template_flags:
        if feedback_summary_backlog_blocked_template.get(flag) is True:
    ...
    output_flags_to_fail = [
        "summary_generation_performed", "backlog_item_created",
        "next_article_signal_created", "audit_record_mutated", "public_url_created"
    ]
    for flag in output_flags_to_fail:
        if feedback_summary_backlog_blocked_output.get(flag) is True:
            blockers.append("feedback_summary_generation_blocked")
            failed = True
    ...
    for field in output_lists:
        if len(feedback_summary_backlog_blocked_output.get(field, [])) > 0:
    ...
    for key in count_keys:
        if feedback_summary_backlog_blocked_output.get(key, 0) > 0:
    ...
    for row in feedback_summary_backlog_gate_matrix:
        ...
        if row.get("summary_backlog_gate_status") != "blocked_missing_feedback_capture_and_policies":
            blockers.append("readiness_matrix_active_lane_detected")
            failed = True
    ...
    if feedback_summary_backlog_input_contract.get("contract_status") != "FUTURE_FEEDBACK_SUMMARY_BACKLOG_INPUT_CONTRACT_ONLY":
        blockers.append("platform_input_contract_incomplete")
        failed = True

    required_inputs = feedback_summary_backlog_input_contract.get("required_inputs")
    expected_names = {
        "community_feedback_capture_ref",
        "redacted_feedback_records_ref",
        "feedback_capture_policy_ref",
    ...
    if not isinstance(required_inputs, list) or len(required_inputs) == 0:
        ...
        for inp in required_inputs:
            ...
            if inp.get("blocks_feedback_summary_backlog_creation") is not True:
    ...
    def check_text(t: str, key_name: str = ""):
        ...
        is_op_key = key_name.lower() in [
            ...
            "backlog_item_statement", "outbox_entry_id", "approval_queue_entry_id", "dispatch_attempt_id",
        ...
        if "reaction" in key_name.lower() and not is_placeholder(t):
            ...
            failed = True
        if "summary" in key_name.lower() and not is_placeholder(t):
            blockers.append("summary_text_leak_detected")
            failed = True
        if "backlog" in key_name.lower() and not is_placeholder(t):
    ...
    check_value(feedback_summary_backlog_packet)
    check_value(feedback_summary_backlog_input_contract)
    check_value(feedback_summary_backlog_blocked_template)
    check_value(feedback_summary_backlog_blocked_output)
    check_value(feedback_summary_backlog_gate_matrix)
    check_value(feedback_summary_backlog_checklist)
    ...
    report = {
        ...
        "runtime_truth": False,
        "feedback_summary_backlog_validated": True,
        "blockers": blockers,
