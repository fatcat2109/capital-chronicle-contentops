"""V6 Canonical Article Studio Refinement Validator.

Ensures empty refinement states are secure, redacted, and unapproved.
"""
...
def is_placeholder(val: Any) -> bool:
    ...
    if isinstance(val, str):
        ...
        placeholders = [
            ...
            "editorial_refinement_blocked_waiting_for_rendered_draft",
            "future_refinement_input_contract_only",
            "blocked_no_refinement_rendered",
            "blocked_missing_rendered_draft",
            "refinement_blocked_pending_rendered_draft"
        ]
...
def validate_canonical_article_studio_editorial_refinement_queue(
    refinement_queue_packet: dict[str, Any],
    refinement_input_contract: dict[str, Any],
    blocked_refinement_output: dict[str, Any],
    refinement_slot_status_matrix: list[dict[str, Any]],
    refinement_checklist: dict[str, Any]
) -> tuple[dict[str, Any], list[str]]:
    """Validates unapproved refinement queue state and reports blockers."""
    blockers = []
    failed = False
    ...
    copy_keys = [
        ...
        "refinement_values_materialized"
    ]
    for key in copy_keys:
        if refinement_queue_packet.get(key) is True or blocked_refinement_output.get(key) is True:
            blockers.append("article_copy_generation_blocked")
            failed = True

    # 3. Check renderer availability/refinement flags
    if refinement_queue_packet.get("rendered_draft_available") is True:
        blockers.append("rendered_draft_available_claimed")
        failed = True

    # 4. Check refinement execution flags
    execution_flags = ["refinement_execution_allowed", "refinement_execution_performed"]
    for flag in execution_flags:
        if refinement_queue_packet.get(flag) is True or blocked_refinement_output.get(flag) is True:
            blockers.append("refinement_execution_blocked")
            failed = True

    # 4b. Check score generation flags
    score_flags = ["editorial_score_generated", "seo_score_generated", "readability_score_generated"]
    for flag in score_flags:
        if refinement_queue_packet.get(flag) is True:
            blockers.append("scores_generated_claimed")
            failed = True

    # 5. Fail if any slot rendered/refined/placeholder value is non-null
    for row in refinement_slot_status_matrix:
        if row.get("rendered_value") is not None:
            blockers.append("non_null_rendered_value_detected")
            failed = True
        if row.get("refined_value") is not None:
            blockers.append("non_null_refined_value_detected")
            failed = True
        if row.get("placeholder_value") is not None:
            blockers.append("non_null_placeholder_value_detected")
            failed = True
        if row.get("source_values_materialized") is True or row.get("refinement_values_materialized") is True or row.get("generated") is True:
            blockers.append("non_null_rendered_value_detected")
            failed = True
    ...
    # 6. Fail if draft/refinement fields in blocked output are non-null
    output_fields = ["refined_title", "refined_dek", "refined_body", "seo_title", "seo_meta_description"]
    for field in output_fields:
        if blocked_refinement_output.get(field) is not None:
            blockers.append("non_null_rendered_value_detected")
            failed = True

    # 6b. Fail if scores are non-null
    score_fields = ["editorial_score", "seo_score", "readability_score"]
    ...
    list_fields = ["editorial_notes", "seo_notes", "citations", "evidence_refs", "source_names"]
    for field in list_fields:
        if len(blocked_refinement_output.get(field, [])) > 0:
    ...
    for key in count_keys:
        if blocked_refinement_output.get(key, 0) > 0:
    ...
    # 10b. Check refinement input contract details
    required_inputs = refinement_input_contract.get("required_inputs")
    expected_names = {
    ...
    if not isinstance(required_inputs, list) or len(required_inputs) == 0:
        blockers.append("refinement_input_contract_invalid")
        failed = True
    else:
        found_names = set()
        for inp in required_inputs:
            ...
            if inp.get("value_ref") is not None:
                blockers.append("refinement_input_value_ref_present")
                failed = True

            if inp.get("raw_value_persisted") is not False:
                blockers.append("refinement_input_raw_value_persisted")
                failed = True

            if inp.get("blocks_refinement_execution") is not True:
                blockers.append("refinement_input_not_blocking_execution")
                failed = True
    ...
    check_value(refinement_queue_packet)
    check_value(refinement_input_contract)
    check_value(blocked_refinement_output)
    check_value(refinement_slot_status_matrix)
    check_value(refinement_checklist)
