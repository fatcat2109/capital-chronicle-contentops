"""Apply the owner-approved V1 simple Gemini reset in a guarded CI workspace.

This file is temporary setup machinery under .github/ and is removed before the product
commit. It imports the audited patch library without executing its main function, replaces
the CI-only detached-checkout guard, materializes the three new product/test files from
.github/reset templates, applies the exact legacy-blob patches, and updates the existing
ci-fast workflow only with permanent reset regression/authority checks.
"""
...
RESET_BRANCH = "agent/web-v1-simple-gemini-runtime-reset-v5"
...
expected_blobs[".github/workflows/ci-fast.yml"] = "4e86e52fa5ce48bc3b5e31bdb863181b8d1ca4fd"
...
# Reuse the already-audited final authority assertion block from the patch library while
# adapting only the ci-fast mechanics to the untouched master workflow.
