...
export const Root: React.FC = () => <>
  ...
  <Composition id="TreasuryPositioningShort" component={TreasuryPositioningShort} durationInFrames={1500} fps={30} width={1080} height={1920} defaultProps={{...defaults,variant:'short',scenes:[],audioFile:'audio/short.wav'} as PositioningProps} calculateMetadata={({props})=>({durationInFrames:Math.max(1,Math.ceil(props.scenes.reduce((sum,row)=>sum+row.duration_seconds,0)*30))})}/>
  <Composition id="TreasuryPositioningLongform" component={TreasuryPositioningLongform} durationInFrames={13500} fps={30} width={1920} height={1080} defaultProps={{...defaults,variant:'longform',scenes:[],audioFile:'audio/longform.wav'} as PositioningProps} calculateMetadata={({props})=>({durationInFrames:Math.max(1,Math.ceil(props.scenes.reduce((sum,row)=>sum+row.duration_seconds,0)*30))})}/>
</>;
