...
import {authoredShots, internalSourceLabelShotIds} from './generated/authored_shots';
...
const Chrome: React.FC<{job: BeatRenderJob; portrait: boolean; state: EditState}> = ({job, portrait, state}) => <>
  ...
  <div style={{position: 'absolute', left: portrait ? '6.5%' : '5.5%', bottom: portrait ? '0.8%' : '2.2%', right: portrait ? '6.5%' : '5.5%', color: '#90a5af', fontSize: portrait ? 15 : 16, lineHeight: 1.1, zIndex: 30, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 20}}>
...
export const RetentionNativeBeat: React.FC<{job: BeatRenderJob}> = ({job}) => {
  ...
  const AuthoredShot = authoredShots[job.beat_id];
  ...
  return <AbsoluteFill style={{fontFamily: 'Arial, Helvetica, sans-serif', background: navy, color: ink, overflow: 'hidden'}}>
    ...
    {AuthoredShot ? <AuthoredShot
      frame={frame}
      fps={config.fps}
      width={config.width}
      height={config.height}
      progress={Math.min(1, Math.max(0, frame / Math.max(1, config.durationInFrames - 1)))}
      assetPath={state.asset_path || undefined}
      assetClass={state.asset_class || undefined}
      narration={job.narration_text}
      sourceLabel={internalSourceLabelShotIds.has(job.beat_id) ? state.attribution || job.source_label : ''}
