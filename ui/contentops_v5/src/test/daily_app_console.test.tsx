...
function snapshot(overrides: Partial<DailyAppSnapshot> = {}): DailyAppSnapshot {
  const base: DailyAppSnapshot = {
    ...
    today: { current_cycle: null, pending_lifecycle_recovery_count: 1, immediate_incident_count: 1, published_today_count: 0, published_corpus_count: 0, daily_target_band: [5, 8], newsroom_production_day_id: 'newsroom-production-day-2026-08-10-bangkok', build_qualified_floor: 4, final_published_target_min: 5, final_published_target_max: 8, qualified_articles_today: 2, published_articles_today: 0, remaining_build_deficit: 2, production_day_state: 'DEFICIT_RECOVERABLE', hard_external_block_reason: null, routine_opportunities_used: 2, routine_opportunities_remaining: 2 },
    automation: { configured_intent: { state: 'CONFIGURED_INTENT', task_count: 4 }, observed_host_state: { state: 'OBSERVED_HOST_STATE', task_count: 4 }, observation_timestamp_utc: '2026-08-10T11:59:00Z', freshness: 'FRESH', age_seconds: 60 },
...
describe('Final Daily App production console', () => {
  it('renders final navigation and sparse real-state Today without a fake cycle', async () => {
    ...
    expect(screen.getByText('Qualified build articles')).toBeInTheDocument();
    expect(screen.getByText('2 / 4')).toBeInTheDocument();
    expect(screen.getByText('Final published target')).toBeInTheDocument();
