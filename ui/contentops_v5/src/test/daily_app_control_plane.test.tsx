...
function snapshot(): DailyAppSnapshot {
  return {
    ...
    today: { current_cycle: null, pending_lifecycle_recovery_count: 0, immediate_incident_count: 0, published_today_count: 0, published_corpus_count: 10, daily_target_band: [5, 8], newsroom_production_day_id: 'newsroom-production-day-2026-08-13-bangkok', build_qualified_floor: 4, final_published_target_min: 5, final_published_target_max: 8, qualified_articles_today: 1, published_articles_today: 0, remaining_build_deficit: 3, production_day_state: 'DEFICIT_RECOVERABLE', hard_external_block_reason: null, routine_opportunities_used: 1, routine_opportunities_remaining: 3 },
    automation: { configured_intent: { state: 'CONFIGURED_INTENT', task_count: 4 }, observed_host_state: { state: 'OBSERVED_HOST_STATE', task_count: 4, tasks: [{ id: 'v1-newsroom-london-1700', name: 'V1 Newsroom London 17:00', status: 'PAUSED', rrule: 'DTSTART;TZID=Asia/Bangkok:20260813T170000\nRRULE:FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR', timezone: 'Asia/Bangkok', model: 'gpt-5.6-sol', reasoning_effort: 'high', host_config_sha256: 'ea8e1e11' + 'a'.repeat(56), observation_projection_sha256: 'b'.repeat(64) }] }, observation_timestamp_utc: '2026-08-13T10:00:00Z', freshness: 'FRESH', age_seconds: 0 },
...
it('shows production-day deficit and observed Automation provenance', async () => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => snapshot() }));
  render(<DailyAppConsole />);
  expect(await screen.findByText('1 / 4')).toBeInTheDocument();
  expect(screen.getByText('5–8 / day')).toBeInTheDocument();
  expect(screen.getByText('newsroom-production-day-2026-08-13-bangkok')).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Codex Automations' })).toBeInTheDocument();
  expect(screen.getByText('V1 Newsroom London 17:00')).toBeInTheDocument();
  expect(screen.getByText('gpt-5.6-sol / HIGH')).toBeInTheDocument();
  expect(screen.getByText('Host config SHA256')).toBeInTheDocument();
  expect(screen.getByText('ea8e1e11' + 'a'.repeat(56))).toBeInTheDocument();
  expect(screen.getByText('Observation projection SHA256')).toBeInTheDocument();
});
