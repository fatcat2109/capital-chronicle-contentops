...
The current reusable learning authority is `TASK_CONTENTOPS_GENERIC_FOUNDATION_V2_GOVERNED_EVIDENCE_PROVENANCE_AND_ROLE_BINDING`. Commit `6f2755a471c41ccc5a6c06e8babcae2534dd065d` is accepted as `ACCEPT_GENERIC_AUTHORITY_GATE_COUNT_AND_DUPLICATE_REPAIR`; this repair removes caller-only governed-ref certification, binds evidence provenance and semantic roles, scopes feature evidence reuse, and exposes complete versus qualifying lineage without changing the 20 domain specifications, uncalibrated config, accepted release, or historical V1 adapters. Read `docs/architecture/CONTENTOPS_GENERICITY_AND_DOMAIN_GENERALIZATION_STANDARD_V1.md`, the prior repair manifest, and the current repair manifest before extending ContentOps intelligence or learning features. The next action is `INDEPENDENT_CHATGPT_AUDIT_GENERIC_FOUNDATION_V2_GOVERNED_EVIDENCE_PROVENANCE_AND_ROLE_BINDING`.
