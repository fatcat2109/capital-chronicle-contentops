...
Wave 01 classification is `PASS_WAVE01_CANONICAL_ORCHESTRATOR_BOUNDARY_ACCEPTED_AND_MERGED`. Wave 01 status is `COMPLETE_ACCEPTED_AND_MERGED`. Completed Wave 02 task is `TASK_CONTENTOPS_DURABLE_OPERATIONAL_STORE_AND_CANONICAL_STATE_MACHINE_V1`. Wave 02 classification is `PASS_WAVE02_DURABLE_STATE_TRANSACTION_FENCING_AND_AUTHORITY_CORRECTION_AWAITING_INDEPENDENT_AUDIT`. Wave 02 status is `COMPLETE_AWAITING_INDEPENDENT_AUDIT`. Wave 03 status is `NEXT_NOT_STARTED`. Wave 03 is now the exact next task, but it has not started.
