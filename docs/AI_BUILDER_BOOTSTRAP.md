...
The current reusable learning authority is `TASK_CONTENTOPS_EXTRACTED_AUTHORITY_PERMISSION_ROLE_AND_AGGREGATION_BINDING_V1`. Commit `165920c90e62d1cee0b5ea8dc8ec2ec9a149e2d4` is accepted as `ACCEPT_SCHEMA_AWARE_BYTE_EXTRACTION_PORTABLE_REPLAY_AND_REAL_CANARY_WITH_SEMANTIC_AUTHORITY_GAP`; the superseding repair retains portable exact-Git receipts and byte-derived extraction while making extractor contracts the maximum authority for evidence authority, permissions, and roles. Candidate authorization is an all-bound-record deterministic derivation, and multi-evidence feature values require exact evidence-set equality or a registered logical-hash-bound aggregation. The BLS, Treasury auction, and New York Fed canary remains context-only and no-publication. It does not change domain specifications, the uncalibrated config, accepted release, historical V1 adapters, DQR/permission policy, or upstream state. Read the mandatory genericity standard, the schema-aware manifest, and the current binding manifest before extending learning features. The next action is `INDEPENDENT_CHATGPT_AUDIT_EXTRACTED_AUTHORITY_PERMISSION_ROLE_AND_AGGREGATION_BINDING_V1`.
