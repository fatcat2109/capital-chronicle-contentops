...
Wave 02 classification is `PASS_DURABLE_OPERATIONAL_STORE_AND_CANONICAL_STATE_MACHINE_V1_AWAITING_INDEPENDENT_AUDIT`. Wave 02 status is `COMPLETE_AWAITING_INDEPENDENT_AUDIT`. Wave 03 status is `NEXT_NOT_STARTED`. Wave 03 is now the exact next task, but it has not started.
