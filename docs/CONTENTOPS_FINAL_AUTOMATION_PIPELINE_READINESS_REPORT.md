...
The current backend authority is `TASK_CONTENTOPS_SCHEMA_AWARE_EVIDENCE_EXTRACTION_AND_PORTABLE_REAL_CANARY_V1`, classified `PASS_SCHEMA_AWARE_EVIDENCE_EXTRACTION_AND_PORTABLE_REAL_CANARY_V1_AWAITING_CHATGPT_AUDIT`. It preserves committed verifier and exact Git transport proof while requiring registered schema-aware extraction for semantic evidence. Evidence refs, selected record hashes, artifact-native timestamps, and feature values derive from exact bytes; pinned producer commits remain valid ancestors after branch advancement. The real no-write canary uses BLS CPI, U.S. Treasury auction announcements, and New York Fed reference rates, with unavailable values kept null and stale freshness explicit zero. It performs no publication, dispatch, policy mutation, browser, provider, credential, live fetch, or live-metric action. Configuration remains `UNCALIBRATED_FOUNDATION`, and the accepted `v1.0` product baseline remains unchanged.
