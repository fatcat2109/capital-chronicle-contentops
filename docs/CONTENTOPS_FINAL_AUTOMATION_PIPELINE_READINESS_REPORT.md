...
The current backend authority is `TASK_CONTENTOPS_GENERIC_FOUNDATION_V2_AUTHORITY_AND_EVIDENCE_INTEGRITY_REPAIR`, classified `PASS_GENERIC_FOUNDATION_V2_AUTHORITY_AND_EVIDENCE_INTEGRITY_REPAIR_AWAITING_CHATGPT_AUDIT`. It retains the enforcement architecture while deriving reserved canonical gates only from governed candidate fields, verifying declared evidence counts against deduplicated evidence lineage, qualifying governed evidence explicitly, and separating published identity matches from real governed deltas. It performs no publication, dispatch, policy mutation, browser, provider, credential, or live-metric action. Configuration remains `UNCALIBRATED_FOUNDATION`, and the accepted `v1.0` product baseline remains unchanged.
