# V6 Discord Supervised Live Pilot Materialization Gate - Technical Contract

## Purpose

The Discord supervised live pilot materialization gate consumes a valid explicit Discord live scope contract packet and an operator supervised live pilot materialization declaration to emit a deterministic Discord supervised live pilot materialization packet.

## Core Rules

1. **Create Local Discord Supervised Live Pilot Materialization Packet Only**: This contract creates a local Discord supervised live pilot materialization packet only.
...
10. **Banned States**:
    ...
    - `materialized_hidden_retry_allowed` must remain `false`.
    - `materialized_max_request_count` must remain 1.
    - `materialized_max_retries` must remain 0.
    - `materialized_package_non_executable` must remain `true`.
    - `materialized_contains_webhook_url` must remain `false`.
    - `materialized_contains_webhook_token` must remain `false`.
    - `materialized_contains_endpoint_url` must remain `false`.
    - `materialized_contains_channel_identity` must remain `false`.
    - `materialized_contains_http_headers` must remain `false`.
    - `materialized_contains_http_body` must remain `false`.
    - `materialized_contains_curl_command` must remain `false`.
    - `materialized_contains_browser_instruction` must remain `false`.
