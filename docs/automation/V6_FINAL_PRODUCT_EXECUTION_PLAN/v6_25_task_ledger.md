...
| 10 | Approval/outbox/audit integration | complete_fixture_only | Payload hash, approval, outbox, bridge, manual/deferred distribution, and audit artifacts exist across lanes; V5 Command Center now displays deterministic manual audit rows, operator-supplied approve/hold/reject decision packets bound to adapter-built payload hashes, local outbox readiness reconciliation rows, Discord/Telegram local-only bridge rows, and Facebook Page/Threads/Instagram/TikTok/Generic Manual local-only handoff rows. Approval/readiness/bridge/manual-deferred status is local/manual evidence only and does not authorize dispatch. |
