# V6 Explicit Discord Live Scope Contract Gate - Technical Contract

## Purpose

The explicit Discord live scope contract gate consumes a valid Discord supervised live pilot planning packet and an operator Discord live scope contract declaration to emit a deterministic explicit Discord live scope contract packet.

## Core Rules

1. **Create Local Explicit Discord Live Scope Contract Packet Only**: This contract creates a local explicit Discord live scope contract packet only.
...
10. **Banned States**:
    ...
    - `scoped_hidden_retry_allowed` must remain `false`.
    - `scoped_max_request_count` must remain 1.
    - `scoped_max_retries` must remain 0.
