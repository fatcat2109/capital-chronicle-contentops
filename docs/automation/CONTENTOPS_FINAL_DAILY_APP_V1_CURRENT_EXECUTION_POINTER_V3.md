...
1. `AGENTS.md`
2. current authority map + stale manifest
3. current North Star/Master Plan
4. `docs/automation/CONTENTOPS_V1_POST_ACCEPTANCE_ACTIVATION_AUTHORITY_V1.md`
5. `docs/automation/CONTENTOPS_V1_SIMPLE_GEMINI_RUNTIME_RESET_V1.md`
6. `docs/codegraph/V1_CONTEXT.md`
7. exact code/tests/evidence for the active slice.
...
Merge commit `db0befb8ad44f1080c67fcb801e5470ce7852369` records the V1 grant.

Do not route from stale wording that says routine V1 public-write/readback is ungranted or V1 acceptance is pending.
...
The existing `DurablePublicationCoordinator`, durable V1 store, destination registry, canonical Substack-first transports, destination-local readback/reconciliation, and `UNKNOWN_WRITE = STOP RETRY -> READ BACK -> RECONCILE` are accepted reusable foundation.
...
1. Wire the accepted Simple qualified article/native package into the existing `DurablePublicationCoordinator` without a second publisher/store/scheduler/package stack.
2. Make current production composition have one unambiguous routine owner: Simple, not native Desktop/legacy rolling-X.
3. Count strictly reconciled published canonical articles separately from zero-write qualified-article telemetry; a publication failure must not satisfy the live daily-output count.
4. Ensure one-click emergency stop/process ownership covers the canonical Simple scheduler/runtime.
...
1. repository authority/static safety closure and CodeGraph refresh — no public write;
2. Simple -> existing publication coordinator bridge + published-count accounting — no live write required for implementation proof;
3. current-host read-only activation preflight — no public write;
4. one fresh live end-to-end V1 canary under already-granted authority: Simple -> canonical Substack -> exactly eight derivatives -> strict readback/reconciliation;
5. if the canary reconciles cleanly with `UNKNOWN_WRITE=0`, enable the four routine live windows toward 5–8 useful published articles/day.
...
Candidate-level abstention is valid. A below-target live day without an exact hard external blocker is `DEGRADED_DAILY_OUTPUT_DEFICIT`.
