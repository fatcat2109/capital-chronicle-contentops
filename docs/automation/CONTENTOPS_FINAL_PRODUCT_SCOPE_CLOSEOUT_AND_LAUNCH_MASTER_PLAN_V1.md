...
Target base: remote `master` at `c87e338f25922f4d03454ba199139353ca7198ff`
...
A Wave 02 durable operational-store candidate exists outside accepted `master`; it must not be treated as current master authority until independently accepted, reconciled, committed, pushed, and read back.
...
Purpose: make the repository agree on the product boundary, operating modes, accepted baseline, current Wave 02 truth, and owner-approval gate for the next task.
...
- no unauthorized exact next task;
- no runtime change.

Exit: GitHub branch/commit is auditable and Jim accepts the product direction.
...
- audit the existing Wave 02 candidate;
- accept and merge it if it is correct and proportionate;
- otherwise implement the smallest compatible correction;
...
Exit: restart-safe local work state and exact evidence export are available to the newsroom vertical slice.
...
This document does not authorize an implementation task.

The exact next builder task remains:

`OWNER_APPROVAL_REQUIRED_NO_EXACT_NEXT_TASK_AUTHORIZED`

After Jim approves this product direction, ChatGPT must verify current remote refs again and issue one heavy but bounded Antigravity task that delivers the highest-value next capability without reopening the prior horizontal roadmap.
