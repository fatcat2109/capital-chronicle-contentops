"""Tests for ledger-11 next manual gate packet builder."""
...
BUILDER_PATH = REPO_ROOT / "tools/telegram_build_ledger11_next_manual_gate_packet.py"
REMOTE_PACKET_PATH = REPO_ROOT / "docs/automation/0174WS_WT_WU/telegram_ledger11_remote_operator_loop_state_packet.json"


def load_module():
    spec = importlib.util.spec_from_file_location("ledger11_builder", BUILDER_PATH)
...
def built(builder, remote_packet):
    return builder.build_ledger11_next_manual_gate_packet(
        remote_packet, operator_gate_id="fresh_test_gate")


def test_import_has_no_side_effects():
    result = subprocess.run(
        [sys.executable, "-c", "import importlib.util; s=importlib.util.spec_from_file_location('b', r'tools/telegram_build_ledger11_next_manual_gate_packet.py'); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); print('imported')"],
...
def test_source_validation_accepts_committed_packet(builder, remote_packet):
    res = builder.validate_ledger11_remote_loop_state_for_next_gate(remote_packet)
    assert res["ledger11_source_validation_outcome_class"] == builder.LEDGER11_SOURCE_OK


def test_source_validation_blocks_missing_packet(builder):
    res = builder.validate_ledger11_remote_loop_state_for_next_gate(None)
    assert res["ledger11_source_validation_outcome_class"] == builder.LEDGER11_SOURCE_MISSING
...
def test_source_validation_blocks_unreconciled(builder, remote_packet):
    ...
    res = builder.validate_ledger11_remote_loop_state_for_next_gate(packet)
    assert res["ledger11_source_validation_outcome_class"] == builder.LEDGER11_SOURCE_UNRECONCILED
...
def test_source_validation_blocks_wrong_ledger_count(builder, remote_packet):
    ...
    res = builder.validate_ledger11_remote_loop_state_for_next_gate(packet)
    assert res["ledger11_source_validation_outcome_class"] == builder.LEDGER11_SOURCE_WRONG_COUNT
...
def test_source_validation_fail_closes_raw_token_url_destination(builder, remote_packet):
    ...
    res = builder.validate_ledger11_remote_loop_state_for_next_gate(packet)
    assert res["ledger11_source_validation_outcome_class"] == builder.LEDGER11_SOURCE_FAIL_CLOSED


def test_candidate_evidence_uses_sequence_12(builder, remote_packet):
    ev = builder.build_ledger11_next_candidate_evidence(remote_packet)
    assert ev["live_test_sequence"] == 13
...
def test_candidate_evidence_message_checksum_matches_adapter(builder, remote_packet):
    ...
    rendered = builder.adapter.render_telegram_payload(
        approved_text=builder.TEST13_MESSAGE,
...
def test_manual_gate_builder_requires_fresh_operator_gate(builder, remote_packet):
    res = builder.build_ledger11_next_manual_gate_packet(remote_packet, operator_gate_id=None)
...
def test_raw_operator_gate_id_never_serialized(builder, built):
    ...
    assert "ledger11_next_manual_gate_transient_local_gate_v1" not in blob
...
def test_builder_artifact_core_fields(builder, remote_packet):
    packet, result = builder.build_builder_artifact_packet(remote_packet, operator_gate_id="fresh")
    assert packet["source_validation_outcome_class"] == builder.LEDGER11_SOURCE_OK
    assert packet["source_current_ledger_count"] == 11
    ...
    assert packet["live_test_sequence"] == 13
...
def test_builder_artifact_baseline_matches_required_when_head_matches(builder, remote_packet, monkeypatch):
    ...
    assert packet["source_baseline_commit"] == "19c7c24253f8ab948e5034da8ce1739f15c365b8"
...
def test_builder_artifact_blocks_stale_baseline_false_when_head_matches(builder, remote_packet, monkeypatch):
    monkeypatch.setattr(builder, "_git_head", lambda ref="HEAD": "19c7c24253f8ab948e5034da8ce1739f15c365b8")
...
def test_metadata_cleanup_builder_and_manual_gate_baseline_match(builder, remote_packet, monkeypatch):
    ...
    assert builder_packet["source_baseline_commit"] == "19c7c24253f8ab948e5034da8ce1739f15c365b8"
    assert manual["source_baseline_commit"] == "19c7c24253f8ab948e5034da8ce1739f15c365b8"
    assert builder_packet["start_head"] == "19c7c24253f8ab948e5034da8ce1739f15c365b8"
    assert manual["start_head"] == "19c7c24253f8ab948e5034da8ce1739f15c365b8"
...
def test_metadata_cleanup_next_task_not_stale(builder, remote_packet):
    ...
    for value in (packet["next_recommended_task"], manual["next_recommended_task"]):
        ...
        assert "0174WG_WH_WI_TELEGRAM_ledger11_next_manual_gate" not in value
        assert "0174WV_WW_WX" in value
