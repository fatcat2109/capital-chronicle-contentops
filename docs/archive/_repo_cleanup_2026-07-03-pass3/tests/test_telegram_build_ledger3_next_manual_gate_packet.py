...
BUILDER_PATH = REPO_ROOT / "tools/telegram_build_ledger3_next_manual_gate_packet.py"
REMOTE_PACKET_PATH = REPO_ROOT / "docs/automation/0174VL_VM_VN/telegram_remote_operator_loop_state_packet.json"


def load_module():
    spec = importlib.util.spec_from_file_location("ledger3_builder", BUILDER_PATH)
...
def built(builder, remote_packet):
    return builder.build_ledger3_next_manual_gate_packet(
        remote_packet, operator_gate_id="fresh_test_gate")


def test_import_has_no_side_effects():
    result = subprocess.run(
        [sys.executable, "-c", "import importlib.util; s=importlib.util.spec_from_file_location('b', r'tools/telegram_build_ledger3_next_manual_gate_packet.py'); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); print('imported')"],
...
def test_source_validation_accepts_committed_packet(builder, remote_packet):
    res = builder.validate_remote_loop_state_for_next_gate(remote_packet)
    assert res["ledger3_source_validation_outcome_class"] == builder.LEDGER3_SOURCE_OK


def test_source_validation_blocks_missing_packet(builder):
    res = builder.validate_remote_loop_state_for_next_gate(None)
    assert res["ledger3_source_validation_outcome_class"] == builder.LEDGER3_SOURCE_MISSING
...
def test_source_validation_blocks_unreconciled(builder, remote_packet):
    ...
    res = builder.validate_remote_loop_state_for_next_gate(packet)
    assert res["ledger3_source_validation_outcome_class"] == builder.LEDGER3_SOURCE_UNRECONCILED


def test_source_validation_blocks_wrong_ledger_count(builder, remote_packet):
    packet = copy.deepcopy(remote_packet)
    packet["current_ledger_count"] = 2
    res = builder.validate_remote_loop_state_for_next_gate(packet)
    assert res["ledger3_source_validation_outcome_class"] == builder.LEDGER3_SOURCE_WRONG_COUNT
...
def test_source_validation_fail_closes_raw_token_url_destination(builder, remote_packet):
    ...
    res = builder.validate_remote_loop_state_for_next_gate(packet)
    assert res["ledger3_source_validation_outcome_class"] == builder.LEDGER3_SOURCE_FAIL_CLOSED


def test_candidate_evidence_uses_sequence_5(builder, remote_packet):
    ev = builder.build_ledger3_next_candidate_evidence(remote_packet)
    assert ev["live_test_sequence"] == 5
...
def test_candidate_evidence_message_checksum_matches_adapter(builder, remote_packet):
    ...
    rendered = builder.adapter.render_telegram_payload(
        approved_text=builder.TEST5_MESSAGE,
...
def test_next_gate_precheck_clears_new_payload_with_fresh_gate(builder, remote_packet):
    ...
    assert pre["next_gate_outcome_class"] == builder.loop.NEXT_CLEAR


def test_exact_replay_example_still_blocks(builder, remote_packet):
    pre = remote_packet["next_gate_examples"]["exact_replay"]
    assert pre["next_gate_outcome_class"] == builder.loop.NEXT_BLOCKED_EXACT
...
def test_same_payload_without_gate_still_requires_fresh_gate(builder, remote_packet):
    ...
    assert pre["next_gate_outcome_class"] == builder.loop.NEXT_REQUIRES_FRESH_GATE


def test_manual_gate_builder_requires_fresh_operator_gate(builder, remote_packet):
    res = builder.build_ledger3_next_manual_gate_packet(remote_packet, operator_gate_id=None)
...
def test_raw_operator_gate_id_never_serialized(builder, built):
    ...
    assert "ledger3_next_manual_gate_transient_local_gate_v1" not in blob
...
def test_builder_artifact_core_fields(builder, remote_packet):
    packet, result = builder.build_builder_artifact_packet(remote_packet, operator_gate_id="fresh")
    assert packet["source_validation_outcome_class"] == builder.LEDGER3_SOURCE_OK
    assert packet["source_current_ledger_count"] == 3
    assert packet["next_gate_precheck_outcome_class"] == builder.loop.NEXT_CLEAR
    assert packet["manual_gate_packet_checksum"] == result["manual_gate_packet"]["manual_gate_packet_checksum"]
    assert packet["live_test_sequence"] == 5
