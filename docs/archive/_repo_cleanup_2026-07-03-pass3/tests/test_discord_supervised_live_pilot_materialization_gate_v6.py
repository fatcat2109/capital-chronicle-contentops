...
from live_contentops.discord_supervised_live_pilot_materialization_gate_v6 import (
    make_discord_supervised_live_pilot_materialization_packet,
    main,
)
...
def _preflight_packet():
    return {
        ...
        "explicit_discord_live_scope_contract_declaration_id": "explicit_discord_live_scope_contract_decl_abc123",
        "operator_id": "jim",
        "created_at_manual": "2026-06-30T05:40:00+07:00",
        ...
        "scoped_payload_hash": "a4d3b5b6307374eeaa27f6cfb08c903823790e668b5a1532822a16d8cfb08cb0",
        "scoped_request_budget": "single_supervised_request_future_scope",
        "scoped_max_request_count": 1,
        "scoped_timeout_seconds": 15,
        ...
        "explicit_discord_live_scope_contract_gate_available": True,
        "explicit_discord_live_scope_contract_gate_declared_ready": True,
        "eligible_for_supervised_live_pilot_materialization_gate": True,
...
def _declaration():
    return {
        ...
        "combined_payload_hash": "combined_sha256_xyz",
        "platform": "discord",
        "materialization_mode": "operator_declared_discord_supervised_live_pilot_materialization_only_not_live",
        ...
        "declaration_decision": "mark_discord_supervised_live_pilot_materialization_ready",
        "approval_phrase": "MARK_DISCORD_SUPERVISED_LIVE_PILOT_MATERIALIZATION_READY_ONLY_NOT_LIVE",
        "approval_scope": "discord_supervised_live_pilot_materialization_gate_only",
        "notes": "Verified Discord supervised live pilot materialization."
    }
...
def test_valid_inputs_emits_eligible_packet():
    packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), _declaration())
    assert packet.discord_supervised_live_pilot_materialization_gate_available is True
    assert packet.discord_supervised_live_pilot_materialization_gate_declared_ready is True
    assert packet.eligible_for_exact_operator_live_dispatch_approval_gate is True
    ...
    assert packet.next_exact_operator_live_dispatch_approval_required is True
    assert not packet.blockers
    _assert_no_public_state(packet)


def test_materialized_payload_hash_mismatch_fails_closed():
    decl = _declaration()
    decl["materialized_payload_hash"] = "wrong"
    packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False
    assert "declaration_materialized_payload_hash_mismatch" in packet.blockers


def test_materialized_credential_key_mismatch_fails_closed():
    decl = _declaration()
    decl["materialized_credential_key_name"] = "wrong"
    packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False
    assert "declaration_materialized_credential_key_name_invalid" in packet.blockers


def test_materialized_action_class_family_mismatch_fails_closed():
    for k, v in [
        ("materialized_action_class", "wrong"),
        ("materialized_platform_family", "wrong"),
        ("materialized_endpoint_family_label", "wrong")
    ]:
        decl = _declaration()
        decl[k] = v
        packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
...
def test_materialized_package_non_executable_false_fails_closed():
    decl = _declaration()
    decl["materialized_package_non_executable"] = False
    packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False


def test_materialized_contains_webhook_url_token_endpoint_channel_header_body_curl_browser_true_fails_closed():
    for f in [
...
def test_materialized_max_request_count_not_1_fails_closed():
    decl = _declaration()
    decl["materialized_max_request_count"] = 2
    packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False


def test_materialized_timeout_seconds_mismatch_or_out_of_bounds_fails_closed():
    decl1 = _declaration()
    decl1["materialized_timeout_seconds"] = 16  # mismatch with preflight 15
    packet1 = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl1)
    assert packet1.discord_supervised_live_pilot_materialization_gate_available is False

    decl2 = _declaration()
    decl2["materialized_timeout_seconds"] = 4  # out of bounds
    preflight = _preflight_packet()
    preflight["scoped_timeout_seconds"] = 4
    packet2 = make_discord_supervised_live_pilot_materialization_packet(preflight, decl2)
    assert packet2.discord_supervised_live_pilot_materialization_gate_available is False


def test_materialized_max_retries_not_0_or_materialized_hidden_retry_true_fails_closed():
    for k, v in [("materialized_max_retries", 1), ("materialized_hidden_retry_allowed", True)]:
        decl = _declaration()
        decl[k] = v
        packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
        assert packet.discord_supervised_live_pilot_materialization_gate_available is False
...
def test_any_materialized_required_boolean_false_fails_closed():
    for f in [
        "materialized_idempotency_required", "materialized_kill_switch_required", "materialized_audit_redaction_required",
        "materialized_manual_fallback_required", "materialized_permission_probe_required_later",
        "materialized_payload_hash_revalidation_required", "materialized_exact_operator_approval_required_later",
        "materialized_destination_binding_required_later", "materialized_request_artifact_creation_required_later"
    ]:
        decl = _declaration()
        decl[f] = False
        packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
...
def test_wrong_preflight_task_label_fails_closed():
    ...
    packet = make_discord_supervised_live_pilot_materialization_packet(preflight, _declaration())
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False


def test_preflight_unavailable_or_not_ready_fails_closed():
    for f in ["explicit_discord_live_scope_contract_gate_available", "explicit_discord_live_scope_contract_gate_declared_ready"]:
        preflight = _preflight_packet()
        preflight[f] = False
        packet = make_discord_supervised_live_pilot_materialization_packet(preflight, _declaration())
        assert packet.discord_supervised_live_pilot_materialization_gate_available is False
...
def test_operator_materialization_decision_reject_or_defer_fails_closed():
    for dec in ["reject", "defer"]:
        decl = _declaration()
        decl["operator_materialization_decision"] = dec
        packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
        assert packet.discord_supervised_live_pilot_materialization_gate_available is False
...
def test_materialization_mode_mismatch_fails_closed():
    decl = _declaration()
    decl["materialization_mode"] = "wrong"
    packet = make_discord_supervised_live_pilot_materialization_packet(_preflight_packet(), decl)
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False
...
def test_secret_marker_in_inputs_fails_closed_without_hash_persistence():
    preflight = _preflight_packet()
    preflight["explicit_discord_live_scope_contract_gate_id"] = "private_key = my_secret"
    packet = make_discord_supervised_live_pilot_materialization_packet(preflight, _declaration())
    assert packet.discord_supervised_live_pilot_materialization_gate_available is False
    assert packet.materialized_payload_hash == ""


def test_module_contains_no_os_or_env_reads_and_no_dotenv():
    source = Path("live_contentops/discord_supervised_live_pilot_materialization_gate_v6.py").read_text(encoding="utf-8")
    forbidden = [
        r"\bimport\s+os\b",
        r"\benviron\b",
        r"\bgetenv\b",
...
def test_cli_writes_deterministic_packet(tmp_path):
    ...
    output_file = tmp_path / "materialization_packet.json"
    assert main([
        str(p_path),
        str(b_path),
    ...
    assert first["discord_supervised_live_pilot_materialization_gate_id"] == second["discord_supervised_live_pilot_materialization_gate_id"]
    assert first["discord_supervised_live_pilot_materialization_gate_available"] is True


def test_new_files_and_sample_json_are_utf8_without_bom():
    docs_dir = Path("docs/automation/V6_DISCORD_SUPERVISED_LIVE_PILOT_MATERIALIZATION_GATE_FROM_EXPLICIT_LIVE_SCOPE_CONTRACT")
    paths = [
        Path("live_contentops/discord_supervised_live_pilot_materialization_gate_v6.py"),
        Path("tests/test_discord_supervised_live_pilot_materialization_gate_v6.py"),
    ]
    paths.extend(path for path in docs_dir.glob("*") if path.is_file())
    for path in paths:
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf")
    loaded = json.loads((docs_dir / "sample_discord_supervised_live_pilot_materialization_gate_packet.json").read_text(encoding="utf-8"))
...
def test_malformed_non_object_cli_blocked_test(tmp_path):
    ...
    assert written["discord_supervised_live_pilot_materialization_gate_available"] is False
    assert written["discord_supervised_live_pilot_materialization_gate_declared_ready"] is False
    assert written["eligible_for_exact_operator_live_dispatch_approval_gate"] is False
