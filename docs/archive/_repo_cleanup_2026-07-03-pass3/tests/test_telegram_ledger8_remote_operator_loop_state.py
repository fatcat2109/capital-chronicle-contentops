...
MODULE_PATH = REPO_ROOT / "live_contentops" / "telegram_ledger8_remote_operator_loop_state.py"
NINTH_PROOF_PATH = REPO_ROOT / "docs/automation/0174WJ_WK_WL/telegram_ledger7_ninth_send_proof_packet.json"
MANUAL_GATE_PATH = REPO_ROOT / "docs/automation/0174WJ_WK_WL/telegram_ledger7_next_manual_gate_packet.json"
PREVIOUS_LOOP_PATH = REPO_ROOT / "docs/automation/0174WG_WH_WI/telegram_ledger7_remote_operator_loop_state_packet.json"
...
@pytest.fixture(scope="module")
def proof():
    return json.loads(NINTH_PROOF_PATH.read_text(encoding="utf-8"))
...
def state(ledger7, proof, manual_gate, previous_loop):
    return ledger7.build_ledger8_remote_operator_loop_state(
        proof, manual_gate, previous_loop_state_packet=previous_loop)


def test_import_has_no_side_effects():
    result = subprocess.run(
        [sys.executable, "-c", "import importlib.util; s=importlib.util.spec_from_file_location('m', r'live_contentops/telegram_ledger8_remote_operator_loop_state.py'); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); print('imported')"],
        cwd=str(REPO_ROOT), capture_output=True, text=True)
    assert result.returncode == 0
    assert result.stdout.strip() == "imported"


def test_accept_committed_ninth_proof(ledger7, proof, previous_loop):
    rec = ledger7.reconcile_ninth_send_proof(proof, previous_loop)
    assert rec["reconciliation_outcome_class"] == ledger7.RECONCILE_OK


def test_block_missing_proof(ledger7):
    rec = ledger7.reconcile_ninth_send_proof(None)
...
def test_block_send_not_succeeded(ledger7, proof):
    ...
    rec = ledger7.reconcile_ninth_send_proof(p)
...
def test_state_has_ledger_count_8_and_sequence_9(state):
    assert state["current_ledger_count"] == 8
    assert state["last_successful_send_sequence"] == 9
...
def test_exact_replay_blocks(ledger7, proof, state):
    ...
    pre = ledger7.build_next_gate_precheck_state(
        state, candidate, fresh_operator_gate_id=ledger7.NINTH_GATE_ID)
...
def test_build_artifact_from_repo(ledger7):
    ...
    assert packet["current_ledger_count"] == 8
    assert packet["last_successful_send_sequence"] == 9
