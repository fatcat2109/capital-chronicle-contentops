# V6 Discord Supervised Request Package Staging Gate - Technical Contract

## Purpose

The Discord supervised request package staging gate consumes a valid Discord request policy gate packet and an operator Discord request package staging declaration to emit a deterministic Discord supervised request package staging packet.

## Core Rules

1. **Create Local Discord Supervised Request Package Staging Packet Only**: This contract creates a local Discord supervised request package staging packet only.
...
10. **Banned States**:
    ...
    - `executable_request_artifact_created` must remain `false`.
    - `webhook_url_included` must remain `false`.
    - `webhook_token_included` must remain `false`.
    - `endpoint_url_included` must remain `false`.
    - `channel_identity_included` must remain `false`.
    ...
    - `hidden_retry_allowed` must remain `false`.
    - `max_request_count` must remain 1.
    - `max_retries` must remain 0.
