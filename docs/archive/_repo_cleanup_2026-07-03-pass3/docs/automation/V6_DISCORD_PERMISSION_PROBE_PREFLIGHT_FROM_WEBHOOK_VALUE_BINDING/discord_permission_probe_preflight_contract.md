# V6 Discord Permission Probe Preflight Gate - Technical Contract

## Purpose

The Discord permission probe preflight gate consumes a valid Discord webhook value binding preflight packet and an operator Discord permission-probe declaration to emit a deterministic Discord permission probe preflight packet.

## Core Rules

1. **Create Local Discord Permission Probe Preflight Packet Only**: This contract creates a local Discord permission probe preflight packet only.
...
10. **Banned States**:
    ...
    - `webhook_url_shape_validation_allowed` must remain `false`.
