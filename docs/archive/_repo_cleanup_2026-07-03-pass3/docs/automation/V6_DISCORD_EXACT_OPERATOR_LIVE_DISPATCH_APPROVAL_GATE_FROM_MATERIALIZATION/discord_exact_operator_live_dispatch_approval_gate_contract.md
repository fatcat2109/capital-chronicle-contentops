# V6 Discord Exact Operator Live Dispatch Approval Gate - Technical Contract

## Purpose

The Discord exact operator live dispatch approval gate consumes a valid Discord supervised live pilot materialization packet and an operator exact live dispatch approval declaration to emit a deterministic Discord exact operator live dispatch approval packet.

## Core Rules

1. **Create Local Discord Exact Operator Live Dispatch Approval Packet Only**: This contract creates a local Discord exact operator live dispatch approval packet only.
...
10. **Banned States**:
    ...
    - `approved_hidden_retry_allowed` must remain `false`.
    - `approved_max_request_count` must remain 1.
    - `approved_max_retries` must remain 0.
    - `approved_package_non_executable` must remain `true`.
    - `approved_contains_webhook_url` must remain `false`.
    - `approved_contains_webhook_token` must remain `false`.
    - `approved_contains_endpoint_url` must remain `false`.
    - `approved_contains_channel_identity` must remain `false`.
    - `approved_contains_http_headers` must remain `false`.
    - `approved_contains_http_body` must remain `false`.
    - `approved_contains_curl_command` must remain `false`.
    - `approved_contains_browser_instruction` must remain `false`.
