# V6 Discord Dry-Run Payload Gate - Technical Contract

## Purpose

The Discord dry-run payload gate consumes a valid Discord permission probe preflight packet and an operator Discord dry-run payload declaration to emit a deterministic Discord dry-run payload gate packet.

## Core Rules

1. **Create Local Discord Dry-Run Payload Gate Packet Only**: This contract creates a local Discord dry-run payload gate packet only.
...
10. **Banned States**:
    ...
    - `payload_http_request_artifact_created` must remain `false`.
    - `payload_contains_financial_advice` must remain `false`.
    - `payload_contains_signal_service_framing` must remain `false`.
    - `payload_contains_public_url` must remain `false`.
    - `payload_contains_metrics` must remain `false`.
    - `payload_contains_raw_source_urls` must remain `false`.
    - `payload_contains_webhook_url` must remain `false`.
    - `payload_contains_token` must remain `false`.
    - `payload_contains_channel_id` must remain `false`.
    - `payload_contains_account_or_workspace_id` must remain `false`.
