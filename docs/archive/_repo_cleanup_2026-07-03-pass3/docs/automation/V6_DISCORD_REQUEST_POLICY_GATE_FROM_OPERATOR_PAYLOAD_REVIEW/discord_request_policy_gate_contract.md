# V6 Discord Request Policy Gate - Technical Contract

## Purpose

The Discord request policy gate consumes a valid Discord operator payload review gate packet and an operator Discord request policy declaration to emit a deterministic Discord request policy gate packet.

## Core Rules

1. **Create Local Discord Request Policy Gate Packet Only**: This contract creates a local Discord request policy gate packet only.
...
10. **Banned States**:
    ...
    - `request_artifact_creation_allowed` must remain `false`.
