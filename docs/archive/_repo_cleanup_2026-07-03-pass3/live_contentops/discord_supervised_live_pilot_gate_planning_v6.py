"""V6 Discord Supervised Live Pilot Gate Planning."""
...
TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_SUPERVISED_LIVE_PILOT_GATE_PLANNING_FROM_FINAL_MANUAL_REVIEW_V0"
FINAL_REVIEW_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_FINAL_MANUAL_EXECUTION_REVIEW_FROM_REQUEST_PACKAGE_STAGING_V0"
...
class DiscordSupervisedLivePilotGatePlanningPacket:
    schema_version: str
    task_label: str
    discord_supervised_live_pilot_gate_planning_id: str
    discord_supervised_live_pilot_gate_planning_declaration_id: str
    operator_id: str
    ...
    platform: str
    planned_action_class: str
    planned_endpoint_family_label: str
    planned_credential_key_name: str
    planned_payload_hash: str
    planned_request_budget: str
    planned_max_request_count: int
    planned_timeout_seconds: int
    planned_max_retries: int
    planned_hidden_retry_allowed: bool
    planned_idempotency_required: bool
    planned_kill_switch_required: bool
    planned_audit_redaction_required: bool
    planned_manual_fallback_required: bool
    planned_permission_probe_required_later: bool
    planned_payload_hash_revalidation_required: bool
    planned_exact_operator_approval_required_later: bool
    operator_live_pilot_planning_decision: str
    ...
    discord_supervised_live_pilot_gate_planning_available: bool
    discord_supervised_live_pilot_gate_planning_declared_ready: bool
    eligible_for_future_explicit_discord_live_scope_contract_gate: bool
    eligible_for_full_live_dispatch_endpoint_mapping_gate: bool
    next_explicit_live_scope_required: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != FINAL_REVIEW_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("discord_final_manual_execution_review_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("discord_final_manual_execution_review_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_discord_supervised_live_pilot_gate_planning") is not True:
        blockers.append("preflight_eligible_for_discord_supervised_live_pilot_gate_planning_invalid")
    ...
    t_sec = packet.get("reviewed_timeout_seconds")
    if not isinstance(t_sec, int) or isinstance(t_sec, bool) or t_sec < 5 or t_sec > 30:
        blockers.append("preflight_reviewed_timeout_seconds_invalid")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "platform",
        "planning_mode",
        "planned_action_class",
        "planned_endpoint_family_label",
        "planned_credential_key_name",
        "planned_payload_hash",
        "planned_request_budget",
        "planned_max_request_count",
        "planned_timeout_seconds",
        "planned_max_retries",
        "planned_hidden_retry_allowed",
        "planned_idempotency_required",
        "planned_kill_switch_required",
        "planned_audit_redaction_required",
        "planned_manual_fallback_required",
        "planned_permission_probe_required_later",
        "planned_payload_hash_revalidation_required",
        "planned_exact_operator_approval_required_later",
        ...
        "public_url_included",
        "metrics_included",
        "operator_live_pilot_planning_decision",
        "next_explicit_live_scope_required",
    ...
    if decl.get("planning_mode") != "operator_declared_discord_supervised_live_pilot_gate_planning_only_not_live":
        blockers.append("declaration_planning_mode_invalid")

    if decl.get("platform") != "discord":
        blockers.append("declaration_platform_invalid")

    if decl.get("planned_action_class") != "future_supervised_discord_webhook_send_pilot_not_now":
        blockers.append("declaration_planned_action_class_invalid")

    if decl.get("planned_endpoint_family_label") != "discord_execute_webhook_label_only":
        blockers.append("declaration_planned_endpoint_family_label_invalid")

    if decl.get("planned_credential_key_name") != "DISCORD_LIVE_ANNOUNCEMENTS_WEBHOOK":
        blockers.append("declaration_planned_credential_key_name_invalid")

    if decl.get("planned_payload_hash") != preflight.get("reviewed_payload_hash"):
        blockers.append("declaration_planned_payload_hash_mismatch")

    if decl.get("planned_request_budget") != "single_supervised_request_future_scope":
        blockers.append("declaration_planned_request_budget_invalid")

    if decl.get("planned_max_request_count") != preflight.get("reviewed_max_request_count"):
        blockers.append("declaration_planned_max_request_count_mismatch")

    if decl.get("planned_max_request_count") != 1:
        blockers.append("declaration_planned_max_request_count_invalid")

    if decl.get("planned_timeout_seconds") != preflight.get("reviewed_timeout_seconds"):
        blockers.append("declaration_planned_timeout_seconds_mismatch")

    t_sec = decl.get("planned_timeout_seconds")
    if not isinstance(t_sec, int) or isinstance(t_sec, bool) or t_sec < 5 or t_sec > 30:
        blockers.append("declaration_planned_timeout_seconds_invalid")

    if decl.get("planned_max_retries") != 0:
        blockers.append("declaration_planned_max_retries_invalid")

    if decl.get("planned_hidden_retry_allowed") is not False:
        blockers.append("declaration_planned_hidden_retry_allowed_not_false")

    # Planned required confirmations must be True
    for f in [
        "planned_idempotency_required",
        "planned_kill_switch_required",
        "planned_audit_redaction_required",
    ...
    pilot_decision = decl.get("operator_live_pilot_planning_decision")
    if pilot_decision not in ["approve_live_pilot_plan_for_future_explicit_live_scope_only", "reject", "defer"]:
        blockers.append("declaration_operator_live_pilot_planning_decision_invalid")
    elif pilot_decision in ["reject", "defer"]:
        blockers.append(f"declaration_operator_live_pilot_planning_rejected_or_deferred_{pilot_decision}")

    if decl.get("next_explicit_live_scope_required") is not True:
        blockers.append("declaration_next_explicit_live_scope_required_not_true")

    decision = decl.get("declaration_decision")
    if decision not in ["mark_discord_supervised_live_pilot_gate_planning_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_supervised_live_pilot_gate_planning_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_SUPERVISED_LIVE_PILOT_GATE_PLANNING_READY_ONLY_NOT_LIVE":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_supervised_live_pilot_gate_planning_only":
...
def make_discord_supervised_live_pilot_gate_planning_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordSupervisedLivePilotGatePlanningPacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_final_manual_execution_review_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_supervised_live_pilot_planning_declaration_json")
    ...
    planned_payload_hash = ""
    planned_request_budget = ""
    planned_max_request_count = 0
    planned_timeout_seconds = 0
    planned_max_retries = 0
    planned_hidden_retry_allowed = False
    planned_idempotency_required = False
    planned_kill_switch_required = False
    planned_audit_redaction_required = False
    planned_manual_fallback_required = False
    planned_permission_probe_required_later = False
    planned_payload_hash_revalidation_required = False
    planned_exact_operator_approval_required_later = False
    operator_live_pilot_planning_decision = ""
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_supervised_live_pilot_gate_planning_declaration_id = str(operator_declaration.get("discord_supervised_live_pilot_gate_planning_declaration_id") or "")
        ...
        planned_action_class = str(operator_declaration.get("planned_action_class") or "")
        planned_endpoint_family_label = str(operator_declaration.get("planned_endpoint_family_label") or "")
        planned_credential_key_name = str(operator_declaration.get("planned_credential_key_name") or "")
        planned_payload_hash = str(operator_declaration.get("planned_payload_hash") or "")
        planned_request_budget = str(operator_declaration.get("planned_request_budget") or "")
        planned_max_request_count = int(operator_declaration.get("planned_max_request_count") or 0)
        planned_timeout_seconds = int(operator_declaration.get("planned_timeout_seconds") or 0)
        planned_max_retries = int(operator_declaration.get("planned_max_retries") or 0)
        planned_hidden_retry_allowed = bool(operator_declaration.get("planned_hidden_retry_allowed"))
        planned_idempotency_required = bool(operator_declaration.get("planned_idempotency_required"))
        planned_kill_switch_required = bool(operator_declaration.get("planned_kill_switch_required"))
        planned_audit_redaction_required = bool(operator_declaration.get("planned_audit_redaction_required"))
        planned_manual_fallback_required = bool(operator_declaration.get("planned_manual_fallback_required"))
        planned_permission_probe_required_later = bool(operator_declaration.get("planned_permission_probe_required_later"))
        planned_payload_hash_revalidation_required = bool(operator_declaration.get("planned_payload_hash_revalidation_required"))
        planned_exact_operator_approval_required_later = bool(operator_declaration.get("planned_exact_operator_approval_required_later"))
        operator_live_pilot_planning_decision = str(operator_declaration.get("operator_live_pilot_planning_decision") or "")
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_supervised_live_pilot_gate_planning_ready")
    )

    eligible_contract = False
    if (
        prepared
        and declared_ready
        and planned_payload_hash == preflight_packet.get("reviewed_payload_hash")
        and planned_max_request_count == 1
        and planned_max_retries == 0
        and not planned_hidden_retry_allowed
        and planned_idempotency_required
        and planned_kill_switch_required
        and planned_audit_redaction_required
        and planned_manual_fallback_required
        and planned_permission_probe_required_later
        and planned_payload_hash_revalidation_required
        and planned_exact_operator_approval_required_later
        and operator_live_pilot_planning_decision == "approve_live_pilot_plan_for_future_explicit_live_scope_only"
    ):
        eligible_contract = True

    # Safety redaction on secret/marker detection
    has_secrets = any("secret_marker_detected" in b for b in blockers)

    if has_secrets:
        discord_supervised_live_pilot_gate_planning_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_supervised_live_pilot_gate_planning_declaration_id": discord_supervised_live_pilot_gate_planning_declaration_id,
        "blockers": blockers,
    }
    discord_supervised_live_pilot_gate_planning_id = f"discord_supervised_live_pilot_gate_planning_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_supervised_live_pilot_gate_planning_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordSupervisedLivePilotGatePlanningPacket(
        ...
        platform=platform,
        planned_action_class=planned_action_class,
        planned_endpoint_family_label=planned_endpoint_family_label,
        planned_credential_key_name=planned_credential_key_name,
        planned_payload_hash=planned_payload_hash,
        planned_request_budget=planned_request_budget,
        planned_max_request_count=planned_max_request_count,
        planned_timeout_seconds=planned_timeout_seconds,
        planned_max_retries=planned_max_retries,
        planned_hidden_retry_allowed=planned_hidden_retry_allowed,
        planned_idempotency_required=planned_idempotency_required,
        planned_kill_switch_required=planned_kill_switch_required,
        planned_audit_redaction_required=planned_audit_redaction_required,
        planned_manual_fallback_required=planned_manual_fallback_required,
        planned_permission_probe_required_later=planned_permission_probe_required_later,
        planned_payload_hash_revalidation_required=planned_payload_hash_revalidation_required,
        planned_exact_operator_approval_required_later=planned_exact_operator_approval_required_later,
        operator_live_pilot_planning_decision=operator_live_pilot_planning_decision,
        ...
        discord_supervised_live_pilot_gate_planning_available=prepared,
        discord_supervised_live_pilot_gate_planning_declared_ready=declared_ready,
        eligible_for_future_explicit_discord_live_scope_contract_gate=eligible_contract,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Supervised Live Pilot Planning CLI")
    parser.add_argument("preflight_packet", help="Path to Discord final manual execution review packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator live pilot planning declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write live pilot planning packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_final_manual_execution_review_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_supervised_live_pilot_planning_declaration_json")

        packet = make_discord_supervised_live_pilot_gate_planning_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.discord_supervised_live_pilot_gate_planning_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordSupervisedLivePilotGatePlanningPacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_supervised_live_pilot_gate_planning_id=f"discord_supervised_live_pilot_gate_planning_blocked_{str(val_err)}",
            discord_supervised_live_pilot_gate_planning_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_final_manual_execution_review_id="",
            ...
            public_url_included=False,
            metrics_included=False,
            discord_supervised_live_pilot_gate_planning_available=False,
            discord_supervised_live_pilot_gate_planning_declared_ready=False,
            eligible_for_future_explicit_discord_live_scope_contract_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            next_explicit_live_scope_required=True,
            substack_manual_fallback_required=True,
            all_platforms_endpoint_mapping_ready=False,
            blockers=[str(val_err)],
            warnings=["discord_supervised_live_pilot_gate_planning_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
