"""Ledger-8 Telegram remote operator loop state after ninth send.

Task 0174WJ/WK/WL. Promotes accepted ninth-send proof into local-only
...
TASK_LABEL = (
    "TASK_CONTENTOPS_0174WJ_WK_WL_TELEGRAM_LEDGER8_REMOTE_OPERATOR_LOOP_"
    "RECONCILIATION_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER8_REMOTE_OPERATOR_LOOP_STATE_0174WJ_WK_WL"
MODEL_VERSION = "0174WJ_WK_WL_TELEGRAM_LEDGER8_REMOTE_OPERATOR_LOOP_STATE_V1"
SOURCE_BASELINE_COMMIT = "718f5bd7a03729f6bfab2af40d26ee8cd7b6965c"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WJ_WK_WL")
PACKET_FILENAME = "telegram_ledger8_remote_operator_loop_state_packet.json"
DOC_FILENAME = "telegram_ledger8_remote_operator_loop_state.md"
NINTH_PROOF_REL = os.path.join(
    "docs", "automation", "0174WJ_WK_WL", "telegram_ledger7_ninth_send_proof_packet.json")
MANUAL_GATE_REL = os.path.join(
    "docs", "automation", "0174WJ_WK_WL", "telegram_ledger7_next_manual_gate_packet.json")
PREVIOUS_LOOP_REL = os.path.join(
    "docs", "automation", "0174WG_WH_WI", "telegram_ledger7_remote_operator_loop_state_packet.json")

RECONCILE_OK = "ledger8_reconciliation_ok_ledger_advanced_to_8"
RECONCILE_MISSING = "ledger8_reconciliation_blocked_missing_ninth_send_proof"
RECONCILE_SEND_NOT_SUCCEEDED = "ledger8_reconciliation_blocked_send_not_succeeded"
RECONCILE_GATE_NOT_REVALIDATED = "ledger8_reconciliation_blocked_manual_gate_not_revalidated"
RECONCILE_LEDGER_NOT_ADVANCED = "ledger8_reconciliation_blocked_ledger_not_advanced"
RECONCILE_MANIFEST_NOT_ADVANCED = "ledger8_reconciliation_blocked_manifest_not_advanced"
RECONCILE_FAIL_CLOSED = "ledger8_reconciliation_fail_closed_forbidden_value"
TRUTH_CLASS = "remote_loop_truth_ledger_count_8_test9_accepted"
...
NINTH_GATE_ID = "ledger7_next_manual_gate_transient_local_gate_v1"
...
def _entry_from_proof(proof):
    ...
    entry = ledger.build_ledger_entry(ev, operator_gate_id=NINTH_GATE_ID)
    p = proof or {}
...
def reconcile_ninth_send_proof(ninth_send_proof, previous_loop_state_packet=None):
    p = ninth_send_proof or {}
    blockers = []
    ...
    elif not (p.get("ledger_appended") is True and p.get("ledger_entry_count_before") == 7
              and p.get("ledger_entry_count") == 8 and p.get("request_budget_used") == 1):
    ...
    result = {
        ...
        "ninth_send_proof_checksum": p.get("evidence_checksum") or compute_checksum(p) if p else None,
...
def build_ledger8_remote_operator_loop_state(ninth_send_proof, manual_gate_packet,
                                             previous_loop_state_packet=None):
    proof = ninth_send_proof or {}
    ...
    reconciliation = reconcile_ninth_send_proof(proof, prev_packet)
    reconciled = reconciliation.get("reconciled") is True
    entry = _entry_from_proof(proof) if proof else {}
    audit_refs = {
        "ninth_send_proof_checksum": proof.get("evidence_checksum") or compute_checksum(proof) if proof else None,
...
def _candidate_from_proof(proof, *, sequence=None, response_suffix=None):
    ...
    if response_suffix:
        ev["response_checksum"] = compute_checksum({
            "kind": "ledger8_new_candidate_response_placeholder",
...
def _new_payload_candidate_from_proof(proof):
    ev = _candidate_from_proof(proof, sequence=9, response_suffix="new_payload")
    ev["send_text_checksum"] = compute_checksum({
        "kind": "ledger8_new_payload_example",
...
def build_artifact_packet(ninth_send_proof, manual_gate_packet, previous_loop_state_packet=None):
    proof = ninth_send_proof or {}
    state = build_ledger8_remote_operator_loop_state(
    ...
    same_payload_candidate = _candidate_from_proof(proof, sequence=9, response_suffix="same_payload")
    ...
    examples = {
        ...
        "exact_replay": build_next_gate_precheck_state(
            state, exact_replay_candidate, fresh_operator_gate_id=NINTH_GATE_ID),
        ...
        "same_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, same_payload_candidate, fresh_operator_gate_id="fresh_ledger8_next_gate"),
        "new_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, new_payload_candidate, fresh_operator_gate_id="fresh_ledger8_next_gate"),
    }
    packet = {
        "task_label": TASK_LABEL,
        "model": MODEL,
...
def build_artifact_from_repo(repo_root):
    ...
    ninth = load_packet(os.path.join(root, NINTH_PROOF_REL))
    manual = load_packet(os.path.join(root, MANUAL_GATE_REL))
    previous = load_packet(os.path.join(root, PREVIOUS_LOOP_REL))
    return build_artifact_packet(ninth, manual, previous_loop_state_packet=previous)
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger8 remote loop artifacts: scan found %d violation(s)" % len(violations))
