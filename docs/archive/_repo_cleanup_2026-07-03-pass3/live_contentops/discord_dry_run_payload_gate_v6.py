"""V6 Discord Dry-Run Payload Gate."""
...
PERMISSION_PROBE_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_PERMISSION_PROBE_PREFLIGHT_FROM_WEBHOOK_VALUE_BINDING_V0"
...
class DiscordDryRunPayloadGatePacket:
    schema_version: str
    task_label: str
    discord_dry_run_payload_gate_id: str
    discord_dry_run_payload_declaration_id: str
    operator_id: str
    ...
    payload_text_preview_hash: str
    payload_text_preview_hash_algorithm: str
    payload_text_preview_non_runtime: bool
    payload_preview_stored: str
    payload_preview_redacted: bool
    payload_length_observed: int
    payload_http_request_artifact_created: bool
    discord_dry_run_payload_gate_available: bool
    discord_dry_run_payload_gate_declared_ready: bool
    eligible_for_operator_payload_review_gate: bool
    ...
    operator_payload_review_later_required: bool
...
def _check_packet_safety(packet: dict[str, Any], prefix: str) -> list[str]:
    ...
    for k, v in packet.items():
        ...
        if prefix == "preflight" and k == "notes":
            continue
        if prefix == "declaration" and k == "payload_text_preview":
            continue
        vals_to_check = []
        if isinstance(v, str):
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != PERMISSION_PROBE_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("permission_probe_preflight_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("permission_probe_preflight_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_discord_dry_run_payload_gate") is not True:
        blockers.append("preflight_eligible_for_discord_dry_run_payload_gate_invalid")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "dispatch_request_package_gate_id",
        "combined_payload_hash",
        "dry_run_payload_mode",
        "platform",
        "payload_kind",
        "payload_source_kind",
        "payload_text_preview",
        "payload_text_preview_hash",
        "payload_text_preview_hash_algorithm",
        "payload_text_preview_non_runtime",
        "payload_contains_financial_advice",
        "payload_contains_signal_service_framing",
        "payload_contains_public_url",
        "payload_contains_metrics",
        "payload_contains_raw_source_urls",
        "payload_contains_citations",
        "payload_contains_webhook_url",
        "payload_contains_token",
        "payload_contains_channel_id",
        "payload_contains_account_or_workspace_id",
        "payload_http_request_artifact_created",
        "webhook_value_read_allowed",
        "discord_api_call_allowed",
        "webhook_send_test_allowed",
        "operator_payload_review_later_required",
        "payload_hash_revalidation_later_required",
        "permission_verification_later_required",
    ...
    if decl.get("dry_run_payload_mode") != "operator_declared_discord_dry_run_payload_gate_only_not_send":
        blockers.append("declaration_dry_run_payload_mode_invalid")

    if decl.get("platform") != "discord":
        blockers.append("declaration_platform_invalid")

    if decl.get("payload_kind") != "discord_text_or_embed_preview_non_runtime":
        blockers.append("declaration_payload_kind_invalid")

    if decl.get("payload_source_kind") != "operator_supplied_preview_text_only":
        blockers.append("declaration_payload_source_kind_invalid")
    ...
    if decl.get("payload_text_preview_non_runtime") is not True:
        blockers.append("declaration_payload_text_preview_non_runtime_not_true")

    preview = decl.get("payload_text_preview")
    if not isinstance(preview, str) or not preview:
        blockers.append("declaration_payload_text_preview_empty_or_invalid")
    elif len(preview) > 2000:
        blockers.append("declaration_payload_text_preview_too_long")
    else:
        # Validate hash
        computed_hash = hashlib.sha256(preview.encode("utf-8")).hexdigest()
        if decl.get("payload_text_preview_hash") != computed_hash:
            blockers.append("declaration_payload_text_preview_hash_mismatch")

    if decl.get("payload_text_preview_hash_algorithm") != "sha256":
        blockers.append("declaration_payload_text_preview_hash_algorithm_invalid")
    ...
    if decision not in ["mark_discord_dry_run_payload_gate_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_dry_run_payload_gate_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_DRY_RUN_PAYLOAD_GATE_READY_ONLY_NOT_SEND":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_dry_run_payload_gate_only":
    ...
    # Safety checks on preview text
    if isinstance(preview, str):
        lowered_preview = preview.lower()
        if _has_secret_marker(preview):
            blockers.append("declaration_preview_secret_marker_detected")
        for claim in FAKE_CLAIMS_MARKERS:
            if claim in lowered_preview:
                blockers.append(f"declaration_preview_fake_claim_detected_{claim}")
        for pat in TRADING_ADVICE_RE:
            if pat.search(preview):
                blockers.append("declaration_preview_financial_advice_or_signal_framing_detected")
                break
        forbidden_term = _scan_for_forbidden_live_claims(preview)
        if forbidden_term:
            blockers.append(f"declaration_preview_forbidden_live_claim_detected_{forbidden_term.replace(' ', '_')}")

    return blockers


def make_discord_dry_run_payload_gate_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordDryRunPayloadGatePacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_permission_probe_preflight_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_discord_dry_run_payload_declaration_json")
    ...
    operator_payload_review_later_required = False
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_dry_run_payload_declaration_id = str(operator_declaration.get("discord_dry_run_payload_declaration_id") or "")
        ...
        payload_kind = str(operator_declaration.get("payload_kind") or "")
        payload_source_kind = str(operator_declaration.get("payload_source_kind") or "")
        payload_text_preview_hash = str(operator_declaration.get("payload_text_preview_hash") or "")
        payload_text_preview_hash_algorithm = str(operator_declaration.get("payload_text_preview_hash_algorithm") or "")
        payload_text_preview_non_runtime = bool(operator_declaration.get("payload_text_preview_non_runtime"))

        preview_text = operator_declaration.get("payload_text_preview")
        if isinstance(preview_text, str):
            payload_length_observed = len(preview_text)

        operator_payload_review_later_required = bool(operator_declaration.get("operator_payload_review_later_required"))
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_dry_run_payload_gate_ready")
    )

    eligible_payload_review = False
    if prepared and declared_ready:
        eligible_payload_review = True

    # Safety redaction on secret/marker detection
    has_secrets = any("secret_marker_detected" in b for b in blockers)

    if has_secrets:
        discord_dry_run_payload_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_dry_run_payload_declaration_id": discord_dry_run_payload_declaration_id,
        "blockers": blockers,
    }
    discord_dry_run_payload_gate_id = f"discord_dry_run_payload_gate_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_dry_run_payload_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordDryRunPayloadGatePacket(
        ...
        payload_text_preview_hash=payload_text_preview_hash,
        payload_text_preview_hash_algorithm=payload_text_preview_hash_algorithm,
        payload_text_preview_non_runtime=payload_text_preview_non_runtime,
        payload_preview_stored=payload_preview_stored,
        payload_preview_redacted=payload_preview_redacted,
        payload_length_observed=payload_length_observed,
        payload_http_request_artifact_created=payload_http_request_artifact_created,
        discord_dry_run_payload_gate_available=prepared,
        discord_dry_run_payload_gate_declared_ready=declared_ready,
        eligible_for_operator_payload_review_gate=eligible_payload_review,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
        ...
        operator_payload_review_later_required=operator_payload_review_later_required,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Dry-Run Payload Gate CLI")
    parser.add_argument("preflight_packet", help="Path to Discord permission probe preflight packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator Discord dry-run payload declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write dry-run payload gate packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_permission_probe_preflight_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_discord_dry_run_payload_declaration_json")

        packet = make_discord_dry_run_payload_gate_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.discord_dry_run_payload_gate_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordDryRunPayloadGatePacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_dry_run_payload_gate_id=f"discord_dry_run_payload_gate_blocked_{str(val_err)}",
            discord_dry_run_payload_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_permission_probe_preflight_id="",
            ...
            payload_length_observed=0,
            payload_http_request_artifact_created=False,
            discord_dry_run_payload_gate_available=False,
            discord_dry_run_payload_gate_declared_ready=False,
            eligible_for_operator_payload_review_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            webhook_value_read_allowed=False,
            discord_api_call_allowed=False,
            webhook_send_test_allowed=False,
            ...
            warnings=["discord_dry_run_payload_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
