"""V6 Discord Operator Payload Review Gate."""
...
DRY_RUN_PAYLOAD_TASK_LABEL = "TASK_CONTENTOPS_V6_DISCORD_DRY_RUN_PAYLOAD_GATE_FROM_PERMISSION_PROBE_PREFLIGHT_V0"
...
class DiscordOperatorPayloadReviewGatePacket:
    schema_version: str
    task_label: str
    discord_operator_payload_review_gate_id: str
    discord_operator_payload_review_declaration_id: str
    operator_id: str
    ...
    reviewed_payload_hash_algorithm: str
    reviewed_payload_preview_non_runtime: bool
    operator_payload_review_decision: str
    live_dispatch_approval_granted: bool
    publication_approval_granted: bool
    request_artifact_creation_allowed: bool
    discord_operator_payload_review_gate_available: bool
    discord_operator_payload_review_gate_declared_ready: bool
    eligible_for_discord_request_policy_gate: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != DRY_RUN_PAYLOAD_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("discord_dry_run_payload_gate_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("discord_dry_run_payload_gate_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_operator_payload_review_gate") is not True:
        blockers.append("preflight_eligible_for_operator_payload_review_gate_invalid")
    ...
    if not packet.get("payload_text_preview_hash") or packet.get("payload_text_preview_hash_algorithm") != "sha256":
        blockers.append("preflight_payload_hash_invalid")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "reviewed_payload_hash_algorithm",
        "reviewed_payload_preview_non_runtime",
        "reviewed_payload_contains_financial_advice",
        "reviewed_payload_contains_signal_service_framing",
        "reviewed_payload_contains_public_url",
        "reviewed_payload_contains_metrics",
        "reviewed_payload_contains_webhook_url",
        "reviewed_payload_contains_token",
        "reviewed_payload_contains_channel_id",
        "reviewed_payload_contains_account_or_workspace_id",
        ...
        "operator_payload_review_decision",
    ...
    if decl.get("review_mode") != "operator_declared_discord_payload_review_gate_only_not_send":
        blockers.append("declaration_review_mode_invalid")

    if decl.get("platform") != "discord":
        blockers.append("declaration_platform_invalid")

    if decl.get("reviewed_payload_hash") != preflight.get("payload_text_preview_hash"):
        blockers.append("declaration_reviewed_payload_hash_mismatch")

    if decl.get("reviewed_payload_hash_algorithm") != "sha256":
        blockers.append("declaration_reviewed_payload_hash_algorithm_invalid")

    if decl.get("reviewed_payload_preview_non_runtime") is not True:
        blockers.append("declaration_reviewed_payload_preview_non_runtime_not_true")
    ...
    review_decision = decl.get("operator_payload_review_decision")
    if review_decision not in ["approve_payload_hash_for_next_local_gate_only", "reject", "defer"]:
        blockers.append("declaration_operator_payload_review_decision_invalid")
    elif review_decision in ["reject", "defer"]:
        blockers.append(f"declaration_review_rejected_or_deferred_{review_decision}")

    decision = decl.get("declaration_decision")
    if decision not in ["mark_discord_operator_payload_review_gate_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_operator_payload_review_gate_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_OPERATOR_PAYLOAD_REVIEW_GATE_READY_ONLY_NOT_SEND":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_operator_payload_review_gate_only":
...
def make_discord_operator_payload_review_gate_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordOperatorPayloadReviewGatePacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_discord_dry_run_payload_gate_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_payload_review_declaration_json")
    ...
    reviewed_payload_hash = ""
    reviewed_payload_hash_algorithm = ""
    reviewed_payload_preview_non_runtime = False
    operator_payload_review_decision = ""
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_operator_payload_review_declaration_id = str(operator_declaration.get("discord_operator_payload_review_declaration_id") or "")
        ...
        reviewed_payload_hash_algorithm = str(operator_declaration.get("reviewed_payload_hash_algorithm") or "")
        reviewed_payload_preview_non_runtime = bool(operator_declaration.get("reviewed_payload_preview_non_runtime"))
        operator_payload_review_decision = str(operator_declaration.get("operator_payload_review_decision") or "")
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_operator_payload_review_gate_ready")
    )

    eligible_policy = False
    if prepared and declared_ready and operator_payload_review_decision == "approve_payload_hash_for_next_local_gate_only":
    ...
    if has_secrets:
        discord_operator_payload_review_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_operator_payload_review_declaration_id": discord_operator_payload_review_declaration_id,
        "blockers": blockers,
    }
    discord_operator_payload_review_gate_id = f"discord_operator_payload_review_gate_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_operator_payload_review_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordOperatorPayloadReviewGatePacket(
        ...
        reviewed_payload_hash_algorithm=reviewed_payload_hash_algorithm,
        reviewed_payload_preview_non_runtime=reviewed_payload_preview_non_runtime,
        operator_payload_review_decision=operator_payload_review_decision,
        ...
        discord_operator_payload_review_gate_available=prepared,
        discord_operator_payload_review_gate_declared_ready=declared_ready,
        eligible_for_discord_request_policy_gate=eligible_policy,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Operator Payload Review Gate CLI")
    parser.add_argument("preflight_packet", help="Path to Discord dry-run payload gate packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator payload review declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write operator payload review gate packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_discord_dry_run_payload_gate_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_payload_review_declaration_json")

        packet = make_discord_operator_payload_review_gate_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.discord_operator_payload_review_gate_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordOperatorPayloadReviewGatePacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_operator_payload_review_gate_id=f"discord_operator_payload_review_gate_blocked_{str(val_err)}",
            discord_operator_payload_review_declaration_id="",
            operator_id="",
            created_at_manual="",
            discord_dry_run_payload_gate_id="",
            ...
            publication_approval_granted=False,
            request_artifact_creation_allowed=False,
            discord_operator_payload_review_gate_available=False,
            discord_operator_payload_review_gate_declared_ready=False,
            eligible_for_discord_request_policy_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            webhook_value_read_allowed=False,
            discord_api_call_allowed=False,
            webhook_send_test_allowed=False,
            ...
            warnings=["discord_operator_payload_review_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
