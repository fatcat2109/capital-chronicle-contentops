"""V6 Discord Supervised Live Pilot Materialization Gate."""
...
EXPLICIT_CONTRACT_TASK_LABEL = "TASK_CONTENTOPS_V6_EXPLICIT_DISCORD_LIVE_SCOPE_CONTRACT_GATE_FROM_SUPERVISED_LIVE_PILOT_PLANNING_V0"
...
class DiscordSupervisedLivePilotMaterializationPacket:
    schema_version: str
    task_label: str
    discord_supervised_live_pilot_materialization_gate_id: str
    discord_supervised_live_pilot_materialization_declaration_id: str
    operator_id: str
    ...
    platform: str
    materialized_action_class: str
    materialized_platform_family: str
    materialized_endpoint_family_label: str
    materialized_credential_key_name: str
    materialized_payload_hash: str
    materialized_request_budget: str
    materialized_max_request_count: int
    materialized_timeout_seconds: int
    materialized_max_retries: int
    materialized_hidden_retry_allowed: bool
    materialized_idempotency_required: bool
    materialized_kill_switch_required: bool
    materialized_audit_redaction_required: bool
    materialized_manual_fallback_required: bool
    materialized_permission_probe_required_later: bool
    materialized_payload_hash_revalidation_required: bool
    materialized_exact_operator_approval_required_later: bool
    materialized_destination_binding_required_later: bool
    materialized_request_artifact_creation_required_later: bool
    materialized_package_non_executable: bool
    materialized_contains_webhook_url: bool
    materialized_contains_webhook_token: bool
    materialized_contains_endpoint_url: bool
    materialized_contains_channel_identity: bool
    materialized_contains_http_headers: bool
    materialized_contains_http_body: bool
    materialized_contains_curl_command: bool
    materialized_contains_browser_instruction: bool
    operator_materialization_decision: str
    ...
    discord_supervised_live_pilot_materialization_gate_available: bool
    discord_supervised_live_pilot_materialization_gate_declared_ready: bool
    eligible_for_exact_operator_live_dispatch_approval_gate: bool
    eligible_for_full_live_dispatch_endpoint_mapping_gate: bool
    next_exact_operator_live_dispatch_approval_required: bool
...
def _validate_preflight_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("task_label") != EXPLICIT_CONTRACT_TASK_LABEL:
        blockers.append("preflight_task_label_invalid")

    if packet.get("explicit_discord_live_scope_contract_gate_available") is not True:
        blockers.append("preflight_not_available")

    if packet.get("explicit_discord_live_scope_contract_gate_declared_ready") is not True:
        blockers.append("preflight_not_declared_ready")

    if packet.get("eligible_for_supervised_live_pilot_materialization_gate") is not True:
        blockers.append("preflight_eligible_for_supervised_live_pilot_materialization_gate_invalid")

    gating_rules = {
        "eligible_for_full_live_dispatch_endpoint_mapping_gate": False,
        "all_platforms_endpoint_mapping_ready": False,
        "platform": "discord",
        "scoped_action_class": "supervised_discord_webhook_send_pilot_future_scope_only",
        "scoped_platform_family": "discord_webhook",
        "scoped_endpoint_family_label": "discord_execute_webhook_label_only",
        "scoped_credential_key_name": "DISCORD_LIVE_ANNOUNCEMENTS_WEBHOOK",
        "scoped_request_budget": "single_supervised_request_future_scope",
        "scoped_max_request_count": 1,
        "scoped_max_retries": 0,
        "scoped_hidden_retry_allowed": False,
        "scoped_idempotency_required": True,
        "scoped_kill_switch_required": True,
        "scoped_audit_redaction_required": True,
        "scoped_manual_fallback_required": True,
        "scoped_permission_probe_required_later": True,
        "scoped_payload_hash_revalidation_required": True,
        "scoped_exact_operator_approval_required_later": True,
        "scoped_destination_binding_required_later": True,
        "scoped_request_artifact_creation_required_later": True,
        "operator_live_scope_contract_decision": "approve_explicit_discord_live_scope_contract_for_future_materialization_only",
    ...
    t_sec = packet.get("scoped_timeout_seconds")
    if not isinstance(t_sec, int) or isinstance(t_sec, bool) or t_sec < 5 or t_sec > 30:
        blockers.append("preflight_scoped_timeout_seconds_invalid")
    ...
    if not packet.get("scoped_payload_hash"):
        blockers.append("preflight_scoped_payload_hash_missing")
...
def _validate_declaration(decl: dict[str, Any], preflight: dict[str, Any]) -> list[str]:
    ...
    required_keys = [
        ...
        "public_url_included",
        "metrics_included",
        "operator_materialization_decision",
        "next_exact_operator_live_dispatch_approval_required",
    ...
    if decl.get("materialization_mode") != "operator_declared_discord_supervised_live_pilot_materialization_only_not_live":
        blockers.append("declaration_materialization_mode_invalid")

    if decl.get("platform") != "discord":
        blockers.append("declaration_platform_invalid")

    if decl.get("materialized_action_class") != "supervised_discord_webhook_send_pilot_materialization_future_only":
        blockers.append("declaration_materialized_action_class_invalid")

    if decl.get("materialized_platform_family") != "discord_webhook":
        blockers.append("declaration_materialized_platform_family_invalid")

    if decl.get("materialized_endpoint_family_label") != "discord_execute_webhook_label_only":
        blockers.append("declaration_materialized_endpoint_family_label_invalid")

    if decl.get("materialized_credential_key_name") != "DISCORD_LIVE_ANNOUNCEMENTS_WEBHOOK":
        blockers.append("declaration_materialized_credential_key_name_invalid")

    if decl.get("materialized_payload_hash") != preflight.get("scoped_payload_hash"):
        blockers.append("declaration_materialized_payload_hash_mismatch")

    if decl.get("materialized_request_budget") != "single_supervised_request_future_scope":
        blockers.append("declaration_materialized_request_budget_invalid")

    if decl.get("materialized_max_request_count") != preflight.get("scoped_max_request_count"):
        blockers.append("declaration_materialized_max_request_count_mismatch")

    if decl.get("materialized_max_request_count") != 1:
        blockers.append("declaration_materialized_max_request_count_invalid")

    if decl.get("materialized_timeout_seconds") != preflight.get("scoped_timeout_seconds"):
        blockers.append("declaration_materialized_timeout_seconds_mismatch")

    t_sec = decl.get("materialized_timeout_seconds")
    if not isinstance(t_sec, int) or isinstance(t_sec, bool) or t_sec < 5 or t_sec > 30:
        blockers.append("declaration_materialized_timeout_seconds_invalid")

    if decl.get("materialized_max_retries") != 0:
        blockers.append("declaration_materialized_max_retries_invalid")

    if decl.get("materialized_hidden_retry_allowed") is not False:
        blockers.append("declaration_materialized_hidden_retry_allowed_not_false")

    # Materialized required confirmations must be True
    for f in [
        "materialized_idempotency_required",
        "materialized_kill_switch_required",
        "materialized_audit_redaction_required",
    ...
    if decl.get("materialized_package_non_executable") is not True:
        blockers.append("declaration_materialized_package_non_executable_not_true")

    # Contains check flags must be False
    for f in [
        "materialized_contains_webhook_url",
        "materialized_contains_webhook_token",
        "materialized_contains_endpoint_url",
    ...
    if decl.get("explicit_discord_live_scope_contract_gate_id") != preflight.get("explicit_discord_live_scope_contract_gate_id"):
        blockers.append("declaration_explicit_discord_live_scope_contract_gate_id_mismatch")
    ...
    mat_decision = decl.get("operator_materialization_decision")
    if mat_decision not in ["approve_supervised_live_pilot_materialization_for_future_exact_operator_approval_only", "reject", "defer"]:
        blockers.append("declaration_operator_materialization_decision_invalid")
    elif mat_decision in ["reject", "defer"]:
        blockers.append(f"declaration_operator_materialization_rejected_or_deferred_{mat_decision}")

    if decl.get("next_exact_operator_live_dispatch_approval_required") is not True:
        blockers.append("declaration_next_exact_operator_live_dispatch_approval_required_not_true")

    decision = decl.get("declaration_decision")
    if decision not in ["mark_discord_supervised_live_pilot_materialization_ready", "reject", "defer"]:
    ...
    elif decision == "mark_discord_supervised_live_pilot_materialization_ready":
        if decl.get("approval_phrase") != "MARK_DISCORD_SUPERVISED_LIVE_PILOT_MATERIALIZATION_READY_ONLY_NOT_LIVE":
            blockers.append("declaration_approval_phrase_invalid")
        if decl.get("approval_scope") != "discord_supervised_live_pilot_materialization_gate_only":
...
def make_discord_supervised_live_pilot_materialization_packet(
    preflight_packet: Any,
    operator_declaration: Any
) -> DiscordSupervisedLivePilotMaterializationPacket:
    blockers: list[str] = []

    preflight_is_dict = isinstance(preflight_packet, dict)
    if not preflight_is_dict:
        blockers.append("malformed_explicit_discord_live_scope_contract_packet_json")

    decl_is_dict = isinstance(operator_declaration, dict)
    if not decl_is_dict:
        blockers.append("malformed_operator_supervised_live_pilot_materialization_declaration_json")
    ...
    materialized_payload_hash = ""
    materialized_request_budget = ""
    materialized_max_request_count = 0
    materialized_timeout_seconds = 0
    materialized_max_retries = 0
    materialized_hidden_retry_allowed = False
    materialized_idempotency_required = False
    materialized_kill_switch_required = False
    materialized_audit_redaction_required = False
    materialized_manual_fallback_required = False
    materialized_permission_probe_required_later = False
    materialized_payload_hash_revalidation_required = False
    materialized_exact_operator_approval_required_later = False
    materialized_destination_binding_required_later = False
    materialized_request_artifact_creation_required_later = False
    materialized_package_non_executable = False
    ...
    if preflight_is_dict and decl_is_dict and not any("secret_marker_detected" in b for b in blockers):
        discord_supervised_live_pilot_materialization_declaration_id = str(operator_declaration.get("discord_supervised_live_pilot_materialization_declaration_id") or "")
        ...
        materialized_action_class = str(operator_declaration.get("materialized_action_class") or "")
        materialized_platform_family = str(operator_declaration.get("materialized_platform_family") or "")
        materialized_endpoint_family_label = str(operator_declaration.get("materialized_endpoint_family_label") or "")
        materialized_credential_key_name = str(operator_declaration.get("materialized_credential_key_name") or "")
        materialized_payload_hash = str(operator_declaration.get("materialized_payload_hash") or "")
        materialized_request_budget = str(operator_declaration.get("materialized_request_budget") or "")
        materialized_max_request_count = int(operator_declaration.get("materialized_max_request_count") or 0)
        materialized_timeout_seconds = int(operator_declaration.get("materialized_timeout_seconds") or 0)
        materialized_max_retries = int(operator_declaration.get("materialized_max_retries") or 0)
        materialized_hidden_retry_allowed = bool(operator_declaration.get("materialized_hidden_retry_allowed"))
        materialized_idempotency_required = bool(operator_declaration.get("materialized_idempotency_required"))
        materialized_kill_switch_required = bool(operator_declaration.get("materialized_kill_switch_required"))
        materialized_audit_redaction_required = bool(operator_declaration.get("materialized_audit_redaction_required"))
        materialized_manual_fallback_required = bool(operator_declaration.get("materialized_manual_fallback_required"))
        materialized_permission_probe_required_later = bool(operator_declaration.get("materialized_permission_probe_required_later"))
        materialized_payload_hash_revalidation_required = bool(operator_declaration.get("materialized_payload_hash_revalidation_required"))
        materialized_exact_operator_approval_required_later = bool(operator_declaration.get("materialized_exact_operator_approval_required_later"))
        materialized_destination_binding_required_later = bool(operator_declaration.get("materialized_destination_binding_required_later"))
        materialized_request_artifact_creation_required_later = bool(operator_declaration.get("materialized_request_artifact_creation_required_later"))
        materialized_package_non_executable = bool(operator_declaration.get("materialized_package_non_executable"))
        materialized_contains_webhook_url = bool(operator_declaration.get("materialized_contains_webhook_url"))
        materialized_contains_webhook_token = bool(operator_declaration.get("materialized_contains_webhook_token"))
        materialized_contains_endpoint_url = bool(operator_declaration.get("materialized_contains_endpoint_url"))
        materialized_contains_channel_identity = bool(operator_declaration.get("materialized_contains_channel_identity"))
        materialized_contains_http_headers = bool(operator_declaration.get("materialized_contains_http_headers"))
        materialized_contains_http_body = bool(operator_declaration.get("materialized_contains_http_body"))
        materialized_contains_curl_command = bool(operator_declaration.get("materialized_contains_curl_command"))
        materialized_contains_browser_instruction = bool(operator_declaration.get("materialized_contains_browser_instruction"))
        operator_materialization_decision = str(operator_declaration.get("operator_materialization_decision") or "")
    ...
    declared_ready = (
        prepared and decl_is_dict and (operator_declaration.get("declaration_decision") == "mark_discord_supervised_live_pilot_materialization_ready")
    )

    eligible_approval = False
    if (
        prepared
        and declared_ready
        and materialized_payload_hash == preflight_packet.get("scoped_payload_hash")
        and materialized_max_request_count == 1
        and materialized_max_retries == 0
        and not materialized_hidden_retry_allowed
        and materialized_idempotency_required
        and materialized_kill_switch_required
        and materialized_audit_redaction_required
        and materialized_manual_fallback_required
        and materialized_permission_probe_required_later
        and materialized_payload_hash_revalidation_required
        and materialized_exact_operator_approval_required_later
        and materialized_destination_binding_required_later
        and materialized_request_artifact_creation_required_later
        and materialized_package_non_executable
        and not materialized_contains_webhook_url
        and not materialized_contains_webhook_token
        and not materialized_contains_endpoint_url
        and not materialized_contains_channel_identity
        and not materialized_contains_http_headers
        and not materialized_contains_http_body
        and not materialized_contains_curl_command
        and not materialized_contains_browser_instruction
        and operator_materialization_decision == "approve_supervised_live_pilot_materialization_for_future_exact_operator_approval_only"
    ):
        eligible_approval = True

    # Safety redaction on secret/marker detection
    has_secrets = any("secret_marker_detected" in b for b in blockers)

    if has_secrets:
        discord_supervised_live_pilot_materialization_declaration_id = "[REDACTED_SECRET_MARKER_DETECTED]"
    ...
    intake_material = {
        ...
        "discord_supervised_live_pilot_materialization_declaration_id": discord_supervised_live_pilot_materialization_declaration_id,
        "blockers": blockers,
    }
    discord_supervised_live_pilot_materialization_gate_id = f"discord_supervised_live_pilot_materialization_{hashlib.sha256(_canonical_json(intake_material).encode('utf-8')).hexdigest()[:16]}"

    warnings = []
    if not prepared:
        warnings.append("discord_supervised_live_pilot_materialization_gate_blocked_pending_operator_repair")

    if prepared:
        warnings.append("substack_official_api_docs_unverified")

    return DiscordSupervisedLivePilotMaterializationPacket(
        ...
        platform=platform,
        materialized_action_class=materialized_action_class,
        materialized_platform_family=materialized_platform_family,
        materialized_endpoint_family_label=materialized_endpoint_family_label,
        materialized_credential_key_name=materialized_credential_key_name,
        materialized_payload_hash=materialized_payload_hash,
        materialized_request_budget=materialized_request_budget,
        materialized_max_request_count=materialized_max_request_count,
        materialized_timeout_seconds=materialized_timeout_seconds,
        materialized_max_retries=materialized_max_retries,
        materialized_hidden_retry_allowed=materialized_hidden_retry_allowed,
        materialized_idempotency_required=materialized_idempotency_required,
        materialized_kill_switch_required=materialized_kill_switch_required,
        materialized_audit_redaction_required=materialized_audit_redaction_required,
        materialized_manual_fallback_required=materialized_manual_fallback_required,
        materialized_permission_probe_required_later=materialized_permission_probe_required_later,
        materialized_payload_hash_revalidation_required=materialized_payload_hash_revalidation_required,
        materialized_exact_operator_approval_required_later=materialized_exact_operator_approval_required_later,
        materialized_destination_binding_required_later=materialized_destination_binding_required_later,
        materialized_request_artifact_creation_required_later=materialized_request_artifact_creation_required_later,
        materialized_package_non_executable=materialized_package_non_executable,
        materialized_contains_webhook_url=materialized_contains_webhook_url,
        materialized_contains_webhook_token=materialized_contains_webhook_token,
        materialized_contains_endpoint_url=materialized_contains_endpoint_url,
        materialized_contains_channel_identity=materialized_contains_channel_identity,
        materialized_contains_http_headers=materialized_contains_http_headers,
        materialized_contains_http_body=materialized_contains_http_body,
        materialized_contains_curl_command=materialized_contains_curl_command,
        materialized_contains_browser_instruction=materialized_contains_browser_instruction,
        operator_materialization_decision=operator_materialization_decision,
        ...
        discord_supervised_live_pilot_materialization_gate_available=prepared,
        discord_supervised_live_pilot_materialization_gate_declared_ready=declared_ready,
        eligible_for_exact_operator_live_dispatch_approval_gate=eligible_approval,
        eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="V6 Discord Supervised Live Pilot Materialization CLI")
    parser.add_argument("preflight_packet", help="Path to explicit Discord live scope contract packet JSON")
    parser.add_argument("operator_declaration", help="Path to operator live pilot materialization declaration JSON")
    parser.add_argument("--output-file", required=True, help="Path to write live pilot materialization packet JSON")

    args = parser.parse_args(argv)

    try:
        preflight = load_json_packet(args.preflight_packet, "malformed_explicit_discord_live_scope_contract_packet_json")
        decl = load_json_packet(args.operator_declaration, "malformed_operator_supervised_live_pilot_materialization_declaration_json")

        packet = make_discord_supervised_live_pilot_materialization_packet(preflight, decl)

        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")

        if not packet.discord_supervised_live_pilot_materialization_gate_available:
            return 1
        return 0

    except ValueError as val_err:
        packet = DiscordSupervisedLivePilotMaterializationPacket(
            schema_version=SCHEMA_VERSION,
            task_label=TASK_LABEL,
            discord_supervised_live_pilot_materialization_gate_id=f"discord_supervised_live_pilot_materialization_blocked_{str(val_err)}",
            discord_supervised_live_pilot_materialization_declaration_id="",
            operator_id="",
            created_at_manual="",
            explicit_discord_live_scope_contract_gate_id="",
            ...
            public_url_included=False,
            metrics_included=False,
            discord_supervised_live_pilot_materialization_gate_available=False,
            discord_supervised_live_pilot_materialization_gate_declared_ready=False,
            eligible_for_exact_operator_live_dispatch_approval_gate=False,
            eligible_for_full_live_dispatch_endpoint_mapping_gate=False,
            next_exact_operator_live_dispatch_approval_required=True,
            substack_manual_fallback_required=True,
            all_platforms_endpoint_mapping_ready=False,
            blockers=[str(val_err)],
            warnings=["discord_supervised_live_pilot_materialization_blocked_pending_operator_repair"],
        )
        out_path = Path(args.output_file)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(asdict(packet), indent=2, sort_keys=True), encoding="utf-8")
