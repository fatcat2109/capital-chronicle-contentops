"""Ledger-11 Telegram remote operator loop state after twelfth send.

Task 0174WS/WT/WU. Promotes accepted twelfth-send proof into local-only
...
TASK_LABEL = (
    "TASK_CONTENTOPS_0174WS_WT_WU_TELEGRAM_LEDGER11_REMOTE_OPERATOR_LOOP_"
    "RECONCILIATION_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER11_REMOTE_OPERATOR_LOOP_STATE_0174WS_WT_WU"
MODEL_VERSION = "0174WS_WT_WU_TELEGRAM_LEDGER11_REMOTE_OPERATOR_LOOP_STATE_V1"
SOURCE_BASELINE_COMMIT = "4c3b76b9c9e2eb6495a0638e65253667e0fbe697"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WS_WT_WU")
PACKET_FILENAME = "telegram_ledger11_remote_operator_loop_state_packet.json"
DOC_FILENAME = "telegram_ledger11_remote_operator_loop_state.md"
twelfth_PROOF_REL = os.path.join(
    "docs", "automation", "0174WS_WT_WU", "telegram_ledger10_twelfth_send_proof_packet.json")
MANUAL_GATE_REL = os.path.join(
    "docs", "automation", "0174WS_WT_WU", "telegram_ledger10_next_manual_gate_packet.json")
PREVIOUS_LOOP_REL = os.path.join("docs", "automation", "0174WP_WQ_WR", "telegram_ledger10_remote_operator_loop_state_packet.json")

RECONCILE_OK = "ledger11_reconciliation_ok_ledger_advanced_to_11"
RECONCILE_MISSING = "ledger11_reconciliation_blocked_missing_twelfth_send_proof"
RECONCILE_SEND_NOT_SUCCEEDED = "ledger11_reconciliation_blocked_send_not_succeeded"
RECONCILE_GATE_NOT_REVALIDATED = "ledger11_reconciliation_blocked_manual_gate_not_revalidated"
RECONCILE_LEDGER_NOT_ADVANCED = "ledger11_reconciliation_blocked_ledger_not_advanced"
RECONCILE_MANIFEST_NOT_ADVANCED = "ledger11_reconciliation_blocked_manifest_not_advanced"
RECONCILE_FAIL_CLOSED = "ledger11_reconciliation_fail_closed_forbidden_value"
TRUTH_CLASS = "remote_loop_truth_ledger_count_10_test12_accepted"
NEXT_ALLOWED_ACTION = "prepare_next_manual_gate_candidate"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WS_WT_WU_TELEGRAM_LEDGER11_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
twelfth_GATE_ID = "ledger10_next_manual_gate_transient_local_gate_v1"
...
def _entry_from_proof(proof):
    ...
    entry = ledger.build_ledger_entry(ev, operator_gate_id=twelfth_GATE_ID)
    p = proof or {}
...
def reconcile_twelfth_send_proof(twelfth_send_proof, previous_loop_state_packet=None):
    p = twelfth_send_proof or {}
    blockers = []
    ...
    elif not (p.get("ledger_appended") is True and p.get("ledger_entry_count_before") == 10 and p.get("ledger_entry_count") == 11 and p.get("request_budget_used") == 1):
    ...
    result = {
        ...
        "twelfth_send_proof_checksum": p.get("evidence_checksum") or compute_checksum(p) if p else None,
...
def build_ledger11_remote_operator_loop_state(twelfth_send_proof, manual_gate_packet,
                                             previous_loop_state_packet=None):
    proof = twelfth_send_proof or {}
    ...
    reconciliation = reconcile_twelfth_send_proof(proof, prev_packet)
    reconciled = reconciliation.get("reconciled") is True
    entry = _entry_from_proof(proof) if proof else {}
    audit_refs = {
        "twelfth_send_proof_checksum": proof.get("evidence_checksum") or compute_checksum(proof) if proof else None,
...
def _candidate_from_proof(proof, *, sequence=None, response_suffix=None):
    ...
    if response_suffix:
        ev["response_checksum"] = compute_checksum({
            "kind": "ledger11_new_candidate_response_placeholder",
...
def _new_payload_candidate_from_proof(proof):
    ...
    ev["send_text_checksum"] = compute_checksum({
        "kind": "ledger11_new_payload_example",
...
def build_artifact_packet(twelfth_send_proof, manual_gate_packet, previous_loop_state_packet=None):
    proof = twelfth_send_proof or {}
    state = build_ledger11_remote_operator_loop_state(
    ...
    examples = {
        ...
        "exact_replay": build_next_gate_precheck_state(
            state, exact_replay_candidate, fresh_operator_gate_id=twelfth_GATE_ID),
        ...
        "same_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, same_payload_candidate, fresh_operator_gate_id="fresh_ledger11_next_gate"),
        "new_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, new_payload_candidate, fresh_operator_gate_id="fresh_ledger11_next_gate"),
    }
    packet = {
        "task_label": TASK_LABEL,
        "model": MODEL,
...
def build_artifact_from_repo(repo_root):
    ...
    twelfth = load_packet(os.path.join(root, twelfth_PROOF_REL))
    manual = load_packet(os.path.join(root, MANUAL_GATE_REL))
    previous = load_packet(os.path.join(root, PREVIOUS_LOOP_REL))
    return build_artifact_packet(twelfth, manual, previous_loop_state_packet=previous)
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger11 remote loop artifacts: scan found %d violation(s)" % len(violations))
