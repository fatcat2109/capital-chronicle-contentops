"""Ledger-6 Telegram remote operator loop state after seventh send.

Task 0174WD/WE/WF. Promotes accepted seventh-send proof into local-only
...
MODEL = "TELEGRAM_LEDGER6_REMOTE_OPERATOR_LOOP_STATE_0174WD_WE_WF"
MODEL_VERSION = "0174WD_WE_WF_TELEGRAM_LEDGER6_REMOTE_OPERATOR_LOOP_STATE_V1"
SOURCE_BASELINE_COMMIT = "053ac56e35e363c0b1ca3f397e384e5a2ded6bcf"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WD_WE_WF")
PACKET_FILENAME = "telegram_ledger6_remote_operator_loop_state_packet.json"
DOC_FILENAME = "telegram_ledger6_remote_operator_loop_state.md"
SEVENTH_PROOF_REL = os.path.join(
    "docs", "automation", "0174WD_WE_WF", "telegram_ledger5_seventh_send_proof_packet.json")
...
RECONCILE_OK = "ledger6_reconciliation_ok_ledger_advanced_to_6"
RECONCILE_MISSING = "ledger6_reconciliation_blocked_missing_seventh_send_proof"
RECONCILE_SEND_NOT_SUCCEEDED = "ledger6_reconciliation_blocked_send_not_succeeded"
RECONCILE_GATE_NOT_REVALIDATED = "ledger6_reconciliation_blocked_manual_gate_not_revalidated"
RECONCILE_LEDGER_NOT_ADVANCED = "ledger6_reconciliation_blocked_ledger_not_advanced"
RECONCILE_MANIFEST_NOT_ADVANCED = "ledger6_reconciliation_blocked_manifest_not_advanced"
RECONCILE_FAIL_CLOSED = "ledger6_reconciliation_fail_closed_forbidden_value"
TRUTH_CLASS = "remote_loop_truth_ledger_count_6_test7_accepted"
...
SEVENTH_GATE_ID = "ledger5_next_manual_gate_transient_local_gate_v1"
...
def _entry_from_proof(proof):
    ...
    entry = ledger.build_ledger_entry(ev, operator_gate_id=SEVENTH_GATE_ID)
    p = proof or {}
...
def reconcile_seventh_send_proof(seventh_send_proof, previous_loop_state_packet=None):
    p = seventh_send_proof or {}
    blockers = []
    ...
    elif not (p.get("ledger_appended") is True and p.get("ledger_entry_count_before") == 5
              and p.get("ledger_entry_count") == 6 and p.get("request_budget_used") == 1):
    ...
    result = {
        ...
        "seventh_send_proof_checksum": p.get("evidence_checksum") or compute_checksum(p) if p else None,
...
def build_ledger6_remote_operator_loop_state(seventh_send_proof, manual_gate_packet,
                                             previous_loop_state_packet=None):
    proof = seventh_send_proof or {}
    ...
    reconciliation = reconcile_seventh_send_proof(proof, prev_packet)
    reconciled = reconciliation.get("reconciled") is True
    entry = _entry_from_proof(proof) if proof else {}
    audit_refs = {
        "seventh_send_proof_checksum": proof.get("evidence_checksum") or compute_checksum(proof) if proof else None,
...
def _candidate_from_proof(proof, *, sequence=None, response_suffix=None):
    ...
    if response_suffix:
        ev["response_checksum"] = compute_checksum({
            "kind": "ledger6_new_candidate_response_placeholder",
...
def _new_payload_candidate_from_proof(proof):
    ev = _candidate_from_proof(proof, sequence=7, response_suffix="new_payload")
    ev["send_text_checksum"] = compute_checksum({
        "kind": "ledger6_new_payload_example",
...
def build_artifact_packet(seventh_send_proof, manual_gate_packet, previous_loop_state_packet=None):
    proof = seventh_send_proof or {}
    state = build_ledger6_remote_operator_loop_state(
    ...
    same_payload_candidate = _candidate_from_proof(proof, sequence=7, response_suffix="same_payload")
    ...
    examples = {
        ...
        "exact_replay": build_next_gate_precheck_state(
            state, exact_replay_candidate, fresh_operator_gate_id=SEVENTH_GATE_ID),
        ...
        "same_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, same_payload_candidate, fresh_operator_gate_id="fresh_ledger6_next_gate"),
        "new_payload_with_fresh_gate": build_next_gate_precheck_state(
            state, new_payload_candidate, fresh_operator_gate_id="fresh_ledger6_next_gate"),
    }
    packet = {
        "task_label": TASK_LABEL,
        "model": MODEL,
...
def build_artifact_from_repo(repo_root):
    ...
    seventh = load_packet(os.path.join(root, SEVENTH_PROOF_REL))
    manual = load_packet(os.path.join(root, MANUAL_GATE_REL))
    previous = load_packet(os.path.join(root, PREVIOUS_LOOP_REL))
    return build_artifact_packet(seventh, manual, previous_loop_state_packet=previous)
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger6 remote loop artifacts: scan found %d violation(s)" % len(violations))
