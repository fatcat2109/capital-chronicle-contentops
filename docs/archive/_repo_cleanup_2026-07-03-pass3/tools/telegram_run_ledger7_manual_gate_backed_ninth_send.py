"""Ledger-7 manual-gate-backed ninth supervised Telegram send runner.

Task 0174WJ/WK/WL. Consumes the accepted ledger-7 manual gate packet, rebuilds
exact test-9 payload + one-request object, verifies ledger-7 remote loop truth,
...
from tools import telegram_build_ledger7_next_manual_gate_packet as builder
...
MODEL = "TELEGRAM_LEDGER7_MANUAL_GATE_BACKED_NINTH_SEND_RUNNER_0174WJ_WK_WL"
MODEL_VERSION = "0174WJ_WK_WL_TELEGRAM_LEDGER7_MANUAL_GATE_BACKED_NINTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "718f5bd7a03729f6bfab2af40d26ee8cd7b6965c"
DOC_REL_DIR = Path("docs/automation/0174WJ_WK_WL")
PACKET_FILENAME = "telegram_ledger7_ninth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger7_ninth_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST9_MESSAGE
...
SEND_OK = "telegram_ledger7_manual_gate_backed_ninth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger7_manual_gate_backed_ninth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger7_manual_gate_backed_ninth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger7_manual_gate_backed_ninth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_7"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 7 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 7:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger7_ninth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger7_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 7
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write ninth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger7_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
