"""Ledger-4 manual-gate-backed sixth supervised Telegram send runner.

Task 0174WA/WB/WC. Consumes the accepted ledger-4 manual gate packet, rebuilds
exact test-6 payload + one-request object, verifies ledger-4 remote loop truth,
...
from tools import telegram_build_ledger4_next_manual_gate_packet as builder
from tools import telegram_run_manual_gate_backed_supervised_send as base_runner

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WA_WB_WC_TELEGRAM_LEDGER4_TO_LEDGER5_SIXTH_SEND_"
    "AND_REMOTE_LOOP_RECONCILIATION_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER4_MANUAL_GATE_BACKED_SIXTH_SEND_RUNNER_0174WA_WB_WC"
MODEL_VERSION = "0174WA_WB_WC_TELEGRAM_LEDGER4_MANUAL_GATE_BACKED_SIXTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "053ac56e35e363c0b1ca3f397e384e5a2ded6bcf"
DOC_REL_DIR = Path("docs/automation/0174WA_WB_WC")
PACKET_FILENAME = "telegram_ledger4_sixth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger4_sixth_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST6_MESSAGE
...
SEND_OK = "telegram_ledger4_manual_gate_backed_sixth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger4_manual_gate_backed_sixth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger4_manual_gate_backed_sixth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger4_manual_gate_backed_sixth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_4"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 4 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 4:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger4_sixth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger4_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 4
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write sixth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger4_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
