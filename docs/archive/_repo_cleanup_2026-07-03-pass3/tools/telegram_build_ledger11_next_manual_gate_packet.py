"""Build Ledger-11 next manual gate packet for supervised Telegram test 13.

Local-only deterministic builder for 0174WV/WW/WX. It consumes the accepted
remote operator loop state at ledger count 11, builds a redacted candidate for live test sequence 13, verifies next-gate precheck through the remote-loop state
...
from live_contentops import telegram_ledger11_remote_operator_loop_state as loop

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WV_WW_WX_TELEGRAM_LEDGER11_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER11_NEXT_MANUAL_GATE_PACKET_BUILDER_0174WV_WW_WX"
MODEL_VERSION = "0174WV_WW_WX_TELEGRAM_LEDGER11_NEXT_MANUAL_GATE_PACKET_BUILDER_V1"
SOURCE_BASELINE_COMMIT = "19c7c24253f8ab948e5034da8ce1739f15c365b8"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WV_WW_WX")
MANUAL_GATE_PACKET_FILENAME = "telegram_ledger11_next_manual_gate_packet.json"
MANUAL_GATE_DOC_FILENAME = "telegram_ledger11_next_manual_gate_packet.md"
BUILDER_PACKET_FILENAME = "telegram_ledger11_next_manual_gate_packet_builder_packet.json"
BUILDER_DOC_FILENAME = "telegram_ledger11_next_manual_gate_packet_builder.md"
REMOTE_LOOP_PACKET_REL = os.path.join(
    "docs", "automation", "0174WS_WT_WU", "telegram_ledger11_remote_operator_loop_state_packet.json")

TEST13_MESSAGE = (
    "Capital Chronicle ContentOps live-gate test 13: ledger-11 remote-loop "
    "manual gate candidate. No market advice."
)
LIVE_TEST_SEQUENCE = 13
DEFAULT_OPERATOR_GATE_ID = "ledger11_next_manual_gate_transient_local_gate_v1"
SYMBOLIC_CREDENTIAL_HANDLE = "ledger11_next_manual_gate_credential_handle_class"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WV_WW_WX_LEDGER11_TO_LEDGER12_THIRTEENTH_"
    "SEND_AND_CONSOLIDATED_EVIDENCE_AUDIT_BATCH_V0"
)

LEDGER11_SOURCE_OK = "ledger11_source_ok"
LEDGER11_SOURCE_MISSING = "ledger11_source_blocked_missing_state"
LEDGER11_SOURCE_UNRECONCILED = "ledger11_source_blocked_unreconciled"
LEDGER11_SOURCE_WRONG_COUNT = "ledger11_source_blocked_wrong_ledger_count"
LEDGER11_SOURCE_MISSING_MANIFEST = "ledger11_source_blocked_missing_manifest"
LEDGER11_SOURCE_FAIL_CLOSED = "ledger11_source_fail_closed_forbidden_value"
...
def validate_ledger11_remote_loop_state_for_next_gate(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    forbidden = bool(scan(packet))
    if forbidden:
        outcome = LEDGER11_SOURCE_FAIL_CLOSED
    elif not packet:
        outcome = LEDGER11_SOURCE_MISSING
    elif (packet.get("reconciliation_outcome_class") != loop.RECONCILE_OK
          or state.get("reconciled") is not True):
        outcome = LEDGER11_SOURCE_UNRECONCILED
    elif packet.get("current_ledger_count") != 11 or state.get("last_successful_send_sequence") != 12:
        outcome = LEDGER11_SOURCE_WRONG_COUNT
    elif not (packet.get("new_ledger_manifest_checksum") and state.get("remote_loop_state_checksum")):
        outcome = LEDGER11_SOURCE_MISSING_MANIFEST
    else:
        outcome = LEDGER11_SOURCE_OK
    blockers = [] if outcome == LEDGER11_SOURCE_OK else [outcome]
    result = {
        "task_label": TASK_LABEL,
        "model": MODEL,
        "model_version": MODEL_VERSION,
        "status": adapter.Status.PASS if outcome == LEDGER11_SOURCE_OK else adapter.Status.BLOCKED,
        "provider": packet.get("provider") or adapter.PROVIDER_TELEGRAM,
        "ledger11_source_validation_outcome_class": outcome,
        "source_valid": outcome == LEDGER11_SOURCE_OK,
    ...
    result["ledger11_source_validation_checksum"] = compute_checksum(result)
    return result


def build_ledger11_next_candidate_evidence(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    rendered = adapter.render_telegram_payload(
        approved_text=TEST13_MESSAGE,
    ...
    evidence = {
        ...
        "candidate_text_class": "ledger11_test13_message_class",
        ...
        "response_checksum": compute_checksum({"kind": "TEST13_response_pending_placeholder"}),
        "response_shape_checksum": compute_checksum({"kind": "TEST13_response_shape_pending_placeholder"}),
...
def build_ledger11_next_manual_gate_packet(remote_loop_state_packet, operator_gate_id=None):
    gate_id = operator_gate_id
    source_validation = validate_ledger11_remote_loop_state_for_next_gate(remote_loop_state_packet)
    candidate_evidence = build_ledger11_next_candidate_evidence(remote_loop_state_packet)
    ...
    if source_validation.get("source_valid") is not True:
        blockers.append("manual_gate_blocker_ledger11_source_not_valid")
    if not gate_id:
...
def build_builder_artifact_packet(remote_loop_state_packet, operator_gate_id=None):
    result = build_ledger11_next_manual_gate_packet(
    ...
    packet = {
        ...
        "source_validation_outcome_class": source_validation.get(
            "ledger11_source_validation_outcome_class"),
        ...
        "test13_message_checksum": evidence.get("send_text_checksum"),
...
def build_manual_gate_doc(manual_gate_packet):
    ...
    return (
        "# 0174WV/WW/WX Ledger-11 Next Manual Gate Packet\n\n"
...
def build_builder_doc(packet):
    return (
        "# 0174WV/WW/WX Ledger-11 Next Manual Gate Packet Builder\n\n"
        ...
        "## Candidate + gate\n\n"
        f"- Test-13 message checksum: `{packet['test13_message_checksum']}`\n"
...
def write_artifacts(base_dir, builder_packet, builder_doc, manual_packet, manual_doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger11 manual gate artifacts: scan found %d violation(s)" % len(violations))
