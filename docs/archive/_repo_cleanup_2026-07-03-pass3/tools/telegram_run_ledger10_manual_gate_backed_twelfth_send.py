"""Ledger-10 Manual-Gate-Backed twelfth supervised Telegram send runner.

Task 0174WS/WT/WU. Consumes the accepted Ledger-10 manual gate packet, rebuilds
exact test-12 payload + one-request object, verifies Ledger-11 remote loop truth,
...
from tools import telegram_build_ledger10_next_manual_gate_packet as builder
from tools import telegram_run_manual_gate_backed_supervised_send as base_runner

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WS_WT_WU_LEDGER10_EVIDENCE_CLEANUP_AND_LEDGER10_TO_LEDGER11_TWELFTH_"
    "SEND_BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER10_MANUAL_GATE_BACKED_TWELFTH_SEND_RUNNER_0174WS_WT_WU"
MODEL_VERSION = "0174WS_WT_WU_TELEGRAM_LEDGER10_MANUAL_GATE_BACKED_TWELFTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "4c3b76b9c9e2eb6495a0638e65253667e0fbe697"
DOC_REL_DIR = Path("docs/automation/0174WS_WT_WU")
PACKET_FILENAME = "telegram_ledger10_twelfth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger10_twelfth_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST12_MESSAGE
...
SEND_OK = "telegram_ledger10_manual_gate_backed_twelfth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger10_manual_gate_backed_twelfth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger10_manual_gate_backed_twelfth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger10_manual_gate_backed_twelfth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_10"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 10 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 10:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger10_twelfth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger10_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 10
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write twelfth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger10_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
