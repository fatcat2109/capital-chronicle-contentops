"""Ledger-11 Manual-Gate-Backed thirteenth supervised Telegram send runner.

Task 0174WV/WW/WX. Consumes the accepted Ledger-11 manual gate packet, rebuilds
exact test-13 payload + one-request object, verifies Ledger-11 remote loop truth,
...
from tools import telegram_build_ledger11_next_manual_gate_packet as builder
...
MODEL = "TELEGRAM_LEDGER11_MANUAL_GATE_BACKED_THIRTEENTH_SEND_RUNNER_0174WV_WW_WX"
MODEL_VERSION = "0174WV_WW_WX_TELEGRAM_LEDGER11_MANUAL_GATE_BACKED_THIRTEENTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "19c7c24253f8ab948e5034da8ce1739f15c365b8"
DOC_REL_DIR = Path("docs/automation/0174WV_WW_WX")
PACKET_FILENAME = "telegram_ledger11_thirteenth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger11_thirteenth_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST13_MESSAGE
...
SEND_OK = "telegram_ledger11_manual_gate_backed_thirteenth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger11_manual_gate_backed_thirteenth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger11_manual_gate_backed_thirteenth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger11_manual_gate_backed_thirteenth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_11"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 11 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 11:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger12_thirteenth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger11_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 11
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write thirteenth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger11_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
