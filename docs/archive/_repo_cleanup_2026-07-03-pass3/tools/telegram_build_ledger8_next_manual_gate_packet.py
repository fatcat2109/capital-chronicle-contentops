"""Build ledger-8 next manual gate packet for supervised Telegram test 10.

Local-only deterministic builder for 0174WM/WN/WO. It consumes the accepted
remote operator loop state at ledger count 8, builds a redacted candidate for
live test sequence 10, verifies next-gate precheck through the remote-loop state
...
from live_contentops import telegram_ledger8_remote_operator_loop_state as loop

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WM_WN_WO_TELEGRAM_LEDGER8_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
MODEL = "TELEGRAM_ledger8_next_manual_gate_PACKET_BUILDER_0174WM_WN_WO"
MODEL_VERSION = "0174WM_WN_WO_TELEGRAM_ledger8_next_manual_gate_PACKET_BUILDER_V1"
SOURCE_BASELINE_COMMIT = "a9c9f2cb727e717dde2b2902414cddc762fe7d72"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WM_WN_WO")
MANUAL_GATE_PACKET_FILENAME = "telegram_ledger8_next_manual_gate_packet.json"
MANUAL_GATE_DOC_FILENAME = "telegram_ledger8_next_manual_gate_packet.md"
BUILDER_PACKET_FILENAME = "telegram_ledger8_next_manual_gate_packet_builder_packet.json"
BUILDER_DOC_FILENAME = "telegram_ledger8_next_manual_gate_packet_builder.md"
REMOTE_LOOP_PACKET_REL = os.path.join(
    "docs", "automation", "0174WJ_WK_WL", "telegram_ledger8_remote_operator_loop_state_packet.json")

TEST10_MESSAGE = (
    "Capital Chronicle ContentOps live-gate test 9: ledger-9 remote-loop "
    "manual gate candidate. No market advice."
)
LIVE_TEST_SEQUENCE = 10
DEFAULT_OPERATOR_GATE_ID = "ledger8_next_manual_gate_transient_local_gate_v1"
SYMBOLIC_CREDENTIAL_HANDLE = "ledger8_next_manual_gate_credential_handle_class"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WM_WN_WO_TELEGRAM_LEDGER8_TO_LEDGER9_TENTH_"
    "SEND_AND_CONSOLIDATED_EVIDENCE_AUDIT_BATCH_V0"
)

ledger8_source_OK = "ledger8_source_ok"
ledger8_source_MISSING = "ledger8_source_blocked_missing_state"
ledger8_source_UNRECONCILED = "ledger8_source_blocked_unreconciled"
ledger8_source_WRONG_COUNT = "ledger8_source_blocked_wrong_ledger_count"
ledger8_source_MISSING_MANIFEST = "ledger8_source_blocked_missing_manifest"
ledger8_source_FAIL_CLOSED = "ledger8_source_fail_closed_forbidden_value"
...
def validate_ledger8_remote_loop_state_for_next_gate(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    forbidden = bool(scan(packet))
    if forbidden:
        outcome = ledger8_source_FAIL_CLOSED
    elif not packet:
        outcome = ledger8_source_MISSING
    elif (packet.get("reconciliation_outcome_class") != loop.RECONCILE_OK
          or state.get("reconciled") is not True):
        outcome = ledger8_source_UNRECONCILED
    elif packet.get("current_ledger_count") != 8 or state.get("last_successful_send_sequence") != 9:
        outcome = ledger8_source_WRONG_COUNT
    elif not (packet.get("new_ledger_manifest_checksum") and state.get("remote_loop_state_checksum")):
        outcome = ledger8_source_MISSING_MANIFEST
    else:
        outcome = ledger8_source_OK
    blockers = [] if outcome == ledger8_source_OK else [outcome]
    result = {
        "task_label": TASK_LABEL,
        "model": MODEL,
        "model_version": MODEL_VERSION,
        "status": adapter.Status.PASS if outcome == ledger8_source_OK else adapter.Status.BLOCKED,
        "provider": packet.get("provider") or adapter.PROVIDER_TELEGRAM,
        "ledger8_source_validation_outcome_class": outcome,
        "source_valid": outcome == ledger8_source_OK,
    ...
    result["ledger8_source_validation_checksum"] = compute_checksum(result)
    return result


def build_ledger8_next_candidate_evidence(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    rendered = adapter.render_telegram_payload(
        approved_text=TEST10_MESSAGE,
    ...
    evidence = {
        ...
        "candidate_text_class": "ledger9_test10_message_class",
        ...
        "response_checksum": compute_checksum({"kind": "test10_response_pending_placeholder"}),
        "response_shape_checksum": compute_checksum({"kind": "test10_response_shape_pending_placeholder"}),
...
def build_ledger8_next_manual_gate_packet(remote_loop_state_packet, operator_gate_id=None):
    gate_id = operator_gate_id
    source_validation = validate_ledger8_remote_loop_state_for_next_gate(remote_loop_state_packet)
    candidate_evidence = build_ledger8_next_candidate_evidence(remote_loop_state_packet)
    ...
    if source_validation.get("source_valid") is not True:
        blockers.append("manual_gate_blocker_ledger8_source_not_valid")
    if not gate_id:
...
def build_builder_artifact_packet(remote_loop_state_packet, operator_gate_id=None):
    result = build_ledger8_next_manual_gate_packet(
    ...
    packet = {
        ...
        "source_validation_outcome_class": source_validation.get(
            "ledger8_source_validation_outcome_class"),
        ...
        "test10_message_checksum": evidence.get("send_text_checksum"),
...
def build_manual_gate_doc(manual_gate_packet):
    ...
    return (
        "# 0174WM/WN/WO Ledger-8 Next Manual Gate Packet\n\n"
...
def build_builder_doc(packet):
    return (
        "# 0174WM/WN/WO Ledger-8 Next Manual Gate Packet Builder\n\n"
        ...
        "## Candidate + gate\n\n"
        f"- Test-10 message checksum: `{packet['test10_message_checksum']}`\n"
...
def write_artifacts(base_dir, builder_packet, builder_doc, manual_packet, manual_doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger8 manual gate artifacts: scan found %d violation(s)" % len(violations))
