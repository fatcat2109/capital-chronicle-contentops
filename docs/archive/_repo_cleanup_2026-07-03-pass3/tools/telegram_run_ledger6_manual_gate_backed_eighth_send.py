"""Ledger-6 manual-gate-backed eighth supervised Telegram send runner.

Task 0174WG/WH/WI. Consumes the accepted ledger-6 manual gate packet, rebuilds
exact test-8 payload + one-request object, verifies ledger-6 remote loop truth,
...
from tools import telegram_build_ledger6_next_manual_gate_packet as builder
...
MODEL = "TELEGRAM_LEDGER6_MANUAL_GATE_BACKED_EIGHTH_SEND_RUNNER_0174WG_WH_WI"
MODEL_VERSION = "0174WG_WH_WI_TELEGRAM_LEDGER6_MANUAL_GATE_BACKED_EIGHTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "67dbf70acbbde4d844d321633318e7ed19ca9015"
DOC_REL_DIR = Path("docs/automation/0174WG_WH_WI")
PACKET_FILENAME = "telegram_ledger6_eighth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger6_eighth_send_proof.md"
...
EXACT_TEST4_PROOF_REL = Path(
    "docs/automation/0174VR_VS_VT/telegram_ledger6_eighth_send_proof_packet.json")

SUPERVISED_TEST_MESSAGE = builder.TEST8_MESSAGE
...
SEND_OK = "telegram_ledger6_manual_gate_backed_eighth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger6_manual_gate_backed_eighth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger6_manual_gate_backed_eighth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger6_manual_gate_backed_eighth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_6"
...
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WG_WH_WI_TELEGRAM_LEDGER6_REMOTE_OPERATOR_LOOP_"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 6 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 6:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger6_eighth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger6_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 6
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write eighth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger6_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
