"""Build ledger-4 next manual gate packet for supervised Telegram test 6.

Local-only deterministic builder for 0174VX/VY/VZ. It consumes the accepted
...
from live_contentops import telegram_ledger4_remote_operator_loop_state as loop

TASK_LABEL = (
    "TASK_CONTENTOPS_0174VX_VY_VZ_TELEGRAM_LEDGER4_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER4_NEXT_MANUAL_GATE_PACKET_BUILDER_0174VX_VY_VZ"
MODEL_VERSION = "0174VX_VY_VZ_TELEGRAM_LEDGER4_NEXT_MANUAL_GATE_PACKET_BUILDER_V1"
SOURCE_BASELINE_COMMIT = "fe54724f26e1fddfedee3cf897afef67506a80e4"
DOC_REL_DIR = os.path.join("docs", "automation", "0174VX_VY_VZ")
MANUAL_GATE_PACKET_FILENAME = "telegram_ledger4_next_manual_gate_packet.json"
MANUAL_GATE_DOC_FILENAME = "telegram_ledger4_next_manual_gate_packet.md"
BUILDER_PACKET_FILENAME = "telegram_ledger4_next_manual_gate_packet_builder_packet.json"
BUILDER_DOC_FILENAME = "telegram_ledger4_next_manual_gate_packet_builder.md"
REMOTE_LOOP_PACKET_REL = os.path.join(
    "docs", "automation", "0174VU_VV_VW", "telegram_ledger4_remote_operator_loop_state_packet.json")

TEST6_MESSAGE = (
    "Capital Chronicle ContentOps live-gate test 6: ledger-4 remote-loop "
    "manual gate candidate. No market advice."
)
LIVE_TEST_SEQUENCE = 6
DEFAULT_OPERATOR_GATE_ID = "ledger4_next_manual_gate_transient_local_gate_v1"
SYMBOLIC_CREDENTIAL_HANDLE = "ledger4_next_manual_gate_credential_handle_class"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WA_WB_WC_TELEGRAM_LEDGER4_MANUAL_GATE_BACKED_SIXTH_"
    "SUPERVISED_SEND_RUNNER_BATCH_V0"
)

LEDGER4_SOURCE_OK = "ledger4_source_ok"
LEDGER4_SOURCE_MISSING = "ledger4_source_blocked_missing_state"
LEDGER4_SOURCE_UNRECONCILED = "ledger4_source_blocked_unreconciled"
LEDGER4_SOURCE_WRONG_COUNT = "ledger4_source_blocked_wrong_ledger_count"
LEDGER4_SOURCE_MISSING_MANIFEST = "ledger4_source_blocked_missing_manifest"
LEDGER4_SOURCE_FAIL_CLOSED = "ledger4_source_fail_closed_forbidden_value"
...
def validate_ledger4_remote_loop_state_for_next_gate(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    forbidden = bool(scan(packet))
    if forbidden:
        outcome = LEDGER4_SOURCE_FAIL_CLOSED
    elif not packet:
        outcome = LEDGER4_SOURCE_MISSING
    elif (packet.get("reconciliation_outcome_class") != loop.RECONCILE_OK
          or state.get("reconciled") is not True):
        outcome = LEDGER4_SOURCE_UNRECONCILED
    elif packet.get("current_ledger_count") != 4 or state.get("last_successful_send_sequence") != 5:
        outcome = LEDGER4_SOURCE_WRONG_COUNT
    elif not (packet.get("new_ledger_manifest_checksum") and state.get("remote_loop_state_checksum")):
        outcome = LEDGER4_SOURCE_MISSING_MANIFEST
    else:
        outcome = LEDGER4_SOURCE_OK
    blockers = [] if outcome == LEDGER4_SOURCE_OK else [outcome]
    result = {
        "task_label": TASK_LABEL,
        "model": MODEL,
        "model_version": MODEL_VERSION,
        "status": adapter.Status.PASS if outcome == LEDGER4_SOURCE_OK else adapter.Status.BLOCKED,
        "provider": packet.get("provider") or adapter.PROVIDER_TELEGRAM,
        "ledger4_source_validation_outcome_class": outcome,
        "source_valid": outcome == LEDGER4_SOURCE_OK,
    ...
    result["ledger4_source_validation_checksum"] = compute_checksum(result)
    return result


def build_ledger4_next_candidate_evidence(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    rendered = adapter.render_telegram_payload(
        approved_text=TEST6_MESSAGE,
    ...
    evidence = {
        ...
        "candidate_text_class": "ledger4_test6_message_class",
        ...
        "response_checksum": compute_checksum({"kind": "test6_response_pending_placeholder"}),
        "response_shape_checksum": compute_checksum({"kind": "test6_response_shape_pending_placeholder"}),
...
def build_ledger4_next_manual_gate_packet(remote_loop_state_packet, operator_gate_id=None):
    gate_id = operator_gate_id
    source_validation = validate_ledger4_remote_loop_state_for_next_gate(remote_loop_state_packet)
    candidate_evidence = build_ledger4_next_candidate_evidence(remote_loop_state_packet)
    ...
    if source_validation.get("source_valid") is not True:
        blockers.append("manual_gate_blocker_ledger4_source_not_valid")
    if not gate_id:
...
def build_builder_artifact_packet(remote_loop_state_packet, operator_gate_id=None):
    result = build_ledger4_next_manual_gate_packet(
    ...
    packet = {
        ...
        "source_validation_outcome_class": source_validation.get(
            "ledger4_source_validation_outcome_class"),
        ...
        "test6_message_checksum": evidence.get("send_text_checksum"),
...
def build_manual_gate_doc(manual_gate_packet):
    ...
    return (
        "# 0174VX/VY/VZ Ledger-4 Next Manual Gate Packet\n\n"
...
def build_builder_doc(packet):
    return (
        "# 0174VX/VY/VZ Ledger-4 Next Manual Gate Packet Builder\n\n"
        ...
        "## Candidate + gate\n\n"
        f"- Test-6 message checksum: `{packet['test6_message_checksum']}`\n"
...
def write_artifacts(base_dir, builder_packet, builder_doc, manual_packet, manual_doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger4 manual gate artifacts: scan found %d violation(s)" % len(violations))
