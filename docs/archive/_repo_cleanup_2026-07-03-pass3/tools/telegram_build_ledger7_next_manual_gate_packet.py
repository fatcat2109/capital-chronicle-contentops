"""Build ledger-7 next manual gate packet for supervised Telegram test 9.

Local-only deterministic builder for 0174WJ/WK/WL. It consumes the accepted
remote operator loop state at ledger count 7, builds a redacted candidate for
live test sequence 9, verifies next-gate precheck through the remote-loop state
...
from live_contentops import telegram_ledger7_remote_operator_loop_state as loop

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WJ_WK_WL_TELEGRAM_LEDGER7_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER7_NEXT_MANUAL_GATE_PACKET_BUILDER_0174WJ_WK_WL"
MODEL_VERSION = "0174WJ_WK_WL_TELEGRAM_LEDGER7_NEXT_MANUAL_GATE_PACKET_BUILDER_V1"
SOURCE_BASELINE_COMMIT = "718f5bd7a03729f6bfab2af40d26ee8cd7b6965c"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WJ_WK_WL")
MANUAL_GATE_PACKET_FILENAME = "telegram_ledger7_next_manual_gate_packet.json"
MANUAL_GATE_DOC_FILENAME = "telegram_ledger7_next_manual_gate_packet.md"
BUILDER_PACKET_FILENAME = "telegram_ledger7_next_manual_gate_packet_builder_packet.json"
BUILDER_DOC_FILENAME = "telegram_ledger7_next_manual_gate_packet_builder.md"
REMOTE_LOOP_PACKET_REL = os.path.join(
    "docs", "automation", "0174WG_WH_WI", "telegram_ledger7_remote_operator_loop_state_packet.json")

TEST9_MESSAGE = (
    "Capital Chronicle ContentOps live-gate test 9: ledger-7 remote-loop "
    "manual gate candidate. No market advice."
)
LIVE_TEST_SEQUENCE = 9
DEFAULT_OPERATOR_GATE_ID = "ledger7_next_manual_gate_transient_local_gate_v1"
SYMBOLIC_CREDENTIAL_HANDLE = "ledger7_next_manual_gate_credential_handle_class"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WJ_WK_WL_LEDGER7_EVIDENCE_METADATA_CLEANUP_AND_"
    "LEDGER7_TO_LEDGER8_NINTH_SEND_BATCH_V0"
)

LEDGER7_SOURCE_OK = "ledger7_source_ok"
LEDGER7_SOURCE_MISSING = "ledger7_source_blocked_missing_state"
LEDGER7_SOURCE_UNRECONCILED = "ledger7_source_blocked_unreconciled"
LEDGER7_SOURCE_WRONG_COUNT = "ledger7_source_blocked_wrong_ledger_count"
LEDGER7_SOURCE_MISSING_MANIFEST = "ledger7_source_blocked_missing_manifest"
LEDGER7_SOURCE_FAIL_CLOSED = "ledger7_source_fail_closed_forbidden_value"
...
def validate_ledger7_remote_loop_state_for_next_gate(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    forbidden = bool(scan(packet))
    if forbidden:
        outcome = LEDGER7_SOURCE_FAIL_CLOSED
    elif not packet:
        outcome = LEDGER7_SOURCE_MISSING
    elif (packet.get("reconciliation_outcome_class") != loop.RECONCILE_OK
          or state.get("reconciled") is not True):
        outcome = LEDGER7_SOURCE_UNRECONCILED
    elif packet.get("current_ledger_count") != 7 or state.get("last_successful_send_sequence") != 8:
        outcome = LEDGER7_SOURCE_WRONG_COUNT
    elif not (packet.get("new_ledger_manifest_checksum") and state.get("remote_loop_state_checksum")):
        outcome = LEDGER7_SOURCE_MISSING_MANIFEST
    else:
        outcome = LEDGER7_SOURCE_OK
    blockers = [] if outcome == LEDGER7_SOURCE_OK else [outcome]
    result = {
        "task_label": TASK_LABEL,
        "model": MODEL,
        "model_version": MODEL_VERSION,
        "status": adapter.Status.PASS if outcome == LEDGER7_SOURCE_OK else adapter.Status.BLOCKED,
        "provider": packet.get("provider") or adapter.PROVIDER_TELEGRAM,
        "ledger7_source_validation_outcome_class": outcome,
        "source_valid": outcome == LEDGER7_SOURCE_OK,
    ...
    result["ledger7_source_validation_checksum"] = compute_checksum(result)
    return result


def build_ledger7_next_candidate_evidence(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    rendered = adapter.render_telegram_payload(
        approved_text=TEST9_MESSAGE,
    ...
    evidence = {
        ...
        "candidate_text_class": "ledger7_test9_message_class",
        ...
        "response_checksum": compute_checksum({"kind": "test9_response_pending_placeholder"}),
        "response_shape_checksum": compute_checksum({"kind": "test9_response_shape_pending_placeholder"}),
...
def build_ledger7_next_manual_gate_packet(remote_loop_state_packet, operator_gate_id=None):
    gate_id = operator_gate_id
    source_validation = validate_ledger7_remote_loop_state_for_next_gate(remote_loop_state_packet)
    candidate_evidence = build_ledger7_next_candidate_evidence(remote_loop_state_packet)
    ...
    if source_validation.get("source_valid") is not True:
        blockers.append("manual_gate_blocker_ledger7_source_not_valid")
    if not gate_id:
...
def build_builder_artifact_packet(remote_loop_state_packet, operator_gate_id=None):
    result = build_ledger7_next_manual_gate_packet(
    ...
    packet = {
        ...
        "source_validation_outcome_class": source_validation.get(
            "ledger7_source_validation_outcome_class"),
        ...
        "test9_message_checksum": evidence.get("send_text_checksum"),
...
def build_manual_gate_doc(manual_gate_packet):
    ...
    return (
        "# 0174WJ/WK/WL Ledger-7 Next Manual Gate Packet\n\n"
...
def build_builder_doc(packet):
    return (
        "# 0174WJ/WK/WL Ledger-7 Next Manual Gate Packet Builder\n\n"
        ...
        "## Candidate + gate\n\n"
        f"- Test-9 message checksum: `{packet['test9_message_checksum']}`\n"
...
def write_artifacts(base_dir, builder_packet, builder_doc, manual_packet, manual_doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger7 manual gate artifacts: scan found %d violation(s)" % len(violations))
