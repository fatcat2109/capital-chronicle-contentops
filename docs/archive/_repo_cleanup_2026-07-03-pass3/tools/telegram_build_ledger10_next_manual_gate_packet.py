"""Build Ledger-10 next manual gate packet for supervised Telegram test 12.

Local-only deterministic builder for 0174WS/WT/WU. It consumes the accepted
remote operator loop state at ledger count 10, builds a redacted candidate for live test sequence 12, verifies next-gate precheck through the remote-loop state
...
from live_contentops import telegram_ledger10_remote_operator_loop_state as loop

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WS_WT_WU_TELEGRAM_LEDGER10_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER10_NEXT_MANUAL_GATE_PACKET_BUILDER_0174WS_WT_WU"
MODEL_VERSION = "0174WS_WT_WU_TELEGRAM_LEDGER10_NEXT_MANUAL_GATE_PACKET_BUILDER_V1"
SOURCE_BASELINE_COMMIT = "4c3b76b9c9e2eb6495a0638e65253667e0fbe697"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WS_WT_WU")
MANUAL_GATE_PACKET_FILENAME = "telegram_ledger10_next_manual_gate_packet.json"
MANUAL_GATE_DOC_FILENAME = "telegram_ledger10_next_manual_gate_packet.md"
BUILDER_PACKET_FILENAME = "telegram_ledger10_next_manual_gate_packet_builder_packet.json"
BUILDER_DOC_FILENAME = "telegram_ledger10_next_manual_gate_packet_builder.md"
REMOTE_LOOP_PACKET_REL = os.path.join(
    "docs", "automation", "0174WP_WQ_WR", "telegram_ledger10_remote_operator_loop_state_packet.json")

TEST12_MESSAGE = (
    "Capital Chronicle ContentOps live-gate test 12: ledger-10 remote-loop "
    "manual gate candidate. No market advice."
)
LIVE_TEST_SEQUENCE = 12
DEFAULT_OPERATOR_GATE_ID = "ledger10_next_manual_gate_transient_local_gate_v1"
SYMBOLIC_CREDENTIAL_HANDLE = "ledger10_next_manual_gate_credential_handle_class"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WS_WT_WU_TELEGRAM_LEDGER11_TO_LEDGER11_TWELFTH_"
    "SEND_AND_CONSOLIDATED_EVIDENCE_AUDIT_BATCH_V0"
)

LEDGER10_SOURCE_OK = "ledger10_source_ok"
LEDGER10_SOURCE_MISSING = "ledger10_source_blocked_missing_state"
LEDGER10_SOURCE_UNRECONCILED = "ledger10_source_blocked_unreconciled"
LEDGER10_SOURCE_WRONG_COUNT = "ledger10_source_blocked_wrong_ledger_count"
LEDGER10_SOURCE_MISSING_MANIFEST = "ledger10_source_blocked_missing_manifest"
LEDGER10_SOURCE_FAIL_CLOSED = "ledger10_source_fail_closed_forbidden_value"
...
def validate_ledger10_remote_loop_state_for_next_gate(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    forbidden = bool(scan(packet))
    if forbidden:
        outcome = LEDGER10_SOURCE_FAIL_CLOSED
    elif not packet:
        outcome = LEDGER10_SOURCE_MISSING
    elif (packet.get("reconciliation_outcome_class") != loop.RECONCILE_OK
          or state.get("reconciled") is not True):
        outcome = LEDGER10_SOURCE_UNRECONCILED
    elif packet.get("current_ledger_count") != 10 or state.get("last_successful_send_sequence") != 11:
        outcome = LEDGER10_SOURCE_WRONG_COUNT
    elif not (packet.get("new_ledger_manifest_checksum") and state.get("remote_loop_state_checksum")):
        outcome = LEDGER10_SOURCE_MISSING_MANIFEST
    else:
        outcome = LEDGER10_SOURCE_OK
    blockers = [] if outcome == LEDGER10_SOURCE_OK else [outcome]
    result = {
        "task_label": TASK_LABEL,
        "model": MODEL,
        "model_version": MODEL_VERSION,
        "status": adapter.Status.PASS if outcome == LEDGER10_SOURCE_OK else adapter.Status.BLOCKED,
        "provider": packet.get("provider") or adapter.PROVIDER_TELEGRAM,
        "ledger10_source_validation_outcome_class": outcome,
        "source_valid": outcome == LEDGER10_SOURCE_OK,
    ...
    result["ledger10_source_validation_checksum"] = compute_checksum(result)
    return result


def build_ledger10_next_candidate_evidence(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    rendered = adapter.render_telegram_payload(
        approved_text=TEST12_MESSAGE,
    ...
    evidence = {
        ...
        "candidate_text_class": "ledger10_test12_message_class",
        ...
        "response_checksum": compute_checksum({"kind": "TEST12_response_pending_placeholder"}),
        "response_shape_checksum": compute_checksum({"kind": "TEST12_response_shape_pending_placeholder"}),
...
def build_ledger10_next_manual_gate_packet(remote_loop_state_packet, operator_gate_id=None):
    gate_id = operator_gate_id
    source_validation = validate_ledger10_remote_loop_state_for_next_gate(remote_loop_state_packet)
    candidate_evidence = build_ledger10_next_candidate_evidence(remote_loop_state_packet)
    ...
    if source_validation.get("source_valid") is not True:
        blockers.append("manual_gate_blocker_ledger10_source_not_valid")
    if not gate_id:
        blockers.append("manual_gate_blocker_missing_fresh_operator_gate")
    if precheck.get("next_gate_outcome_class") != loop.loop3.NEXT_CLEAR:
    ...
    manual_gate_packet["source_current_ledger_count"] = candidate[
        "source_current_ledger_count"]
    manual_gate_packet["source_baseline_commit"] = SOURCE_BASELINE_COMMIT
    manual_gate_packet["next_recommended_task"] = NEXT_RECOMMENDED_TASK
    manual_gate_packet["baseline_matched"] = True
    manual_gate_packet["start_head"] = SOURCE_BASELINE_COMMIT
    manual_gate_packet["builder_metadata_binding_checksum"] = compute_checksum({
        "source_baseline_commit": SOURCE_BASELINE_COMMIT,
        "next_recommended_task": NEXT_RECOMMENDED_TASK,
        "manual_gate_task_label": TASK_LABEL,
    })
...
def build_builder_artifact_packet(remote_loop_state_packet, operator_gate_id=None):
    result = build_ledger10_next_manual_gate_packet(
    ...
    packet = {
        ...
        "source_validation_outcome_class": source_validation.get(
            "ledger10_source_validation_outcome_class"),
        ...
        "test12_message_checksum": evidence.get("send_text_checksum"),
...
def build_manual_gate_doc(manual_gate_packet):
    ...
    return (
        "# 0174WS/WT/WU Ledger-10 Next Manual Gate Packet\n\n"
...
def build_builder_doc(packet):
    return (
        "# 0174WS/WT/WU Ledger-10 Next Manual Gate Packet Builder\n\n"
        ...
        "## Candidate + gate\n\n"
        f"- Test-12 message checksum: `{packet['test12_message_checksum']}`\n"
...
def write_artifacts(base_dir, builder_packet, builder_doc, manual_packet, manual_doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger10 manual gate artifacts: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    manual_packet["start_head"] = builder_packet["start_head"]
    manual_packet["baseline_matched"] = builder_packet["baseline_matched"]
    manual_packet["manual_gate_packet_checksum"] = compute_checksum(manual_packet)
