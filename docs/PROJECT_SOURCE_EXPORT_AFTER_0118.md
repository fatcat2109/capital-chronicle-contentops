# Project Source Export (After 0118)

**Task:** `TASK_CONTENTOPS_0119_PROJECT_SOURCES_REFRESH_AFTER_0118_V0`
**Baseline:** `master`, HEAD `5b6b493`

This bundle represents the safe, local-only snapshot of the pre-alpha ContentOps system after the operator workflow consolidation (task 0118). It is curated specifically to be uploaded to ChatGPT as Project Sources.
...
1. Delete any older `cc-live-contentops` bundles (e.g., `AFTER_0112`, `AFTER_0108`, `AFTER_0101`, `AFTER_0074`).
2. Upload the new contents of the `project_sources_bundle_AFTER_0118` folder.
3. This ensures the LLM understands the newly consolidated 0118 workflow.
