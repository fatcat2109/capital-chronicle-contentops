"""Ledger-8 Manual-Gate-Backed tenth supervised Telegram send runner.

Task 0174WM/WN/WO. Consumes the accepted ledger-8 manual gate packet, rebuilds
exact test-10 payload + one-request object, verifies ledger-8 remote loop truth,
...
from tools import telegram_build_ledger8_next_manual_gate_packet as builder
...
MODEL = "TELEGRAM_LEDGER8_MANUAL_GATE_BACKED_TENTH_SEND_RUNNER_0174WM_WN_WO"
MODEL_VERSION = "0174WM_WN_WO_TELEGRAM_LEDGER8_MANUAL_GATE_BACKED_TENTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "a9c9f2cb727e717dde2b2902414cddc762fe7d72"
DOC_REL_DIR = Path("docs/automation/0174WM_WN_WO")
PACKET_FILENAME = "telegram_ledger8_tenth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger8_tenth_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST10_MESSAGE
...
SEND_OK = "telegram_ledger8_manual_gate_backed_tenth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger8_manual_gate_backed_tenth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger8_manual_gate_backed_tenth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger8_manual_gate_backed_tenth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_8"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 8 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 8:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger8_tenth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger8_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 8
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write tenth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger8_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
