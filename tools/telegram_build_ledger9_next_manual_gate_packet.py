"""Build Ledger-9 next manual gate packet for supervised Telegram test 11.

Local-only deterministic builder for 0174WP/WQ/WR. It consumes the accepted
remote operator loop state at ledger count 9, builds a redacted candidate for live test sequence 11, verifies next-gate precheck through the remote-loop state
...
from live_contentops import telegram_ledger9_remote_operator_loop_state as loop

TASK_LABEL = (
    "TASK_CONTENTOPS_0174WP_WQ_WR_TELEGRAM_LEDGER9_NEXT_MANUAL_GATE_PACKET_"
    "BATCH_V0"
)
MODEL = "TELEGRAM_LEDGER9_NEXT_MANUAL_GATE_PACKET_BUILDER_0174WP_WQ_WR"
MODEL_VERSION = "0174WP_WQ_WR_TELEGRAM_LEDGER9_NEXT_MANUAL_GATE_PACKET_BUILDER_V1"
SOURCE_BASELINE_COMMIT = "d0c9474e76bcd5c1e7b0c305d5d6c7d98585cf22"
DOC_REL_DIR = os.path.join("docs", "automation", "0174WP_WQ_WR")
MANUAL_GATE_PACKET_FILENAME = "telegram_ledger9_next_manual_gate_packet.json"
MANUAL_GATE_DOC_FILENAME = "telegram_ledger9_next_manual_gate_packet.md"
BUILDER_PACKET_FILENAME = "telegram_ledger9_next_manual_gate_packet_builder_packet.json"
BUILDER_DOC_FILENAME = "telegram_ledger9_next_manual_gate_packet_builder.md"
REMOTE_LOOP_PACKET_REL = os.path.join(
    "docs", "automation", "0174WM_WN_WO", "telegram_ledger9_remote_operator_loop_state_packet.json")

TEST11_MESSAGE = (
    "Capital Chronicle ContentOps live-gate test 11: ledger-9 remote-loop "
    "manual gate candidate. No market advice."
)
LIVE_TEST_SEQUENCE = 11
DEFAULT_OPERATOR_GATE_ID = "ledger9_next_manual_gate_transient_local_gate_v1"
SYMBOLIC_CREDENTIAL_HANDLE = "ledger9_next_manual_gate_credential_handle_class"
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WP_WQ_WR_TELEGRAM_LEDGER9_TO_LEDGER10_ELEVENTH_"
    "SEND_AND_CONSOLIDATED_EVIDENCE_AUDIT_BATCH_V0"
)

LEDGER9_SOURCE_OK = "ledger9_source_ok"
LEDGER9_SOURCE_MISSING = "ledger9_source_blocked_missing_state"
LEDGER9_SOURCE_UNRECONCILED = "ledger9_source_blocked_unreconciled"
LEDGER9_SOURCE_WRONG_COUNT = "ledger9_source_blocked_wrong_ledger_count"
LEDGER9_SOURCE_MISSING_MANIFEST = "ledger9_source_blocked_missing_manifest"
LEDGER9_SOURCE_FAIL_CLOSED = "ledger9_source_fail_closed_forbidden_value"
...
def validate_ledger9_remote_loop_state_for_next_gate(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    forbidden = bool(scan(packet))
    if forbidden:
        outcome = LEDGER9_SOURCE_FAIL_CLOSED
    elif not packet:
        outcome = LEDGER9_SOURCE_MISSING
    elif (packet.get("reconciliation_outcome_class") != loop.RECONCILE_OK
          or state.get("reconciled") is not True):
        outcome = LEDGER9_SOURCE_UNRECONCILED
    elif packet.get("current_ledger_count") != 9 or state.get("last_successful_send_sequence") != 10:
        outcome = LEDGER9_SOURCE_WRONG_COUNT
    elif not (packet.get("new_ledger_manifest_checksum") and state.get("remote_loop_state_checksum")):
        outcome = LEDGER9_SOURCE_MISSING_MANIFEST
    else:
        outcome = LEDGER9_SOURCE_OK
    blockers = [] if outcome == LEDGER9_SOURCE_OK else [outcome]
    result = {
        "task_label": TASK_LABEL,
        "model": MODEL,
        "model_version": MODEL_VERSION,
        "status": adapter.Status.PASS if outcome == LEDGER9_SOURCE_OK else adapter.Status.BLOCKED,
        "provider": packet.get("provider") or adapter.PROVIDER_TELEGRAM,
        "ledger9_source_validation_outcome_class": outcome,
        "source_valid": outcome == LEDGER9_SOURCE_OK,
    ...
    result["ledger9_source_validation_checksum"] = compute_checksum(result)
    return result


def build_ledger9_next_candidate_evidence(remote_loop_state_packet):
    packet = remote_loop_state_packet or {}
    state = packet.get("remote_loop_state") or {}
    rendered = adapter.render_telegram_payload(
        approved_text=TEST11_MESSAGE,
    ...
    evidence = {
        ...
        "candidate_text_class": "ledger9_test11_message_class",
        ...
        "response_checksum": compute_checksum({"kind": "TEST11_response_pending_placeholder"}),
        "response_shape_checksum": compute_checksum({"kind": "TEST11_response_shape_pending_placeholder"}),
...
def build_ledger9_next_manual_gate_packet(remote_loop_state_packet, operator_gate_id=None):
    gate_id = operator_gate_id
    source_validation = validate_ledger9_remote_loop_state_for_next_gate(remote_loop_state_packet)
    candidate_evidence = build_ledger9_next_candidate_evidence(remote_loop_state_packet)
    ...
    if source_validation.get("source_valid") is not True:
        blockers.append("manual_gate_blocker_ledger9_source_not_valid")
    if not gate_id:
...
def build_builder_artifact_packet(remote_loop_state_packet, operator_gate_id=None):
    result = build_ledger9_next_manual_gate_packet(
    ...
    packet = {
        ...
        "source_validation_outcome_class": source_validation.get(
            "ledger9_source_validation_outcome_class"),
        ...
        "test11_message_checksum": evidence.get("send_text_checksum"),
...
def build_manual_gate_doc(manual_gate_packet):
    ...
    return (
        "# 0174WP/WQ/WR Ledger-9 Next Manual Gate Packet\n\n"
...
def build_builder_doc(packet):
    return (
        "# 0174WP/WQ/WR Ledger-9 Next Manual Gate Packet Builder\n\n"
        ...
        "## Candidate + gate\n\n"
        f"- Test-11 message checksum: `{packet['test11_message_checksum']}`\n"
...
def write_artifacts(base_dir, builder_packet, builder_doc, manual_packet, manual_doc):
    ...
    if violations:
        raise RuntimeError(
            "refusing to write ledger9 manual gate artifacts: scan found %d violation(s)" % len(violations))
