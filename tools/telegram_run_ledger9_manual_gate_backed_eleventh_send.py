"""Ledger-9 Manual-Gate-Backed eleventh supervised Telegram send runner.

Task 0174WP/WQ/WR. Consumes the accepted Ledger-9 manual gate packet, rebuilds
exact test-10 payload + one-request object, verifies Ledger-9 remote loop truth,
...
from tools import telegram_build_ledger9_next_manual_gate_packet as builder
...
MODEL = "TELEGRAM_ledger9_MANUAL_GATE_BACKED_eleventh_SEND_RUNNER_0174WP_WQ_WR"
MODEL_VERSION = "0174WP_WQ_WR_TELEGRAM_ledger9_MANUAL_GATE_BACKED_eleventh_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "d0c9474e76bcd5c1e7b0c305d5d6c7d98585cf22"
DOC_REL_DIR = Path("docs/automation/0174WP_WQ_WR")
PACKET_FILENAME = "telegram_ledger9_eleventh_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger9_eleventh_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST11_MESSAGE
...
SEND_OK = "telegram_ledger9_manual_gate_backed_eleventh_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger9_manual_gate_backed_eleventh_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger9_manual_gate_backed_eleventh_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger9_manual_gate_backed_eleventh_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_9"
...
NEXT_RECOMMENDED_TASK = (
    "TASK_CONTENTOPS_0174WP_WQ_WR_TELEGRAM_LEDGER9_REMOTE_OPERATOR_LOOP_"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    ...
    return [entry] * 9 if entry.get("ledger_entry_checksum") else []
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 9:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger9_eleventh_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger9_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 9
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write eleventh-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    run = run_ledger9_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
