"""Ledger-3 manual-gate-backed fifth supervised Telegram send runner.

Task 0174VR/VS/VT. Consumes the accepted ledger-3 manual gate packet, rebuilds
exact test-5 payload + one-request object, verifies ledger-3 remote loop truth,
...
from tools import telegram_build_ledger3_next_manual_gate_packet as builder
...
MODEL = "TELEGRAM_LEDGER3_MANUAL_GATE_BACKED_FIFTH_SEND_RUNNER_0174VR_VS_VT"
MODEL_VERSION = "0174VR_VS_VT_TELEGRAM_LEDGER3_MANUAL_GATE_BACKED_FIFTH_SEND_RUNNER_V1"
REQUIRED_BASELINE_COMMIT = "0525d39aa28377eeabbdd0ae2e467e374039c347"
DOC_REL_DIR = Path("docs/automation/0174VR_VS_VT")
PACKET_FILENAME = "telegram_ledger3_fifth_send_proof_packet.json"
DOC_FILENAME = "telegram_ledger3_fifth_send_proof.md"
...
SUPERVISED_TEST_MESSAGE = builder.TEST5_MESSAGE
...
SEND_OK = "telegram_ledger3_manual_gate_backed_fifth_send_ok_redacted"
SEND_PROVIDER_ERROR = "telegram_ledger3_manual_gate_backed_fifth_send_provider_error_redacted"
SEND_NETWORK_ERROR = "telegram_ledger3_manual_gate_backed_fifth_send_network_error_redacted"
SEND_BLOCKED = "telegram_ledger3_manual_gate_backed_fifth_send_blocked_before_network"
...
BLOCK_WRONG_LEDGER_COUNT = "source_ledger_count_not_3"
...
def load_existing_ledger_entries(repo_root, remote_loop_packet=None):
    root = Path(repo_root)
    entries = base_runner.load_existing_ledger_entries(
        load_json(root / LEDGER_PACKET_REL), load_json(root / THIRD_PROOF_REL))
    state = (remote_loop_packet or {}).get("remote_loop_state") or {}
    entry = state.get("accepted_ledger_entry") or {}
    if entry.get("ledger_entry_checksum"):
...
def validate_manual_gate_packet(packet, remote_loop_packet=None):
    ...
    if p.get("source_current_ledger_count") != 3:
...
def build_candidate_evidence(remote_loop_packet, rendered, one_request, *,
    ...
    packet = {
        ...
        "response_checksum": compute_checksum({
            "kind": "ledger3_fifth_candidate_response_placeholder",
            "live_test_sequence": LIVE_TEST_SEQUENCE,
        }),
...
def run_ledger3_manual_gate_backed_send(*, manual_gate_packet=None, remote_loop_packet=None,
    ...
    manual_gate_revalidated = (
        ...
        and manual_gate.get("source_current_ledger_count") == 3
...
def build_proof_doc(packet):
    return (
        "# 0174VR/VS/VT Ledger-3 Manual-Gate-Backed Fifth Send Proof\n\n"
...
def write_artifacts(base_dir, packet, doc):
    ...
    if violations:
        raise RuntimeError("refusing to write fifth-send proof: scan found %d violation(s)" % len(violations))
...
def main(argv=None):
    ...
    old_manifest = ledger.compute_ledger_manifest_checksum(existing)
    start_head = _git_head("HEAD")
    origin_head = _git_head("origin/master")
    run = run_ledger3_manual_gate_backed_send(
        manual_gate_packet=manual_gate,
